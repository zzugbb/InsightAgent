...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1831/1831)
...
latest_validation_note: registry-governance 进度约 67%；本轮补 audit detail、trace meta、runtime facade 与 registry artifact provider_sources 列表冲突脱敏 alias 红测/覆盖，并统一复用稳定 `#n` 后缀 alias map。alias targeted 17/17、provider source artifact 2/2、audit 15/15、trace 178/178、registry 533/533、backend full slice 1831/1831 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 67%；已完成 diagnostics_summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options 与唯一脱敏 alias round-trip、冲突 alias 可回写后缀、session governance provider_sources 冲突 alias、audit/trace/runtime artifact provider_sources 冲突 alias、task list/usage dashboard route/service filter alias round-trip、response summary helper provider source 脱敏、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export/audit/SSE 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1831/1831`。
...
- `registry-governance`：进度约 `67%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options 与唯一脱敏 alias round-trip、冲突 alias 可回写后缀、session governance provider_sources 冲突 alias、audit/trace/runtime artifact provider_sources 冲突 alias、task list/usage dashboard route/service filter alias round-trip、response summary helper provider source 脱敏、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，不改变 settings/preflight/trace/export/audit/SSE 可见字段 shape。
