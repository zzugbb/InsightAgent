...
overview: provider-tool-expansion 已 100% 封板；当前完成后端 3000 行文件规模治理，下一候选为 ci-release-engineering。
current_focus:
  mainline: provider-tool-expansion
  status: 100% 封板，进入维护收口
  next_candidate: ci-release-engineering
  latest_change: 继续拆分 runtime、chat persistence 与 tool_runtime_slice 大文件，保持旧导出路径和外部契约稳定
  file_size_baseline:
    scope: backend/app 与 backend/scripts
    boundary: 所有 Python 文件 <= 3000 行
    largest_backend_python: scripts/tool_runtime_slice/planning_provider.py 2923 行
    key_facades: tool_runtime_execution.py 2864、tool_runtime_registry.py 2768、tool_runtime.py 2547、tool_runtime_http_json.py 2522、chat_persistence_service.py 1755
...
validation_baseline:
  ...
  module_boundaries: backend/scripts/test_tool_runtime_module_boundaries.py 4/4，含 3000 行规模边界
  frontend: node tests 121/121; npm run lint; npm run build
  e2e_baseline: backend main passed; frontend full Chromium 52 passed / 1 skipped
