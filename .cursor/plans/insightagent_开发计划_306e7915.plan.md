...
latest_validation_note: concurrency-fairness-policy 本轮增量：先补 settings diagnostics 与前端 model settings 红测，确认后端 task_queue_diagnostics 尚未暴露 waiting_policy/capacity_aware_fifo_enabled、前端摘要尚未展示 capacity-aware FIFO；实现后端只读等待策略诊断与前端 settings 摘要展示，不改变 SSE / trace / export shape。已通过 task_queue_fairness targeted 2/2、settings targeted 191/191、完整 backend slice 1729/1729、frontend model settings utils 7/7、frontend node tests 73/73、frontend lint 与 settings.py py_compile。
todos:
  - id: docs-slimming
    status: completed
