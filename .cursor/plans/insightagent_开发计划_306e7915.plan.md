...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1847/1847)
...
latest_validation_note: registry-governance 进度约 83%；本轮补 task list 跨 item provider source 共享 alias 红测，并让 task list route 先扫描 items governance 再统一编号，避免同一任务列表响应里同 base 脱敏名跨 item 编号不一致。targeted new 1/1、get_tasks 47/47、response_summary 90/90、provider_source 129/129、backend full slice 1847/1847 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1847/1847`。
