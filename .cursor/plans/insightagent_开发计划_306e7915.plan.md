...
current_focus:
  ...
  - 当前主线：rag-product-experience 已启动，进度约 52%；已完成知识库治理表版本明细展开、source/document 文档组摘要、文档组删除治理、Runtime Debug RAG query 查询级召回摘要、命中来源摘要与召回质量标签，RAG status/list、ingest/query、SSE、trace、export 外部契约保持稳定。
constraints:
...
validation_baseline:
  ...
  frontend_rag_main_path_chromium: e2e/workbench-main-path.spec.ts:311 (1/1, RAG query insight summary + recall source attribution + recall quality label + distance explanation)
...
latest_validation_note: rag-product-experience 进度约 52%；本轮新增 Runtime Debug RAG query 查询级召回摘要，继续消费现有 RAG query hits metadata/distance，不改变 RAG status/list、ingest/query、SSE、trace、export 外部契约。先补 runtime-debug-modal-utils 红测并转绿；backend full slice 1950/1950、frontend node 116/116、frontend lint/build、targeted TS、RAG main path Chromium 1/1、git diff --check、备份计划 diff 检查与 8000/3001 端口清理检查通过。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: rag-product-experience
    status: in_progress
    content: 当前主线，进度约 52%；已完成知识库治理表版本明细展开、source/document 文档组摘要、文档组删除治理、Runtime Debug RAG query 查询级召回摘要、命中来源摘要与召回质量标签，按 source/document/version 稳定排序并展示版本数、文档数、chunk 总数、source、document_id、document_version 与 content_hash 摘要；新增文档组删除 route 与 rag_document_delete 审计事件；RAG query 结果基于 hits metadata/distance 显示 best distance、top source、source/document 覆盖数、source/document/version/hash/chunk 命中来源与强/中/弱质量标签，targeted Chromium 已覆盖真实 RAG ingest/query 与文档组删除闭环。
...
- 当前进入 `rag-product-experience` 主线，已完成知识库版本明细展开、source/document 文档组摘要、文档组删除治理、Runtime Debug RAG query 查询级召回摘要、命中来源摘要与召回质量标签，后续继续提升检索解释与召回质量呈现。
...
- Frontend RAG main path Chromium：`e2e/workbench-main-path.spec.ts:311`，`1/1` 通过，覆盖真实 RAG ingest/query 后的查询级召回摘要、命中来源摘要、召回质量标签与 distance 解释。
...
- 当前主线：`rag-product-experience`，进度约 52%，已完成知识库版本明细展开、source/document 文档组摘要、文档组删除治理、RAG query 查询级召回摘要、命中来源摘要与召回质量标签；下一步继续围绕检索解释与召回质量呈现补红测。
