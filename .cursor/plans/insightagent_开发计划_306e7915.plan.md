...
current_focus:
  - 当前主线：registry-governance，进度约 57%；已收口 diagnostics summary 脱敏后去重、provider source values 安全化、settings/validate provider source options 与唯一脱敏 alias round-trip、task list/usage dashboard route/service filter alias round-trip、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1817/1817)
...
latest_validation_note: registry-governance 进度约 57%；本轮补 list_tasks/count_tasks 与 usage dashboard service 直接调用对唯一脱敏 provider source alias filter 的 round-trip 红测，再把 provider source filter alias 解析下沉到持久层 filter normalizer 前。新 targeted 4/4、governance filter 11/11、backend full slice 1817/1817 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1817/1817`。
...
- `registry-governance`：进度约 `57%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options 与唯一脱敏 alias round-trip、task list/usage dashboard route/service filter alias round-trip、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，不改变 settings/preflight/trace/export/audit/SSE 可见字段 shape。

## Pre-flight Cleanup

- 文档瘦身：四份活跃文档只保留当前状态、当前验证基线、下一主线、稳定契约与少量高信号能力摘要。
- 测试拆分：`backend/scripts/test_tool_runtime_slice.py` 已缩为兼容入口，测试主体拆到 `backend/scripts/tool_runtime_slice/` mixin 包并保持原入口命令不变；本轮继续使用 `task_routes_usage_governance.py` 与 `task_usage_dashboard.py` 承接 task list/count 与 usage dashboard service provider source alias filter round-trip 测试，文件规模仍可控。
