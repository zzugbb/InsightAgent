...
latest_validation_note: concurrency-fairness-policy 本轮增量：先补 backend queue e2e helper 红测，要求 settings diagnostics 的 has_waiting_tasks/saturated/pressure_state 必填；随后收紧 assert_safe_queue_settings_diagnostics()，把 status 字段从兼容式“存在才校验”改为 required 且必须与 waiting_count/available_slots 派生状态一致，并补齐 scope 专项 fixtures，保持外部 SSE / trace / export shape 不变。已通过新增 targeted test、backend queue e2e helper slice 24/24、settings slice 201/201、完整 backend slice 1758/1758、前端 type contract、完整 frontend node tests 76/76、frontend lint、backend queue e2e phase fresh；本轮启动的 8011 服务已 Ctrl+C 正常退出，8011 端口残留检查无输出。
todos:
  - id: docs-slimming
    status: completed
