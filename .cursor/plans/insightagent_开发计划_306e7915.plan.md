...
current_focus:
  ...
  - pre-flight cleanup 已完成文档瘦身与 test_tool_runtime_slice 主题拆分；tool_runtime.py planner facade 第一刀已完成，继续开发时保持原测试入口命令不变
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1708/1708)
...
latest_validation_note: tool_runtime.py planner facade 第一刀完成后，planning / tool_plan / facade targeted tests 与完整 backend slice 1708/1708 均通过；本轮尚未重跑 full-stack e2e / Chromium e2e。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: tool-runtime-facade-split
    status: in_progress
    content: app/services/tool_runtime.py planner facade 第一刀已完成，planner/provider planner 实现抽到 app/services/tool_runtime_planning.py；后续继续候选抽 HTTP JSON、registry、result/display、execution loop。
...
- 下一核心主线是 `queue-and-concurrency-lite`。进入主线前的文档瘦身与测试文件拆分已完成；`backend/scripts/test_tool_runtime_slice.py` 保留为兼容入口，`tool_runtime.py` 已开始 facade 拆分并抽出 planner 模块。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1708/1708`；本轮 targeted：`planning`、`tool_plan`、`facade`。
...
- `tool_runtime.py` 拆分放在测试拆分之后分阶段推进，优先抽旁路模块并保留现有 import facade；planner 第一刀已抽到 `backend/app/services/tool_runtime_planning.py`，后续再抽 HTTP JSON、registry、result/display、execution loop。
