...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1844/1844)
...
latest_validation_note: registry-governance 进度约 80%；本轮补 audit detail provider source 单值/列表共享 alias 红测，并让 audit detail sanitizer 先递归收集 provider source 字段再统一编号，避免审计详情里同 base 脱敏名跨字段编号不一致。targeted new 1/1、sanitize_audit_event_detail 3/3、provider_source 126/126、backend full slice 1844/1844 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1844/1844`。
