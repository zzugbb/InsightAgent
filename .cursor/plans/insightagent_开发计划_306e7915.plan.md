...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1838/1838)
...
latest_validation_note: registry-governance 进度约 74%；本轮补 preflight result/summary/service execution provider source alias 对齐红测，并让 preflight result 序列化复用 runtime artifacts 的 provider source alias 上下文，避免顶层、summary、service execution 与 runtime artifacts 编号不一致。targeted new 1/1、provider_source 120/120、backend full slice 1838/1838 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1838/1838`。
