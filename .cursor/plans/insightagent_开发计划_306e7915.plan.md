...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1980/1980)
  ...
  backend_provider_tool_esearchresult_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k esearchresult_count_and_idlist (1/1)
  backend_provider_tool_result_list_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k result_list_hit_count (1/1)
  backend_provider_tool_search_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k provider_search (15/15)
  backend_provider_tool_custom_search_total_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k custom_search_total (1/1)
  backend_provider_tool_serper_organic_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k serper_organic (1/1)
  backend_http_json_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k http_json (531/531)
  ...
  backend_provider_tool_anthropic_tool_use_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k anthropic_tool_use_with_text_content (1/1)
  backend_provider_tool_gemini_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k gemini (2/2)
  backend_provider_tool_camel_case_tool_name_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k camel_case_tool_name (1/1)
  ...
  backend_provider_tool_calls_mapping_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_calls_mapping_container (1/1)
  backend_provider_tool_invocations_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_invocations_container (1/1)
  backend_provider_tool_input_alias_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_input_alias (1/1)
  backend_tool_plan_provider_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_plan_provider (54/54)
  backend_task_stream_reconnect_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k failed_task_error_event_hint (1/1)
  ...
  frontend_remote_error_observability_chromium: e2e/workbench-remote-errors.spec.ts:479 (1/1) and targeted e2e/workbench-edge-cases.spec.ts:1008 + e2e/workbench-remote-errors.spec.ts:479 + e2e/workbench-remote-errors.spec.ts:713 (3/3, failed reconnect error code recovery + Task Center audit failure hint replay + failure source diagnostic groups + diagnostic source chip local drilldown + Failure URL preset replay + readable failure text + needs attention / failed status filters + Audit Logs server keyword request + task detail audit failure hint recovery + failure trace shortcut)
  ...
  frontend_chromium_e2e: full Chromium passed, 52 passed / 1 skipped against real backend/frontend services, including remote error reconnect and knowledge governance version details
...
latest_validation_note: provider-tool-expansion 当前进度约 96%；本轮完成 provider planner 对 tool_input/toolInput 参数别名的解析，并修复 failed terminal reconnect stream 从 error_event failure_hint 复原 remote_provider_network_error；验证包括 tool_input_alias 1/1、failed_task_error_event_hint 1/1、provider_search 15/15、tool_plan_provider 54/54、backend full slice 1980/1980、backend main e2e、frontend node 121/121、frontend lint/build、frontend full Chromium 52 passed / 1 skipped、py_compile、git diff --check 与备份计划 diff 检查通过；外部 SSE、trace、export、display shape 保持稳定。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: provider-tool-expansion
    status: in_progress
    content: 当前进度约 96%；已完成分页型 provider search 显式总量归一化、GraphQL connection 服务端总量归一化、Meilisearch/Algolia 服务端总量别名归一化、Brave web.results 嵌套搜索结果归一化、Bing webPages.totalEstimatedMatches + value[] 服务端总量归一化、SearXNG/元搜索风格 number_of_results + results[] 总量归一化、Crossref/学术检索风格 message.total-results + items[] 总量与命中归一化、PubMed/NCBI ESearch 风格 esearchresult.count + idlist[] 总量与命中归一化、Europe PMC 风格 hitCount + resultList.result[] 服务端总量与当前页命中归一化、Google Custom Search 风格 queries.request[].totalResults + items[] 服务端总量与当前页命中归一化、Serper/Google Search 风格 searchInformation.totalResults + organic[] 服务端总量与当前页命中归一化、引用型 answer-search citations / search_results 命中列表归一化、搜索 provider 千分位总量字符串归一化、显式 result_fields bracket quoted 特殊字段键解析，以及 Gemini/Vertex functionCall、Bedrock/Claude Converse toolUse、Anthropic Messages 混合 text + tool_use 内容块、顶层 message.tool_calls / delta.tool_calls wrapper、tool_calls/toolCalls 映射容器、AI SDK toolInvocations、camelCase toolCalls 容器与 toolCall/toolName/functionName 工具名别名解析，并兼容 input/tool_input/toolInput/arguments JSON 字符串参数和整体序列化 JSON 字符串的 tool_calls/toolCalls 容器；本轮还补齐 failed terminal reconnect stream 错误码复原；下一步继续按小红测补真实 provider/tool 协议输出差异。
...
- 当前主线为 `provider-tool-expansion`，进度约 96%；后续候选为 `ci-release-engineering`。
...
- planner / provider planner：支持 real/extra tools、动态 registry/source 候选、Chat Completions / Responses 风格工具调用输出、Gemini/Vertex functionCall、Bedrock/Claude Converse toolUse、Anthropic Messages mixed text/tool_use content、顶层 message/delta wrapper、tool_calls/toolCalls 映射容器、camelCase toolCalls/toolInvocations/toolName/functionName、typed SDK-style payload 与 usage alias。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1980/1980`。
...
- Backend provider-tool ESearch result slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k esearchresult_count_and_idlist`，当前 `1/1`。
- Backend provider-tool resultList hit count slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k result_list_hit_count`，当前 `1/1`。
- Backend provider-tool search slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k provider_search`，当前 `15/15`。
- Backend provider-tool Custom Search total slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k custom_search_total`，当前 `1/1`。
- Backend provider-tool Serper organic slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k serper_organic`，当前 `1/1`。
- Backend HTTP JSON slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k http_json`，当前 `531/531`。
...
- Backend provider-tool Anthropic toolUse slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k anthropic_tool_use_with_text_content`，当前 `1/1`。
- Backend provider-tool Gemini slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k gemini`，当前 `2/2`。
- Backend provider-tool camelCase tool name slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k camel_case_tool_name`，当前 `1/1`。
...
- Backend provider-tool calls mapping slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_calls_mapping_container`，当前 `1/1`。
- Backend provider-tool invocations slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_invocations_container`，当前 `1/1`。
- Backend tool plan provider slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k tool_plan_provider`，当前 `54/54`。
...
- Frontend Chromium e2e：最终本地 backend/frontend 服务下 full 基线 `52 passed / 1 skipped`；覆盖 remote error reconnect、知识库版本明细展开，低并发 queued 专项在 full 阶段按预期 skip，已由 frontend queue phase 单独覆盖。
...
- `production-reliability-hardening`、`observability-experience` 与 `rag-product-experience` 已 100% 封板；当前 `provider-tool-expansion` 进度约 96%，按先红测、再实现、再 targeted/full slice 的方式推进。
