...
current_focus:
  ...
  - 本轮新增终态任务打开详情流保护：任务打开时若已 completed/failed/cancelled/timed_out，直接走 reconnect 输出终态 SSE，不再返回 409。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1936/1936)
  backend_production_reliability_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability (32/32)
...
latest_validation_note: production-reliability-hardening 进度约 92%；本轮新增终态任务打开详情流走 reconnect；production_reliability 32/32、queue 66/66、task 361/361、settings 216/216、backend full slice 1936/1936、相关 py_compile 通过；本轮未重跑 frontend/e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: production-reliability-hardening
    status: in_progress
    content: 进度约 92%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、running owner guarded start、duplicate active acquire 防双执行、未持有 slot 不释放 active、complete_task owner guarded terminal write、stale heartbeat 接管开关、active stream race 防双执行、terminal start/wait/cancel/complete race 防误复活/防覆盖、provider failure / timeout / tool terminal return lost-race 失败自愈，以及任务详情 reconnect SSE 的终态结束和终态打开。下一步优先围绕异常退出验证、e2e 稳定性与封板验证补红测。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1936/1936`。
- Backend production reliability slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability`，当前 `32/32`。
...
- 当前主线为 `production-reliability-hardening`，进度约 `92%`；后续继续以先红测、再实现、再 targeted/full slice 的方式推进。
