...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1820/1820)
...
latest_validation_note: registry-governance 进度约 60%；本轮补 task response/export 与 usage dashboard response summary helper 直接输出 provider source 脱敏红测，再把归一化后脱敏下沉到共享 summary helper。provider source response targeted 14/14、response summary 89/89、backend full slice 1820/1820 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1820/1820`。
...
- `registry-governance`：进度约 `60%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options 与唯一脱敏 alias round-trip、task list/usage dashboard route/service filter alias round-trip、response summary helper provider source 脱敏、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，不改变 settings/preflight/trace/export/audit/SSE 可见字段 shape。

## Pre-flight Cleanup

- 文档瘦身：四份活跃文档只保留当前状态、当前验证基线、下一主线、稳定契约与少量高信号能力摘要。
- 测试拆分：`backend/scripts/test_tool_runtime_slice.py` 已缩为兼容入口，测试主体拆到 `backend/scripts/tool_runtime_slice/` mixin 包并保持原入口命令不变；本轮继续使用小主题文件 `response_provider_source_artifacts.py` 承接 response summary provider source 脱敏测试，文件规模仍可控。
