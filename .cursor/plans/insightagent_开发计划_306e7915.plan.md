...
current_focus:
  ...
  - 本轮新增 active slot ownership 收口：重复 active acquire 不授予第二执行权，未持有 slot 的执行流退出只清理 waiting、不释放 active slot。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1934/1934)
  backend_production_reliability_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability (30/30)
  backend_queue_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue (66/66)
  backend_task_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task (360/360)
...
latest_validation_note: production-reliability-hardening 进度约 84%；本轮新增 active slot ownership 收口；production_reliability 30/30、queue 66/66、task 360/360、settings 216/216、backend full slice 1934/1934、相关 py_compile 通过；本轮未重跑 frontend/e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: production-reliability-hardening
    status: in_progress
    content: 进度约 84%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、running owner guarded start、duplicate active acquire 防双执行、未持有 slot 不释放 active、stale heartbeat 接管开关、active stream race 防双执行、terminal start/wait/cancel/complete race 防误复活/防覆盖、provider failure / timeout / tool terminal return lost-race 失败自愈，以及任务详情 reconnect SSE 的 cancelled/timed_out 终态结束。下一步优先围绕异常退出后的剩余持久化边界、剩余重连边界、e2e 稳定性与封板验证补红测。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1934/1934`。
- Backend production reliability slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability`，当前 `30/30`。
- Backend queue slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue`，当前 `66/66`。
- Backend task slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task`，当前 `360/360`。
...
- 当前主线为 `production-reliability-hardening`，进度约 `84%`；后续继续以先红测、再实现、再 targeted/full slice 的方式推进。
