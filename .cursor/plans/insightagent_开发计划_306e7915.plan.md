...
current_focus:
  - 当前主线：registry-governance，进度约 23%；已收口 diagnostics summary 脱敏后去重、provider source values 安全化、runtime/preflight/action 回灌 totals 派生/安全规整与 preflight 模型输出层脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1795/1795)
...
latest_validation_note: registry-governance 进度约 23%；本轮继续使用 registry_provider_source_artifacts 主题测试，先补 diagnostics summary、summary model to_dict 与 diagnostics runtime artifacts 的 provider source values 脱敏红测，再统一 summary entries source-name 安全化。targeted diagnostics summary、settings slice、registry slice 与 backend full slice 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 23%；已完成 diagnostics_summary 脱敏后安全值稳定去重、provider source values 安全化、runtime/preflight/action 回灌 totals 派生/安全规整与 preflight 模型输出层脱敏，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1795/1795`。
...
- `registry-governance`：进度约 `23%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、runtime/preflight/action 回灌 totals 派生/安全规整与 preflight 模型输出层脱敏，不改变 settings/preflight/trace/export 可见字段 shape。

## Pre-flight Cleanup

- 文档瘦身：四份活跃文档只保留当前状态、当前验证基线、下一主线、稳定契约与少量高信号能力摘要。
- 测试拆分：`backend/scripts/test_tool_runtime_slice.py` 已缩为兼容入口，测试主体拆到 `backend/scripts/tool_runtime_slice/` mixin 包并保持原入口命令不变；本轮新增 `registry_provider_source_artifacts.py` 承接 provider source artifact 输出层测试，避免继续加大既有 runtime model 文件。
