...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1835/1835)
...
latest_validation_note: registry-governance 进度约 71%；本轮补 diagnostics summary `registry_sources` values 冲突 alias 红测，并把 alias map 限定在 provider source values，普通诊断文本仍保持脱敏后去重。colliding 11/11、alias targeted 19/19、diagnostics summary 12/12、runtime artifacts 28/28、registry 533/533、provider source artifact 2/2、backend full slice 1835/1835 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1835/1835`。
