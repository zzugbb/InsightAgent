...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1855/1855)
...
latest_validation_note: registry-governance 进度约 90%；本轮补 SSE error payload message/detail provider source 碰撞 alias 红测，并让 sse_error_payload 先从 message/detail 文本收集 raw source 后共用 alias map 替换，避免 error payload 中同 base source 丢失 #1/#2 编号。targeted new 1/1、sse 17/17、sse_error 3/3、provider_source 137/137、error_payload 10/10、backend full slice 1855/1855 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1855/1855`。
