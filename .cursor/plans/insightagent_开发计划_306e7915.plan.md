...
current_focus:
  ...
  status: 约 60%
  latest_change: 鉴权依赖将 token parser 异常统一低敏化为 401 invalid token，避免回显内部配置或解析细节
file_size_baseline:
...
validation_baseline:
  ...
  backend: full slice 2013/2013；module boundary 4/4；security 13/13；current_user_hides 2/2；cors 2/2；default_secret 2/2；security_refresh 2/2；auth 1/1；production_operations 12/12；production_operations_health 10/10；production_reliability 39/39；reconnect 9/9
...
- `security-hardening` 已进入，当前约 60%；后端已补全局安全响应头，收紧 access/refresh token 输入边界，阻断生产默认 JWT secret/wildcard CORS，并将认证 token 解析错误低敏化为稳定 401；业务 payload 与 SSE/trace/export 契约不变。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `2013/2013`。
- Backend targeted：`security 13/13`、`current_user_hides 2/2`、`cors 2/2`、`default_secret 2/2`、`security_refresh 2/2`、`auth 1/1`、`production_operations 12/12`、`production_operations_health 10/10`、`production_reliability 39/39`、`reconnect 9/9`、registry/http_json/provider/runtime/trace/export/usage 通过。
...
- 当前状态：`security-hardening` 已进入，当前约 60%；后端全局安全响应头、token 输入边界收紧、生产默认 JWT secret/wildcard CORS 硬阻断与认证错误详情低敏化已按先红测再实现完成，后续继续从鉴权会话、限流与依赖安全审计中补红测。
