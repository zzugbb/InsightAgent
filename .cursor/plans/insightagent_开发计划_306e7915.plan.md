...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1842/1842)
...
latest_validation_note: registry-governance 进度约 78%；本轮补 diagnostics/audit 子结构专属 provider source alias 红测，并让 runtime artifact alias map 从 diagnostics_runtime 与 direct audit_event 递归收集 provider source 字段，避免子结构里同 base 脱敏名局部碰撞。targeted new 1/1、provider_source 124/124、backend full slice 1842/1842 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1842/1842`。
