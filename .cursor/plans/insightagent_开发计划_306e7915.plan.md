...
overview: next-major-upgrade-readiness 当前 80%；React Compiler refs 规则已全局启用并清零 Workbench 命中，Next 16.3.5 / React 19.2.8 双构建与真实 e2e 保持稳定，真实预检剩 2 个兼容性动作且无 blocker。
current_focus:
  mainline: next-major-upgrade-readiness
  status: 80%
  latest_change: 红测要求 react-hooks/refs 全局启用并保持剩余 Compiler lint action；Workbench 的 resize 宽度、发送中 prompt 与最新会话 ref 已迁移，完整 Chromium e2e 通过
file_size_baseline:
...
stable_contracts:
  ...
  - Next 16.3.5 与 eslint-config-next 精确对齐，React / React DOM 固定 19.2.8；ESLint 9.39.5 在 eslint-plugin-react 支持 ESLint 10 前保持兼容锁定，react-hooks/refs 已全局启用，原生 flat config 仅临时关闭 react-hooks/set-state-in-effect
  - data/insightagent.plan.back.md 是只读备份计划，永远不修改
validation_baseline:
  release_gate: bash scripts/ci_run_release_gate.sh --phase all --summary-file /tmp/release-gate-next-major-readiness-slice4-final-summary.md --json-summary-file /tmp/release-gate-next-major-readiness-slice4-final-summary.json passed，覆盖 backend/frontend/tooling/hygiene 全量；Next 16 默认 Turbopack build 与显式 webpack fallback 均在门禁内，JSON summary 复核为 result=PASS、decision_summary.release_decision=approve、operator_summary.status=ready，10 个步骤全过、0 失败
  ...
  frontend: Next 16 readiness contract 8/8，真实预检 ready_with_actions、0 blocker、2 actions；runtime dependency contract 8/8；release gate 内置 node tests 166/166；react-hooks/refs 全局启用，ESLint 为 9.39.5；npm audit 基线 0，lint 0 error / 2 warning，默认 Turbopack build 与显式 webpack fallback passed
  e2e: 本切片已在真实 backend/frontend 服务上重跑 Chromium full e2e，59 passed / 1 skipped
...
next_candidate_mainlines:
  - next-major-upgrade-readiness：Next 16 / React 19.2、原生 flat config、refs 规则、双构建与 PostCSS 审计修复已完成；剩余 set-state-in-effect 迁移和 ESLint 10 插件兼容性验证
next_steps:
  - 按组件主题迁移 react-hooks/set-state-in-effect 命中并移除最后一条 Compiler lint override；持续验证 eslint-plugin-react 对 ESLint 10 的支持后再做受控升级
...
- 当前主线：`next-major-upgrade-readiness` 80%；Next `16.3.5` / React `19.2.8` 保持稳定，`react-hooks/refs` 已全局启用并清零 Workbench 命中，真实仓库为 `ready_with_actions`、0 blocker、2 actions，未修改外部运行时契约。
...
- Frontend：Next 16 readiness contract `8/8`，真实预检 `ready_with_actions`、0 blocker、2 actions；runtime dependency contract `8/8`；release gate 内置 node tests `166/166`；`react-hooks/refs` 全局启用，ESLint 为 `9.39.5`；`npm audit` 基线为 0，lint 0 error / 2 warning，默认 Turbopack build 与显式 webpack fallback 通过。
- E2E/CI：本切片已在真实 backend/frontend 服务上重跑 Chromium full e2e，`59 passed / 1 skipped`。
- Hygiene：`py_compile`、`git diff --check`、`git diff --cached --check` 与备份计划 diff 检查通过；`data/insightagent.plan.back.md` 无修改。

## 当前主线

- 当前主线：`next-major-upgrade-readiness` 80%；`react-hooks/refs` 已全局启用，Next 16 / React 19.2、双构建、零漏洞审计与真实 e2e 保持稳定，项目级 blocker 为 0。
- 下一切片：按组件主题迁移 `react-hooks/set-state-in-effect`，并在 eslint-plugin-react 明确支持 ESLint 10 后再以红测驱动升级。
