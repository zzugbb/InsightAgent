...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1825/1825)
...
latest_validation_note: registry-governance 进度约 63%；本轮补 settings provider source 脱敏 alias 冲突红测，并生成稳定可回写 `#n` 后缀，同时让 validate/update、task list/count 与 usage dashboard filter 复用同一 alias map。alias targeted 11/11、filter alias 6/6、settings 213/213、governance filter 11/11、backend full slice 1825/1825 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1825/1825`。
...
- `registry-governance`：进度约 `63%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options 与唯一脱敏 alias round-trip、冲突 alias 可回写后缀、task list/usage dashboard route/service filter alias round-trip、response summary helper provider source 脱敏、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，不改变 settings/preflight/trace/export/audit/SSE 可见字段 shape。
