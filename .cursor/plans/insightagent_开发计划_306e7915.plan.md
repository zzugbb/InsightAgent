...
current_focus:
  ...
  status: 约 60%
  latest_change: release readiness matrix 增加发布摘要、回滚判定、trend summary 与 artifact retention 检查；backend/frontend/release-gate artifacts 显式保留 14 天；release gate 前端 node 清单纳入 task queue diagnostics 类型契约测试；release gate Markdown/JSON summary 与 trend summary 结构化输出并上传 artifact
file_size_baseline:
...
validation_baseline:
  release_gate: bash scripts/ci_run_release_gate.sh --phase all --summary-file /tmp/release-gate-all-summary.md --json-summary-file /tmp/release-gate-all-summary.json passed，覆盖 backend/frontend/tooling/hygiene 全量；summary 包含 summary_kind、summary_schema_version、service_required、step_summary 与 failed_step_labels；release gate trend summary targeted/tooling 校验 passed；无 backend/.venv fixture 下 release readiness / release gate JSON 校验 passed
...
next_candidate_mainlines:
  - release-observability-polish：当前主线，发布/回滚可见性、artifact 保留策略、release summary 结构化输出与门禁趋势摘要
next_steps:
  - 下一片优先从发布/回滚摘要可见性或 GitHub artifact 下载 previous summary 后的真实跨 run 趋势对比中选择可红测证明的小切片
...
- release 工程：静态 release gate、PR auto routing、结构化 Markdown/JSON summary、baseline/delta 友好的 release gate trend summary、release readiness matrix、backend/frontend queue workflow、artifact diagnostics 与 main push strict policy 已落地。

## 当前验证基线

- Release gate all：PASS，覆盖 backend/frontend/tooling/hygiene；JSON summary 已用 `json.tool` 复核，包含 `summary_kind`、`summary_schema_version`、`service_required`、`step_summary` 与 `failed_step_labels`；trend summary targeted/tooling 校验通过。
...
- 当前状态：`release-observability-polish` 已推进到约 60%；发布/回滚可见性、artifact retention、release summary 结构化输出与 release gate trend summary 已完成。
- 后续候选：发布/回滚摘要可见性、GitHub artifact 下载 previous summary 后的真实跨 run 趋势对比。
