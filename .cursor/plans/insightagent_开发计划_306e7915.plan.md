...
current_focus:
  ...
  - pre-flight cleanup 已完成文档瘦身与 test_tool_runtime_slice 主题拆分，二次细分后最大主题模块约 4.7k 行，继续开发时保持原测试入口命令不变
...
latest_validation_note: test_tool_runtime_slice 二次细分后，http_json / registry / export / runtime / task / response_summary targeted tests 与完整 backend slice 1707/1707 均通过；本轮尚未重跑 full-stack e2e / Chromium e2e。
todos:
  - id: docs-slimming
    status: completed
...
- 下一核心主线是 `queue-and-concurrency-lite`。进入主线前的文档瘦身与测试文件拆分已完成；`backend/scripts/test_tool_runtime_slice.py` 保留为兼容入口，二次细分后最大主题模块约 4.7k 行，原入口命令保持不变。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1707/1707`；本轮 targeted：`http_json`、`registry`、`export`、`runtime`、`task`、`response_summary`。
