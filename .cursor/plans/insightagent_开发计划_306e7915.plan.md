...
current_focus:
  ...
  - 外部 SSE / trace / export / e2e shape 最近完整基线保持通过；本轮新增 RAG route identifier 最终规整，不破坏既有 response shape。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1897/1897)
  backend_rag_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k rag (71/71)
...
latest_validation_note: rag-governance-hardening 进度约 76%；本轮补 RAG route 对底层 service 返回的 knowledge_base_id/collection 做最终规整，并拆出 rag_route_governance 主题测试；targeted service_identifiers、post_rag_query、post_rag_clear、rag 71/71、backend full slice 1897/1897、py_compile、git diff --check 均通过；本轮未改前端代码、未重跑本机 e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1897/1897`。
- Backend RAG slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k rag`，当前 `71/71`。
