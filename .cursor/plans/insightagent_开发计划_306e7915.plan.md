...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1737/1737)
...
latest_validation_note: concurrency-fairness-policy 本轮增量：先补 task_queue_service 旧等待项互斥 eligibility 红测，确认同一用户旧等待项在 per-user limit 下会被重复计入保留容量，导致其他可执行用户任务被保守排队；实现 capacity-aware FIFO 对旧等待项做模拟 scope quota 占用，只计入能互相一起晋升的旧等待任务，不改变外部 SSE / trace / export shape。已通过 mutually_eligible 1/1、task_queue 11/11、完整 backend slice 1737/1737、frontend node tests 73/73、frontend lint 与 task_queue_service py_compile。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1737/1737`。
