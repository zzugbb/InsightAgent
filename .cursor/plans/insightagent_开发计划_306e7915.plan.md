...
current_focus:
  - 当前主线：registry-governance，进度约 29%；已收口 diagnostics summary 脱敏后去重、provider source values 安全化、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact 与 task/usage response provider source 脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1799/1799)
...
latest_validation_note: registry-governance 进度约 29%；本轮新增 response_provider_source_artifacts 主题测试，先补 task detail 与 usage dashboard response provider source 脱敏红测，再让 task response/dashboard route governance coerce 对敏感 source 值做惰性脱敏且保留非敏感 service dict 兼容性。新 targeted、旧兼容性测试与 backend full slice 1799/1799 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 29%；已完成 diagnostics_summary 脱敏后安全值稳定去重、provider source values 安全化、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact 与 task/usage response provider source 脱敏，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1799/1799`。
...
- `registry-governance`：进度约 `29%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact 与 task/usage response provider source 脱敏，不改变 settings/preflight/trace/export 可见字段 shape。

## Pre-flight Cleanup

- 文档瘦身：四份活跃文档只保留当前状态、当前验证基线、下一主线、稳定契约与少量高信号能力摘要。
- 测试拆分：`backend/scripts/test_tool_runtime_slice.py` 已缩为兼容入口，测试主体拆到 `backend/scripts/tool_runtime_slice/` mixin 包并保持原入口命令不变；本轮新增 `response_provider_source_artifacts.py` 承接 task/usage response provider source 输出层测试，避免继续加大既有 task/usage 大文件。
