...
current_focus:
  - 当前主线：registry-governance，进度约 44%；已收口 diagnostics summary 脱敏后去重、provider source values 安全化、settings/validate provider source options、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1807/1807)
...
latest_validation_note: registry-governance 进度约 44%；本轮在 settings_registry 主题测试中补 settings summary 与 validate preview 的敏感 provider source option 名称脱敏红测，再在 settings 响应组装边界复用 provider source artifact sanitizer。targeted 2/2 与 backend full slice 1807/1807 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1807/1807`。
...
- `registry-governance`：进度约 `44%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，不改变 settings/preflight/trace/export/audit/SSE 可见字段 shape。

## Pre-flight Cleanup

- 文档瘦身：四份活跃文档只保留当前状态、当前验证基线、下一主线、稳定契约与少量高信号能力摘要。
- 测试拆分：`backend/scripts/test_tool_runtime_slice.py` 已缩为兼容入口，测试主体拆到 `backend/scripts/tool_runtime_slice/` mixin 包并保持原入口命令不变；本轮继续使用 `settings_registry.py` 承接 settings/validate provider source option 输出层测试，暂不新增大文件。
