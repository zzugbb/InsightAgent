...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1851/1851)
...
latest_validation_note: registry-governance 进度约 86%；本轮补 task export trace governance/steps 跨结构 provider source 共享 alias 红测，并让 task export route 先扫描 trace governance 与 steps meta 后统一编号，避免同一导出 artifact 里同 base 脱敏名跨结构不一致。targeted new 1/1、export 176/176、task_trace 53/53、provider_source 133/133、backend full slice 1851/1851 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1851/1851`。
