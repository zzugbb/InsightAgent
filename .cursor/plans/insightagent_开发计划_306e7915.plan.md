...
current_focus:
  - 当前主线：production-reliability-hardening，进度约 36%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、stale heartbeat 接管开关与 active stream race 防双执行。
  ...
  - 本轮新增 active stream race 治理：pending/queued 任务若已在本进程 active，后续 /stream 会走 reconnect，避免同一 task 被双执行。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1920/1920)
  backend_production_reliability_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability (16/16)
  backend_queue_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue (64/64)
  backend_task_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task (354/354)
...
latest_validation_note: production-reliability-hardening 进度约 36%；本轮新增 active stream race 防双执行；production_reliability 16/16、queue 64/64、task 354/354、settings 216/216、backend full slice 1920/1920、相关 py_compile 通过；本轮未重跑 frontend/e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: production-reliability-hardening
    status: in_progress
    content: 进度约 36%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、stale heartbeat 接管开关与 active stream race 防双执行。下一步优先围绕异常退出后的队列清理、队列持久化边界、执行状态自愈与 e2e 稳定性补红测。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1920/1920`。
- Backend production reliability slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability`，当前 `16/16`。
- Backend queue slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue`，当前 `64/64`。
- Backend task slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task`，当前 `354/354`。
...
- 当前主线为 `production-reliability-hardening`，进度约 `36%`；后续继续以先红测、再实现、再 targeted/full slice 的方式推进。
