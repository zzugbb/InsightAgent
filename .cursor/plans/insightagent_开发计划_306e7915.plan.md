...
current_focus:
  - 当前主线：production-reliability-hardening，进度约 30%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理与 stale heartbeat 接管开关。
  ...
  - 本轮新增 stale heartbeat 治理：running 任务按配置刷新 DB heartbeat；启动恢复默认跳过其他实例，显式配置 TASK_EXECUTION_STALE_AFTER_SEC 后才接管 stale running 任务。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1918/1918)
  backend_production_reliability_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability (14/14)
  backend_queue_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue (63/63)
  backend_task_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task (353/353)
...
latest_validation_note: production-reliability-hardening 进度约 30%；本轮新增 DB heartbeat refresh 与 stale heartbeat 接管开关；production_reliability 14/14、queue 63/63、task 353/353、settings 216/216、backend full slice 1918/1918、相关 py_compile 通过；本轮未重跑 frontend/e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: production-reliability-hardening
    status: in_progress
    content: 进度约 30%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理与 stale heartbeat 接管开关。下一步优先围绕异常退出后的队列清理、队列持久化边界、执行槽位/DB 状态一致性与 e2e 稳定性补红测。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1918/1918`。
- Backend production reliability slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability`，当前 `14/14`。
- Backend queue slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue`，当前 `63/63`。
- Backend task slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task`，当前 `353/353`。
...
- 当前主线为 `production-reliability-hardening`，进度约 `30%`；后续继续以先红测、再实现、再 targeted/full slice 的方式推进。
