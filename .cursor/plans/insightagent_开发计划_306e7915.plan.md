...
current_focus:
  - 当前主线：observability-experience 已推进到约 58%；已完成任务失败线索摘要与来源分类，Task Center 可展示/搜索 trace diagnostics failure hint 与来源，并支持按 Needs attention / Failed status / Failure hint / Failure trace 做观测筛选，任务详情页展示同一失败摘要，Usage Dashboard 任务榜会显示失败摘要并可直接打开任务详情回放，Audit Logs 任务列也可进入同一回放页；Trace 语义统计新增 Failure 维度，Inspector 与任务详情页可按失败语义过滤轨迹，外部 SSE / trace / export shape 不变。
...
validation_baseline:
  ...
  frontend_node_tests: cd frontend && node --test --experimental-strip-types app/components/workbench/utils.node.test.ts lib/stores/chat-stream-store-utils.node.test.ts app/components/workbench/model-settings-modal-utils.node.test.ts (84/84)
  ...
  frontend_targeted_ts: targeted tsc for workbench utils, Task Center, Inspector, task detail, Trace types and i18n
  frontend_task_detail_replay_chromium: e2e/usage-dashboard.spec.ts:1299 (3/3, includes Failure semantic stats)
  frontend_remote_error_observability_chromium: e2e/workbench-remote-errors.spec.ts:479 (1/1, Task Center needs attention / failed status filters)
...
latest_validation_note: observability-experience 继续推进；本轮完成 Task Center 观测筛选，支持按 needs attention / failed status / failure hint / failure trace 定位任务，并保持 Task Center/Trace/任务详情共享失败语义；frontend node 84/84、frontend lint、frontend type contract、targeted TS、backend full slice 1941/1941、remote error observability Chromium 1/1、diff hygiene 与备份计划 diff 检查均通过。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: observability-experience
    status: in_progress
    content: 当前主线，进度约 58%；已完成任务失败线索摘要与来源分类，resolveTaskSnapshotSummary 从 trace diagnostics 提取 failureHint/failureSource，Task Center 展示/搜索该线索与来源，并支持 needs attention / failed status / failure hint / failure trace 观测筛选，任务详情页展示同一失败摘要，Usage Dashboard 任务榜显示失败摘要并可直接打开任务详情回放，Audit Logs 任务列也可进入同一回放页；Trace 语义统计新增 Failure 维度，Inspector 与任务详情页可按失败语义过滤轨迹；下一步继续围绕失败诊断聚合、audit 详情可读性、trace 筛选效率与任务回放效率补红测推进。
...
- Frontend node tests：workbench utils / stream store utils / model settings utils，当前 `84/84`。
...
- Frontend remote error observability Chromium：`e2e/workbench-remote-errors.spec.ts:479`，`1/1` 通过，覆盖 Task Center Needs attention / Failed status 观测筛选。
...
- 当前主线：`observability-experience`，进度约 58%，优先提升 Trace、Task Center、失败诊断、任务回放和 usage/audit 关联体验；本轮已补齐 Task Center 观测筛选。
