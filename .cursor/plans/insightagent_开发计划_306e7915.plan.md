...
current_focus:
  - 当前主线：registry-governance，进度约 20%；已收口 diagnostics summary 脱敏后去重、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层与 provider source 名称脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1792/1792)
...
latest_validation_note: registry-governance 进度约 20%；本轮按单文件规模治理新建 registry_provider_source_artifacts 主题测试，先补 preflight summary/result model 直接 to_dict 的 tool_details 与 diagnostics_summary provider_source 脱敏红测，再在模型输出层统一清洗 summary payload。targeted preflight summary/result、settings slice、registry slice 与 backend full slice 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 20%；已完成 diagnostics_summary 脱敏后安全值稳定去重、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层与 provider source artifact 脱敏，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1792/1792`。
...
- `registry-governance`：进度约 `20%`；已完成 diagnostics summary 脱敏后安全值稳定去重、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层与 provider source artifact 脱敏，不改变 settings/preflight/trace/export 可见字段 shape。
