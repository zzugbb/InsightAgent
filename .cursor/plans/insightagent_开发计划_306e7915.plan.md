...
current_focus:
  - 当前主线：registry-governance，进度约 14%；已收口 diagnostics summary 脱敏后去重、runtime/preflight/action 回灌 totals 派生/安全规整与 provider source 名称脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1787/1787)
...
latest_validation_note: registry-governance 进度约 14%；本轮先补 runtime service action 单模型与 action list dict 回灌 provider_source 脱敏红测，再在 tool_runtime_registry.py 的 action 回灌入口统一递归清洗 provider source 字段。targeted action-from-dict/action-list、settings slice、registry slice 与 backend full slice 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 14%；已完成 diagnostics_summary 脱敏后安全值稳定去重、runtime/preflight/action 回灌 totals 派生/安全规整与 provider source artifact 脱敏，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1787/1787`。
...
- `registry-governance`：进度约 `14%`；已完成 diagnostics summary 脱敏后安全值稳定去重、runtime/preflight/action 回灌 totals 派生/安全规整与 provider source artifact 脱敏，不改变 settings/preflight/trace/export 可见字段 shape。
