...
current_focus:
  ...
  - 当前主线：rag-product-experience 已启动，进度约 24%；已完成知识库治理表版本明细展开、source/document 文档组摘要与文档组删除治理，RAG status/list、ingest/query、SSE、trace、export 外部契约保持稳定。
constraints:
...
latest_validation_note: rag-product-experience 进度约 24%；本轮新增 source/document 文档组删除治理与 rag_document_delete 审计，保持 RAG status/list、ingest/query、SSE、trace、export 外部契约稳定。先补 backend/frontend 红测并转绿；backend RAG slice 79/79、backend full slice 1950/1950、frontend node 112/112、frontend lint/build、targeted TS、knowledge governance targeted Chromium 1/1、git diff --check、备份计划 diff 检查与 8000/3001 端口清理检查通过。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: rag-product-experience
    status: in_progress
    content: 当前主线，进度约 24%；已完成知识库治理表版本明细展开、source/document 文档组摘要与文档组删除治理，按 source/document/version 稳定排序并展示版本数、文档数、chunk 总数、source、document_id、document_version 与 content_hash 摘要；新增文档组删除 route 与 rag_document_delete 审计事件，targeted Chromium 已覆盖真实 RAG ingest、展开明细、删除文档组与状态归零。
...
- 当前进入 `rag-product-experience` 主线，已完成知识库版本明细展开、source/document 文档组摘要与文档组删除治理，后续继续提升检索解释与召回质量呈现。
...
- Frontend node tests：knowledge base governance modal utils / workbench utils / audit logs modal utils / task detail utils / stream store utils / model settings utils，当前 `112/112`。
...
- 当前主线：`rag-product-experience`，进度约 24%，已完成知识库版本明细展开、source/document 文档组摘要与文档组删除治理；下一步继续围绕检索解释与召回质量呈现补红测。
