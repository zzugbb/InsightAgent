...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1846/1846)
...
latest_validation_note: registry-governance 进度约 82%；本轮补 usage dashboard service summary 跨区块 provider source 共享 alias 红测，并让 chat_persistence_service 先扫描 by_session 与 top_tasks governance 再统一编号，避免服务层响应摘要里同 base 脱敏名跨区块编号不一致。targeted new 1/1、usage_dashboard 38/38、response_summary 90/90、provider_source 128/128、backend full slice 1846/1846 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1846/1846`。
