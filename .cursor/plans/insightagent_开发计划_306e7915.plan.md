...
current_focus:
  - 当前主线：observability-experience 已推进到约 88%；已完成任务失败线索摘要与来源分类，Task Center 可从任务列表批量回放 task_failed audit failure hint，展示/搜索 trace diagnostics/TaskResponse/audit failure hint 与来源，并把稳定错误码映射成人类可读失败说明；Task Center 支持按 Needs attention / Failed status / Failure hint / Failure trace 做观测筛选，且 registry profile/provider source 筛选已进入本地任务快照过滤；任务详情页展示同一失败摘要，TaskResponse 可携带 failure_hint/failure_source，任务详情可从 task_failed audit event 恢复远端错误 hint 并快捷定位 Failure 轨迹；Trace Failure 语义已覆盖 rate limited、unauthorized、permission denied、connection refused 等稳定失败码文本；Usage Dashboard 任务榜会显示失败摘要并可直接打开任务详情回放，Audit Logs 任务列也可进入同一回放页，且任务失败/超时审计行会把错误码映射成人类可读 Failure hint 并在展开详情保留 source/code/raw message；Audit Logs keyword 已进入服务端过滤，分页、total、导出和 e2e 搜索口径一致；Trace 语义统计新增 Failure 维度，Inspector 与任务详情页可按失败语义过滤轨迹，外部 SSE / trace / export shape 不变。
...
validation_baseline:
  ...
  frontend_targeted_ts: targeted tsc for workbench utils and task detail trace helper
...
latest_validation_note: observability-experience 继续推进；本轮把 rate_limited / unauthorized 等稳定失败码文本纳入 Trace Failure 语义，任务详情 fallback failure 轨迹合成与 Inspector/Task Detail Failure 过滤口径对齐；frontend node 101/101、frontend lint、targeted TS、backend full slice 1948/1948、remote error observability Chromium 1/1 通过。
todos:
  - id: docs-slimming
    status: completed
...
- 当前主线：`observability-experience`，进度约 88%，优先提升 Trace、Task Center、失败诊断、任务回放和 usage/audit 关联体验；本轮已把稳定失败码文本纳入 Trace Failure 语义，继续保持任务快照语义复用。
