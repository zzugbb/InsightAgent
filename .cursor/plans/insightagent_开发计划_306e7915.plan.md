...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1843/1843)
...
latest_validation_note: registry-governance 进度约 79%；本轮补 session export task/session governance provider source alias 红测，并让 session export response summary/route coercion 共享 provider source alias map，避免导出 JSON/Markdown 中同 base 脱敏名跨层编号不一致。targeted new 1/1、session_export_payload 32/32、provider_source 125/125、backend full slice 1843/1843 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1843/1843`。
