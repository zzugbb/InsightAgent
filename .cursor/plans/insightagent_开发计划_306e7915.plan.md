...
current_focus:
  - 当前主线：registry-governance，进度约 17%；已收口 diagnostics summary 脱敏后去重、runtime/preflight/action 回灌 totals 派生/安全规整、模型输出层与 provider source 名称脱敏。
...
latest_validation_note: registry-governance 进度约 17%；本轮先补 diagnostics runtime model、runtime service action model 与 runtime artifacts model 直接 to_dict provider_source 脱敏红测，再把 provider source 名称/source map key/嵌套字段安全化下沉到模型输出层，并收紧 diagnostics value token 清洗。targeted model-to_dict、settings slice、registry slice 与 backend full slice 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1790/1790`。
...
- `registry-governance`：进度约 `17%`；已完成 diagnostics summary 脱敏后安全值稳定去重、runtime/preflight/action 回灌 totals 派生/安全规整、模型输出层与 provider source artifact 脱敏，不改变 settings/preflight/trace/export 可见字段 shape。
