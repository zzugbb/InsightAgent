...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1837/1837)
...
latest_validation_note: registry-governance 进度约 73%；本轮补 selected/source diagnostics 内部 `registry_sources` alias 引用红测，并让 runtime artifacts 序列化共享 provider_sources/source_diagnostics/selected diagnostics 的同一 alias map，避免 diagnostics key 与 value 编号不一致。targeted new 1/1、provider_source 119/119、backend full slice 1837/1837 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1837/1837`。
