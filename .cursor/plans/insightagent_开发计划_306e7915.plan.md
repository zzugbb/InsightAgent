...
current_focus:
  - 当前主线：registry-governance，进度约 5%；已收口 diagnostics summary 脱敏后去重、runtime artifact 回灌 totals 派生与脏 totals 安全规整。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1779/1779)
...
latest_validation_note: registry-governance 进度约 5%；本轮先补 runtime artifact 回灌 summary totals 派生与无效 totals 安全规整红测，再在 tool_runtime_registry.py 统一从 sanitized entries 派生 totals 并安全转换旧 payload 计数。targeted 回灌、settings slice、registry slice 与 backend full slice 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 5%；已完成 diagnostics_summary 脱敏后安全值稳定去重、runtime artifact 回灌 totals 派生与脏 totals 安全规整，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1779/1779`。
...
- `registry-governance`：进度约 `5%`；已完成 diagnostics summary 脱敏后安全值稳定去重、runtime artifact 回灌 totals 派生与脏 totals 安全规整，不改变 settings/preflight/trace/export 可见字段 shape。
