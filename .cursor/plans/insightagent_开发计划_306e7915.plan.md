...
overview: next-major-upgrade-readiness 当前 91%；set-state-in-effect 已从全局关闭收窄为 6 个明确迁移文件，Runtime Debug 会话 Memory 状态已拆入按会话重建的主题组件，真实预检剩 2 个兼容性动作且无 blocker。
current_focus:
  mainline: next-major-upgrade-readiness
  status: 91%
  latest_change: 红测要求 Runtime Debug 退出 set-state-in-effect 例外清单；会话 Memory 调试区拆为按 activeSessionId 重建的主题组件，RAG 草稿和 mutation 状态保持独立，全库扫描确认剩余 6 文件 17 处，目标 e2e 通过
file_size_baseline:
...
stable_contracts:
  ...
  - Next 16.3.5 与 eslint-config-next 精确对齐，React / React DOM 固定 19.2.8；ESLint 9.39.5 在 eslint-plugin-react 支持 ESLint 10 前保持兼容锁定，react-hooks/refs 已全局启用，set-state-in-effect 仅对 6 个存量迁移文件关闭
  - data/insightagent.plan.back.md 是只读备份计划，永远不修改
validation_baseline:
  release_gate: bash scripts/ci_run_release_gate.sh --phase all --summary-file /tmp/release-gate-next-major-readiness-slice7-final-summary.md --json-summary-file /tmp/release-gate-next-major-readiness-slice7-final-summary.json passed，覆盖 backend/frontend/tooling/hygiene 全量；Next 16 默认 Turbopack build 与显式 webpack fallback 均在门禁内，JSON summary 复核为 result=PASS、decision_summary.release_decision=approve、operator_summary.status=ready，10 个步骤全过、0 失败
  ...
  frontend: Next 16 readiness contract 8/8，真实预检 ready_with_actions、0 blocker、2 actions；runtime dependency contract 9/9；release gate 内置 node tests 167/167；set-state-in-effect 例外限制在 6 个存量文件，全库强制扫描剩 17 处；ESLint 9.39.5，lint 0 error / 2 warning，双构建 passed
  e2e: 本切片真实服务目标 e2e 1/1，覆盖 Runtime Debug RAG 输入保留、错误重试与知识库切换清理；最近 full Chromium 基线 59 passed / 1 skipped
...
next_candidate_mainlines:
  - next-major-upgrade-readiness：Next 16 / React 19.2、原生 flat config、refs 规则、set-state-in-effect 精确债务清单、双构建与 PostCSS 审计修复已完成；剩余 6 文件迁移和 ESLint 10 插件兼容性验证
next_steps:
  - 从 6 个显式文件、17 处 react-hooks/set-state-in-effect 命中继续按组件主题迁移；持续验证 eslint-plugin-react 对 ESLint 10 的支持后再做受控升级
...
- 当前主线：`next-major-upgrade-readiness` 91%；`set-state-in-effect` 已从全局关闭收窄为 6 个明确迁移文件，Runtime Debug 会话 Memory 状态已拆入按会话重建的主题组件，真实仓库为 `ready_with_actions`、0 blocker、2 actions，未修改外部运行时契约。
...
- Frontend：Next 16 readiness contract `8/8`，真实预检 `ready_with_actions`、0 blocker、2 actions；runtime dependency contract `9/9`；release gate 内置 node tests `167/167`；`set-state-in-effect` 例外限制在 6 个存量文件，全库强制扫描剩 17 处；ESLint `9.39.5`，lint 0 error / 2 warning，双构建通过。
- E2E/CI：本切片真实服务目标 e2e `1/1`，覆盖 Runtime Debug RAG 输入保留、错误重试与知识库切换清理；最近 full Chromium 基线 `59 passed / 1 skipped`。
- Hygiene：`py_compile`、`git diff --check`、`git diff --cached --check` 与备份计划 diff 检查通过；`data/insightagent.plan.back.md` 无修改。

## 当前主线

- 当前主线：`next-major-upgrade-readiness` 91%；Compiler lint 全局豁免已消失，Runtime Debug 会话 Memory 主题拆分、目标 e2e 与 Next 16 双构建保持稳定，项目级 blocker 为 0。
- 下一切片：从 6 个显式债务文件、17 处 `react-hooks/set-state-in-effect` 命中继续迁移，并在 eslint-plugin-react 支持 ESLint 10 后再受控升级。
