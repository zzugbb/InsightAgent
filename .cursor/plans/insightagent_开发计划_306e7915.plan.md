...
current_focus:
  - 当前主线：production-reliability-hardening，进度约 42%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、stale heartbeat 接管开关、active stream race 防双执行与 terminal start race 防误复活。
  ...
  - 本轮新增 terminal start race 治理：执行启动只允许 queued/pending/running 切 running；若 running start 竞争失败，不进入 provider 执行，也不覆盖 terminal DB 状态。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1922/1922)
  backend_production_reliability_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability (18/18)
  backend_queue_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue (64/64)
  backend_task_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task (355/355)
...
latest_validation_note: production-reliability-hardening 进度约 42%；本轮新增 terminal start race 防误复活；production_reliability 18/18、queue 64/64、task 355/355、settings 216/216、backend full slice 1922/1922、相关 py_compile 通过；本轮未重跑 frontend/e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: production-reliability-hardening
    status: in_progress
    content: 进度约 42%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、stale heartbeat 接管开关、active stream race 防双执行与 terminal start race 防误复活。下一步优先围绕异常退出后的队列清理、队列持久化边界、执行失败自愈与 e2e 稳定性补红测。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1922/1922`。
- Backend production reliability slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability`，当前 `18/18`。
- Backend queue slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k queue`，当前 `64/64`。
- Backend task slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k task`，当前 `355/355`。
...
- 当前主线为 `production-reliability-hardening`，进度约 `42%`；后续继续以先红测、再实现、再 targeted/full slice 的方式推进。
