...
current_focus:
  - 当前主线：production-reliability-hardening，进度约 61%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、stale heartbeat 接管开关、active stream race 防双执行与 terminal start/wait/cancel/complete race 防误复活/防覆盖。
  ...
  - 本轮新增 terminal complete race 治理：任务完成/失败/超时收尾只允许 pending/queued/running 写终态；若 complete mark 竞争失败，流式成功路径回读真实终态，不再发 done 或写 assistant memory。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1928/1928)
  backend_production_reliability_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability (24/24)
...
latest_validation_note: production-reliability-hardening 进度约 61%；本轮新增 terminal complete race 防覆盖；production_reliability 24/24、queue 66/66、task 359/359、settings 216/216、backend full slice 1928/1928、相关 py_compile 通过；本轮未重跑 frontend/e2e；data/insightagent.plan.back.md 无 diff。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: production-reliability-hardening
    status: in_progress
    content: 进度约 61%；已完成 queue scope cleanup、session delete waiting cleanup、startup orphan running cleanup、execution owner/heartbeat 归属治理、stale heartbeat 接管开关、active stream race 防双执行与 terminal start/wait/cancel/complete race 防误复活/防覆盖。下一步优先围绕异常退出后的队列清理、持久化边界、失败自愈与 e2e 稳定性补红测。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1928/1928`。
- Backend production reliability slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py -k production_reliability`，当前 `24/24`。
...
- 当前主线为 `production-reliability-hardening`，进度约 `61%`；后续继续以先红测、再实现、再 targeted/full slice 的方式推进。
