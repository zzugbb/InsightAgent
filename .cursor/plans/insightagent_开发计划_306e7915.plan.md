...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1845/1845)
...
latest_validation_note: registry-governance 进度约 81%；本轮补 usage dashboard 跨区块 provider source 共享 alias 红测，并让 task usage dashboard route 先扫描 by_session 与 top_tasks governance 再统一编号，避免同一响应里同 base 脱敏名跨区块编号不一致。targeted new 1/1、usage_dashboard 37/37、provider_source 127/127、backend full slice 1845/1845 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1845/1845`。
