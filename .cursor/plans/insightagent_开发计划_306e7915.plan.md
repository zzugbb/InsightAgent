...
current_focus:
  ...
  status: 约 20%
  latest_change: 后端收紧 access token header 校验，拒绝签名有效但 alg/typ 不符合 HS256/JWT 的 token
file_size_baseline:
...
validation_baseline:
  ...
  backend: full slice 2005/2005；module boundary 4/4；security 5/5；auth 1/1；production_operations 12/12；production_operations_health 10/10；production_reliability 39/39；reconnect 9/9
...
- `security-hardening` 已进入，当前约 20%；后端已补全局安全响应头，并收紧 access token header 校验，拒绝签名有效但 `alg/typ` 不符合 `HS256/JWT` 的 token；业务 payload 与 SSE/trace/export 契约不变。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `2005/2005`。
- Backend targeted：`security 5/5`、`auth 1/1`、`production_operations 12/12`、`production_operations_health 10/10`、`production_reliability 39/39`、`reconnect 9/9`、registry/http_json/provider/runtime/trace/export/usage 通过。
...
- 当前状态：`security-hardening` 已进入，当前约 20%；后端全局安全响应头与 access token header 校验收紧已按先红测再实现完成，后续继续从鉴权会话、生产密钥校验、限流与依赖安全审计中补红测。
