...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1836/1836)
...
latest_validation_note: registry-governance 进度约 72%；本轮补 settings 构建路径 runtime artifacts model provider source 字段红测，并在模型构建入口同步脱敏 provider_source_name 与 provider_sources 字典 key，避免 preflight/service execution 读取模型属性时带出原始敏感 source。targeted new 1/1、provider_source 119/119、backend full slice 1836/1836 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1836/1836`。
