...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1741/1741)
  ...
  frontend_queue_phase: low-concurrency backend/frontend scripts/ci_run_frontend_e2e.sh --phase queue fresh passed; default full skips this low-concurrency-only test
...
latest_validation_note: concurrency-fairness-policy 本轮增量：先补 backend queue e2e settings diagnostics 两条红测，确认 has_waiting_tasks 与 waiting_count、saturated 与 available_slots 不一致时 helper 会误通过；实现 helper 自动校验二者一致性，增强真实低并发 e2e 对 settings diagnostics 的漏检防护，不改变外部 SSE / trace / export shape。已通过 inconsistent 3/3、backend_queue_e2e helper 11/11、完整 backend slice 1741/1741、frontend node tests 73/73、frontend lint 与 e2e_queue py_compile。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: concurrency-fairness-policy
    status: in_progress
    content: 当前核心主线；已完成可选 TASK_QUEUE_MAX_CONCURRENT_PER_USER / TASK_QUEUE_MAX_CONCURRENT_PER_SESSION 执行槽位上限、capacity-aware oldest eligible FIFO 防插队、duplicate active task 非拥有 slot 释放防护、旧等待项互斥 eligibility 容量估算、旧等待项预占 scope quota 后的当前任务准入判断、settings task_queue_diagnostics 限额/全局与当前用户 active/waiting/available 安全计数/当前用户限额触顶/has_waiting_tasks/saturated 一致性/压力状态/等待策略诊断，当前用户 available 已按全局空槽与 per-user 剩余额度共同收敛；前端运行设置可观测入口、backend queue e2e-like idle/压力诊断断言、安全 queue snapshot helper、当前用户可用槽位一致性覆盖与真实低并发 backend/frontend queue e2e fresh 复验完成，默认关闭并保持 SSE / trace / export 外形稳定；下一步按风险补更细公平策略或完整 Chromium fresh 复验。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1741/1741`。
- Backend e2e main phase：baseline / main / export consistency / cancel-timeout 已通过。
- Backend e2e queue phase：`TASK_QUEUE_MAX_CONCURRENT=1` backend 上 queued cancel / safe wait_position / settings safe global/current-user active/waiting/available counts、has_waiting_tasks/saturated 一致性与 pressure_state / followup completion 本轮 fresh 通过；脚本现额外校验 settings `task_queue_diagnostics`。
...
- Frontend Chromium e2e：真实 backend/frontend 服务下 full 复跑 `50 passed / 1 skipped`；低并发 queued 专项在 full 阶段按预期 skip。
