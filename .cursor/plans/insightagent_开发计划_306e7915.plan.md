...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1766/1766)
  ...
  backend_e2e_queue: TASK_QUEUE_MAX_CONCURRENT=1 backend 上 queue phase latest fresh passed (queued cancel + safe wait_position + settings global/current-user/current-session active/waiting/available counts + followup completion); helper now also checks queued snapshot structure consistency, scope available slots field dependencies, scope active/waiting count bounds, non-negative governance limits, required governance diagnostics fields, and settings pressure_state enum values
...
latest_validation_note: concurrency-fairness-policy 本轮增量：先补 backend queue e2e helper 红测，要求 settings diagnostics 拒绝半截 scope waiting count、scope active/waiting 计数超过全局计数、负数 per-user/per-session 治理限额；随后收紧 assert_safe_queue_settings_diagnostics()，为 current user/session active/waiting scope 计数增加 active-count 依赖、非负校验与全局上界校验，并要求治理限额非负，保持外部 SSE / trace / export shape 不变。已通过新增红测确认、backend queue slice 53/53、settings slice 202/202、完整 backend slice 1766/1766；本轮未启动本机 e2e 服务，backend queue e2e phase / frontend queue phase / full Chromium 沿用最近 fresh 复验结果。
todos:
  - id: docs-slimming
    status: completed
