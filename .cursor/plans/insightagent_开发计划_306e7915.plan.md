...
current_focus:
  - 当前主线：registry-governance，进度约 50%；已收口 diagnostics summary 脱敏后去重、provider source values 安全化、settings/validate provider source options、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏。
...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1811/1811)
...
latest_validation_note: registry-governance 进度约 50%；本轮在 registry_provider_source_artifacts 主题测试中补核心 runtime service action / runtime artifacts model to_dict 的 tool_registry_provider_source 与 plural source 字段脱敏红测，再补齐 tool_runtime artifact sanitizer 字段集合。targeted 2/2、相关旧测 4/4 与 backend full slice 1811/1811 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
  ...
  - id: registry-governance
    status: in_progress
    content: 当前主线，进度约 50%；已完成 diagnostics_summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，继续统一 registry / profile / provider source / selected source / loader_factory / settings/preflight/trace/export/audit/SSE 的治理语义。
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1811/1811`。
...
- `registry-governance`：进度约 `50%`；已完成 diagnostics summary 脱敏后安全值稳定去重、provider source values 安全化、settings/validate provider source options、runtime artifacts/service actions/direct model outputs provider source 字段集合、runtime/preflight/action 回灌 totals 派生/安全规整、preflight 模型输出层、task/session export artifact、task/usage response、audit detail 短/长字段、SSE error 与 trace meta provider source 脱敏，不改变 settings/preflight/trace/export/audit/SSE 可见字段 shape。

## Pre-flight Cleanup

- 文档瘦身：四份活跃文档只保留当前状态、当前验证基线、下一主线、稳定契约与少量高信号能力摘要。
- 测试拆分：`backend/scripts/test_tool_runtime_slice.py` 已缩为兼容入口，测试主体拆到 `backend/scripts/tool_runtime_slice/` mixin 包并保持原入口命令不变；本轮继续使用 `registry_provider_source_artifacts.py` 承接 registry provider source artifact 输出层测试，文件规模仍较小。
