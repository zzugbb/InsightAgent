...
validation_baseline:
  backend_slice: backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py (1854/1854)
...
latest_validation_note: registry-governance 进度约 89%；本轮补 session export response summary task/session governance provider source 共享 alias 红测，并让 response helper 先基于整份 payload summary 建 alias map，再带入 session 与 task governance 规整，避免 task 行过早脱敏后丢失碰撞后缀。targeted new 1/1、session_export 83/83、provider_source 136/136、export 178/178、model_dump 69/69、backend full slice 1854/1854 均通过，完整 Chromium 基线仍沿用 50 passed / 1 skipped。
todos:
  - id: docs-slimming
    status: completed
...
- Backend slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，当前 `1854/1854`。
