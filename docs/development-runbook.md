...
`scripts/ci_release_gate_trend_summary.sh` 只读取当前和可选上一份 release gate JSON summary，不启动服务；输出 baseline/improved/regressed/changed/unchanged、步骤计数 delta、新增/移除失败步骤标签，并透传 release/rollback `decision_summary` 与 `operator_summary`。GitHub release-gate workflow 会生成 previous download 诊断并上传 `release-gate-trend-summary` artifact。
