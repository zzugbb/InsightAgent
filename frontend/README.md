...
- 最近已对齐到代码的高信号能力：
  ...
  - Trace display / markdown meta wrapper 继续对齐后端：半迁移 trace 里的 tool meta、输出投影 key、planner `steps`、`result_summary` 与 `execution_kind=UserString("http_json")` 会在后端转成 JSON-compatible 安全 payload，再进入前端 trace/detail/export；自定义非 Provider/Hosted 名称的 HTTP JSON tool 也不会因 wrapper kind 漏过脱敏或输出投影。
