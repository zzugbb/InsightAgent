...
- **轨迹可视化（W2 起步）**：右侧「轨迹」面板支持 **时间线 / 流程图**（`@xyflow/react`：只读线性图、`smoothstep` 边、**自定义节点**按 `TraceStep.type` 展示徽章与样式；节点内 **折叠「内容」** 展示正文摘要；随主题亮/暗切换）
- **上下文 · Memory**：展示规划说明与 **实时状态**（`GET .../memory/status`）：Chroma **已连接/未连接**、collection **是否存在**、**向量条数**（后端在任务成功结束时会 **best-effort** 写入摘要，条数会随之增加）

## 下一步

与仓库根目录 `README.md` 一致：Inspector 内可再增加 **手动 add/query** 调试区；后端 **TraceStep** 已与 OpenAPI 对齐（见 `backend` README）。