...
- 阶段 5 CI artifact PR 路径感知扩展（2026-05-12）：`frontend-e2e` 新增 `ci_resolve_artifact_stage_path_level.sh`，PR 只有在命中 frontend/backend/compose/workflow 关键路径时才升级 artifact guard；否则保持默认阈值，降低无关 PR 的误伤概率
