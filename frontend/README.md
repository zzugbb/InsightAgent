...
- 当前主线已进入 `rag-product-experience`，进度约 24%：知识库治理表可展开查看版本明细，并按 source/document 汇总文档组摘要，展示每个文档组的版本数与 chunk 总数；文档组支持删除该 source/document 下全部 chunks，删除后刷新治理表与 RAG status；派生逻辑落在独立 utils/test 文件，前端只消费现有 RAG `document_versions` 字段。

## 当前验证基线

- `cd frontend && node --test --experimental-strip-types app/components/workbench/knowledge-base-governance-modal-utils.node.test.ts app/components/workbench/utils.node.test.ts app/components/workbench/audit-logs-modal-utils.node.test.ts app/tasks/task-detail-page-utils.node.test.ts lib/stores/chat-stream-store-utils.node.test.ts app/components/workbench/model-settings-modal-utils.node.test.ts`：`112/112` 通过
...
- targeted TS：本轮涉及的 knowledge base governance modal/utils、i18n、e2e helper 与 usage dashboard e2e 通过 targeted `tsc`
...
2. 当前主线：`rag-product-experience` 已启动，进度约 24%；已完成知识库版本明细展开、source/document 文档组摘要与文档组删除治理，下一步继续围绕检索解释和召回质量呈现补红测。
