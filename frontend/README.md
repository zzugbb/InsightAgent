...
- 当前主线：`release-observability-polish`，进度约 80%；release gate 内置前端 node 清单已纳入 task queue diagnostics 类型契约测试，发布/e2e artifacts 显式保留 14 天，release gate summary、previous download 诊断与 trend summary 已结构化输出并上传 artifact。
