...
- 最近已对齐到代码的高信号能力：
  ...
  - Provider/source/file-backed registry wrapper 继续对齐后端：`tool_registry_*_json` settings wrapper、file manifest wrapper 与 loader/provider/source/factory adapter wrapper 会先归一再进入 tool details、diagnostics、selected source 与 runtime trace；前端设置面板和回放链路仍沿用既有 API 形态。
