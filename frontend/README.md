...
- 任务详情页 `trace_semantic` replay URL 已支持 `planner/retrieval/calculator/failure` 语义预设；详情页语义筛选会同步地址栏并清理旧搜索/kind 干扰，方便刷新/分享保留当前回放焦点；Task Center 在 `failure_trace` 筛选下打开详情会保持 Failure 回放焦点。
...
- Node tests：workbench utils targeted `69/69`、task detail targeted `8/8`；8 个测试文件 `126/126` 通过，包含 frontend source size boundary。
...
1. 当前状态：`product-ux-polish` 约 32%，已补齐任务详情页语义 replay URL preset、语义筛选 URL 同步/清旧筛选与 Task Center failure trace 详情焦点。
...
- 任务详情页支持通过 `trace_semantic` URL 参数直达 planner/retrieval/calculator/failure 语义 Trace，并在语义筛选切换时更新可分享 URL、清理旧搜索/kind 干扰；Task Center failure trace 结果可直达 Failure 回放视图。
