...
1. 当前推进 `product-ux-polish` 下一阶段，RAG 写入/检索失败分类提示、输入保留与可重试动作已落地。
2. 下一批候选为切换知识库时清理旧写入/检索反馈；前端回归门继续以 node/type/lint、低并发 queue phase、targeted Chromium 与 full Chromium 为准。
