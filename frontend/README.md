...
- 最近已对齐到代码的高信号能力：
  ...
  - 本轮后端继续补齐 `http_json` 成功 payload、RAG follow-up chunk、result-summary、HTTP error body preview、runtime error/artifact/observation summary、半迁移 provider/hosted trace display/export、session JSON trace_preview response/route coercion、task JSON `trace.steps[]` / 顶层 `rag_chunks` response/route coercion、display/markdown meta 的 `result_summary`、`tool_registry.entries`、`diagnostics_runtime`、`runtime_artifacts`、`service_execution`、`preflight_result`、`execution_result`、`audit_event` 与嵌套 `meta.rag.chunks`、半迁移 provider/hosted `output_preview`/`output` 门控拓宽，以及 mock final-answer structured/generic fallback 的 raw text 脱敏；前端工作台、trace、JSON/Markdown 导出回放、session messages、audit logs、model settings validate 错误提示与 mock final answer 不会因 generic value/chunk/summary/error/input/content/excerpt/`result_summary` fallback 看到 `query_params.access_token`、`response_path.data.access_token`、`response_path=$.data.access_token`、`response_path=$['data']['access_token']`、`response_path=$.data['access_token']`、完整 URL 与嵌套 URL 参数/路径/fragment 值中的敏感 query / fragment / userinfo / path segment、成功 payload / raw fallback dict key 中的 diagnostic JSONPath/URL、半迁移 execution_summary `url_path` 里的嵌套 URL 与相对 query/fragment、`http_json` registry label/display name 与显式 `display_name` 后续事件里的 diagnostic URL/assignment、session export `trace_preview.title` 里的 `via http_json` diagnostic label、chat persistence task trace preview 与 parsed `trace_json.meta.tool.label` 里的 diagnostic label、`build_tool_step_output`、`build_tool_trace_event`、stream effects、attempt/retry loop 聚合结果及其 success/postprocess `rag_followup.step/trace`、plan item result success bundle trace/rag_followup、runtime artifacts / service action model 边界里的 wrapper `step+trace` 递归清洗、configured provider runtime service action model execution，以及 direct service action executor / service effects execution 里的 raw output/output_preview、display helper、task response service/route parsed `trace_json`、task trace/delta response route `steps[]`、session messages detail route、task/session export payload/response summary service 与 task/session export route 的 `messages[].content`（含 response-ready BaseModel item）、task export route 的 response-ready `TaskExportTrace.rag_chunks[].content`、session export route 的 response-ready `SessionExportTaskSummary.trace_preview[].content_excerpt`、task markdown builder 直写的 `messages[].content` / `trace.rag_chunks[].content`、session markdown builder 直写的 `messages[].content` / generic `trace_preview.content_excerpt`、settings validate remote preflight `error`、SSE error payload `message/detail`、audit event detail 写入与 audit log list 读取侧、HTTP JSON 上游错误 `RateLimit-*` / `Traceparent` / `X-Trace-ID` 安全 header hint，以及 session export response/route 中非 provider/hosted 标题的 `via http_json` trace/preview URL、`client_secret` 类 compound key 或 bare bearer 片段；后端也会在读完 HTTPError 错误响应体后关闭响应 wrapper，避免测试或运行时留下 ResourceWarning。
  - HTTP JSON response adapter body fallback 已对齐后端：runner 仍优先使用 `read()`，但在常见 SDK adapter 只暴露 `.content` / `.body` / `.data` / `.text` 属性或零参方法，且零参 callable 调用形态不匹配时会继续后续 accessor；或 `read()` 返回不可用 body 而这些 accessor 可用时，也能读取成功 JSON 响应体，仅支持 `read(amt)` 的 adapter 也会按 chunk 分块读到 EOF（含 `None` EOF）；只暴露 `json()` parsed body（含 `.json` 属性、typed `model_dump_json()`/`to_json()`/`json()`/`model_dump(mode="json")`/`model_dump()`/`dict()`/`to_dict()` payload 与嵌套 typed payload 递归归一化）的 adapter 也会重新序列化后继续映射；`.body`/`.data` parsed Mapping/Sequence wrapper/object/array 或 typed payload 也会按 JSON body 受控序列化；只暴露 `iter_bytes()`/`iter_content()`/`iter_text()`/`iter_lines()` 的 streaming body adapter（含 `chunk_size=` keyword-only iterator、`None` keep-alive chunk 与 response 本体 iterable）也会拼接 chunk 后继续映射；缺失 reader 仍保持稳定 `transport error` 诊断；请求侧 `json_body` 已支持 root `$payload` 模板与嵌套 typed/wrapper payload 递归归一化；`query_params` 也已支持 root `$params` 模板与 typed list/scalar wrapper 归一化，URL 拼接前仍只允许 scalar 或 scalar list；`headers` 已支持 root `$headers` 模板与 typed scalar header 归一化，header 校验前仍拒绝 object/list header；`method` 与 `timeout_ms` 已支持 root runtime template，response mapping 的 `response_path` / `result_fields` root runtime template、result field path runtime template 与 JSONPath `${...}` interpolation template、string-like path wrapper/`$fields` mapping key-value wrapper、typed/literal execution spec 自身携带的 `UserString` response_path/result_fields key/path wrapper、literal `UserDict`/`UserList` 请求模板和 result_fields mapping wrapper 的模板扫描与 unsupported reserved 变量诊断，以及 literal request mapping field-name `UserString` wrapper 的 header/query/body normalizer、validation 与 execution summary 兼容也会在发起请求前渲染并校验路径；发起请求前仍校验 allowed method、GET + json_body 与 timeout 范围。
  - HTTP JSON response adapter 现在会在 response 同时暴露 `.json()` / `.json` parsed body 与泛型 `__iter__` 时优先使用显式 parsed JSON body；如果 `.json()`、typed payload `.json(...)` dump 方法或首个 `iter_*` streaming 方法只是调用形态不适配，后端会继续尝试本体 iterable、`model_dump/dict/to_dict` 或后续 iterator fallback，前端看到的真实工具输出不会再因为 SDK response 的 metadata/key 迭代、typed dump 方法差异或 iterator 方法签名差异被误报成 `transport error`。
  - HTTP JSON typed parsed body fallback 继续对齐后端：当根层或嵌套 typed payload 的 `model_dump()` 只是 SDK/Pydantic 方法签名不适配时，后端会继续尝试 `dict()` / `to_dict()`，前端仍能看到映射后的真实工具输出；真正不可序列化 body 仍会保留明确诊断。
  - HTTP JSON typed dump-json 错误诊断继续对齐后端：`model_dump_json()` / `to_json()` / typed `.json()` 只有 TypeError 调用形态不适配时才会 fallback；真实 serializer 运行时异常会保留为 `response json body ... failed`，前端不会看到被后续 `to_dict()` 掩盖的假成功。
  - HTTP JSON typed dump-json TypeError 诊断继续对齐后端：后端会先用 callable signature 判断零参 dump 调用是否可用，只有签名不匹配时才 fallback；签名可匹配后 serializer 内部抛出的 TypeError 会作为真实上游/SDK dump 失败进入 trace/export。
  ...
  - HTTP JSON execution kind string wrapper 继续对齐后端：`execution.kind=UserString("http_json")` 会按真实执行器进入 validation、runner dispatch 与 execution_summary；前端 tool details / trace 不会再因 wrapper kind 退回 missing/unsupported execution。
  - HTTP JSON runtime semantic wrapper 继续对齐后端：半迁移 registration 里已有的 `execution_summary` / `execution_diagnostics` 若携带 `UserString` key/value/list item，会先脱敏再进入 tool_start/action meta/trace/export；`kind` / `runtime_semantic_kind` wrapper 也会继续产出稳定的 `semantic_kind` / `semantic_family`，前端筛选、标题与详情不会退化。
  - HTTP JSON result projection wrapper 继续对齐后端：`result_preview_keys` / `result_output_keys` 的 `UserList` / `UserString` wrapper 会继续生成安全 effective keys，敏感字段不会因 wrapper 容器退回默认投影；planner steps、calc expression、retrieval knowledge_base_id/request_id wrapper 也会进入稳定 result summary / observation / trace/export 摘要。
  - HTTP JSON registry spec wrapper 继续对齐后端：direct `extra_tools`、`overrides` 与 provider adapter spec 的 `UserDict/UserList/UserString` 半迁移 payload 会先归一再进入注册和 diagnostics；前端 model settings / provider source tool details 不会再因为 wrapper spec 整个漏掉真实工具。
  - 工具规划 wrapper 继续对齐后端：provider-generated `tools/plan`、单个 plan item、`task_plan` 的 `planned_tool_names/labels/kinds/execution_kinds` planned fields、action step 初始输入与 `normalize_tool_spec` input 如果来自 `UserDict/UserList/UserString`，后端会先归一再写入 planner steps / trace meta；前端工作台、trace 搜索与导出回放仍消费同一套 SSE / trace / export 契约，不需要新增本地分支。
  - Provider/source/file-backed registry wrapper 继续对齐后端：`tool_registry_*_json` settings wrapper、file manifest/plain registry file wrapper 与 loader/provider/source/factory adapter wrapper 会先归一再进入 tool details、diagnostics、selected source 与 runtime trace；provider adapter 通过 `registry_file` 构建真实工具时会使用当前 provider source 的模板上下文，provider/loader factory 与命名 provider/loader 背后的 `registry_file` 也会按 source settings 重建；registry-file backed factory 自身声明的 `profile` / disabled / `extra_tools` / `overrides` 会在 factory 调用时通过 adapter 生效，factory->factory alias 外层声明的 `extra_tools` / `overrides` 也会继续叠加到内层 file-backed factory，真实 HTTP JSON 请求 URL/header/query、response_path/result_fields 与 invalid execution diagnostics 不再因 alias 层丢失；alias 指向 profile factory 且 outer override 重新启用 profile-disabled 工具时，后端也会提前把该 override 的 HTTP JSON 坏配置并入 diagnostics；provider/source 通过 provider_factory、loader_factory 或 provider factory alias 显式把 profile 重置为 `default` 时，后端会返回合成后的 effective registry，不再把内层 `retrieval_only` 等 profile 的 disabled tools 泄入前端 tool details；registry-file backed factory、settings-backed named loader/provider、以及 source adapter 自身的 override invalid HTTP JSON execution diagnostics 都会并入 selected source diagnostics；manifest `registry_sources` 引用 settings-backed source/provider/loader/factory 时，相关 `registry_file` 相对路径会按当前 manifest 所在目录解析；已访问 registry file 会继续防循环，direct source 自引用、命名 provider/loader/factory 背后的同文件引用，以及 `source -> provider -> loader/loader_factory`、`source -> provider_factory`、`source -> source` 这类传递链都会折算为 `skipped_registry_sources` 而不是误报 missing source；如果命名 loader/provider/factory 或 source->source 链路里的 inner source 因缺失 registry file 无法构建，已收集的 missing/skipped/invalid diagnostics 仍会保留到 artifact map 并并回 root source diagnostics；source spec 的 effective profile 会进入真实 HTTP JSON 请求和 preflight summary，包括 named provider、named loader、provider/loader factory 背后的 file-backed 工具，前端设置面板、trace 与导出回放继续沿用既有 API 形态但不会看到串源后的 source/profile 摘要。本轮后端 real search 组合回归还覆盖了 POST headers/query/body 与 plan item preview/output/summary；file-backed real calc 也已覆盖 provider planner、plan item request/summary、trace preview/session export response 回放、trace detail/delta 与 task export JSON 的结构化 result_summary 回填、脱敏 diagnostics 与 settings tool details preflight summary，前端继续消费既有安全摘要字段而不改变外部 API 形态。
  - Provider/source inline shorthand 继续对齐后端 source/profile 语义：source/provider/loader 在根级直接声明 real tool shorthand 且同时声明 `profile` / disabled / overrides 等控制字段时，后端会把根级工具并入对应 `extra_tools`；同名 `overrides` 会继续胜过 inline base tool 的请求和响应映射，前端 settings、trace、export 仍消费既有 selected registry / execution_summary 字段，不需要新增分支。
  - File manifest `registry_sources` 的 source->source forward reference 继续对齐后端：settings JSON 里 outer source 可以先于 inner source 声明，后端会按依赖解析并继续用 child source/profile 渲染真实 HTTP JSON 请求；inner 缺失 registry file 的 diagnostics 也会回到 outer/root manifest，前端 settings、trace、export 仍消费既有字段，不需要新增声明顺序提示或本地排序分支。
  - Settings-backed named provider 的 provider->provider forward reference 继续对齐后端：`tool_registry_providers_json` 里 outer provider 可以先于 inner provider 声明，后端会按依赖构建并保持 provider/source tool details、trace 与导出字段不变，前端不需要新增 JSON 排序限制。
  - Settings-backed named loader 的 loader->loader forward reference 继续对齐后端：`tool_registry_loaders_json` 里 outer loader 可以先于 inner loader 声明，后端会按依赖构建并保持 loader/provider/source tool details、trace 与导出字段不变，前端不需要新增 JSON 排序限制。
  - Settings-backed loader/provider factory 的 factory->factory forward reference 继续对齐后端：`tool_registry_loader_factories_json` / `tool_registry_provider_factories_json` 里 outer factory 可以先于 inner file-backed factory 声明，且 outer factory 自身 profile/disabled/extra_tools/overrides 会继续套用到 inner factory registry；前端 settings、trace、export 仍消费既有字段，不需要新增 factory 排序或 alias 专用分支。
  - Trace display / markdown meta wrapper 继续对齐后端：半迁移 trace 里的 tool meta、输出投影 key、planner `steps`、`result_summary`、`execution_kind=UserString("http_json")`、`meta.rag.chunks` 与 wrapped `knowledge_base_id` 会在后端转成 JSON-compatible 安全 payload，再进入前端 trace/detail/export；自定义非 Provider/Hosted 名称的 HTTP JSON tool 也不会因 wrapper kind 漏过脱敏或输出投影，RAG chunk wrapper 也不会导致 markdown/export 泄漏敏感片段或丢失命中计数。
  ...
  - HTTP JSON body 属性错误诊断继续对齐后端：当显式 `.content` / `.body` / `.data` / `.text` body 属性是坏类型，而泛型 `__iter__` 只是空 metadata iterator 时，前端会看到 body type 诊断，不会被误报成 `empty JSON response`。
  - HTTP JSON parsed body fallback 继续对齐后端：当 `json()` / `.json` parsed body 返回不可序列化对象但 response 本体 iterable 才是真实 body 时，后端会继续读取 iterable；如果 `json()` 自身抛真实运行时异常，前端仍会看到明确的 `response json failed` 诊断。
  - HTTP JSON parsed body 错误诊断继续对齐后端：当显式 `json()` / `.json` parsed body 返回不可序列化对象，而泛型 `__iter__` 只是空 metadata iterator 时，前端会看到 parsed-body JSON serializable 诊断，不会被误报成 `empty JSON response`。
  - HTTP JSON `json()` 调用形态 fallback 继续对齐后端：当 `json()` 只是 SDK 方法签名不适配，但 `.text` 等后续 source 明确为空时，前端仍会看到稳定的 `empty JSON response`，不会被误报成 parsed-body `transport error`；真实 parsed body 类型错误仍会保留。
  - HTTP JSON 空 `read()` fallback 继续对齐后端：当 `read()` 返回空 bytes/text 但 `.content` 或 `json()` 等后续 source 持有真实 body 时，前端仍会看到成功 tool output；如果没有后续 body，原有 `empty JSON response` 错误不变。
  ...
  - HTTP JSON streaming iterator 方法调用失败 fallback 继续对齐后端：当首个 `iter_*` 方法在返回 iterator 前就因为 SDK stream 工厂不可用而抛错时，后端会继续尝试后续 iterator 或 `json()` parsed body；如果没有可用 source，前端仍会看到原 iterator 失败诊断。
  - HTTP JSON response 本体 iterable 初始失败诊断继续对齐后端：当 `iter_*` 已确认为空流，而泛型 `__iter__` 在首个 chunk 前因为 SDK metadata iterator 坏掉而失败时，前端仍会看到稳定的 `empty JSON response`，不会被升级成 `transport error`。
