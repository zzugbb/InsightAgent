...
- 最近已对齐到代码的高信号能力：
  ...
  - 对 URL、header/query value shape、`json_body` 严格 JSON 与 response mapping path，后端也会给出更明确的配置诊断：相对 URL、带 userinfo credentials 的 URL、显式端口非法或含原始空格/控制字符的 URL、静态 settings/source 渲染后才出现的 header CR/LF 或其他控制字符注入、非法 header name、对象型 header/query、运行时渲染成对象的 query 参数、`json_body` 里的 `NaN` / `Infinity` 或不可序列化对象，以及非法 bracket path 不会再被静默字符串化、延后裸抛或部分解析；前端设置面与运行态 trace 看到的是同一类 real-tool execution 配置错误，`execution_summary.url_origin/url_path` 也会安全化，避免半可信 endpoint 或 path 内 token 泄漏。
  - 当 `http_json` 上游返回 HTTP 错误、连接/传输错误、2xx 非 JSON 响应或非法 UTF-8 响应体时，后端现在会带出稳定的 HTTP status、脱敏限长后的 reason、`transport error` 或 `invalid JSON response` 与敏感键/敏感 assignment 脱敏后的有限长度 preview；前端工作台、trace 与导出回放后续消费到的是更可排障的真实上游错误语义，而不是泛化网络/解析/codec 异常文案、HTTP reason 中的敏感原文或 JSON error body 字符串里的 `token=...` 原文。
  ...
  - 对请求侧 diagnostics 中的敏感字段路径，后端现在也会先脱敏再交给前端：`query_params.api_key`、`json_body.access_token` 或嵌套的 `client_secret` 路径会显示成 `[redacted]`，settings 面板、tool_start、trace subtitle/search 与导出回放都不会因为错误定位路径而显示敏感字段名。
  ...
  - 对 `execution_summary.result_field_names` 与显式 `result_preview_keys` / `result_output_keys` 这类结果元数据，后端现在也会过滤或脱敏敏感字段名；如果 provider/source 显式只声明了 `access_token` / `api_key` 一类敏感输出 key，前端设置面、tool_start、trace 与导出回放也不会再因模板默认投影回退而重新显示这些字段名。
