...
- `real-tool-execution`、`queue-and-concurrency-lite`、`concurrency-fairness-policy` 与 `registry-governance` 均已封板；当前主线进入 `rag-governance-hardening`，进度约 `12%`；前端继续保持与后端 SSE / trace / export 契约稳定对齐，不新增独立本地语义分支。
...
- 后端 `rag-governance-hardening` 已完成 RAG source/metadata 入站与 query 出站脱敏，并为 chunk metadata 增加稳定 `document_version` / `content_hash`；前端可见响应 shape 保持增量兼容。
...
2. 当前主线：`rag-governance-hardening`，进度约 `12%`；后端已收口 RAG source/metadata 脱敏与稳定文档版本 metadata，前端保持字段 shape 稳定。
