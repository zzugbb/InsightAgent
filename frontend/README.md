...
- 最近已对齐到代码的高信号能力：
  ...
  - Provider/source/file-backed registry wrapper 继续对齐后端：`tool_registry_*_json` settings wrapper、file manifest/plain registry file wrapper 与 loader/provider/source/factory adapter wrapper 会先归一再进入 tool details、diagnostics、selected source 与 runtime trace；provider adapter 通过 `registry_file` 构建真实工具时会使用当前 provider source 的模板上下文，provider/loader factory 与命名 provider/loader 背后的 `registry_file` 也会按 source settings 重建；前端设置面板、trace 与导出回放继续沿用既有 API 形态但不会看到串源后的 source 摘要。
