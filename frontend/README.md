...
- 最近已对齐到代码的高信号能力：
  ...
  - Settings-backed loader/provider factory 的 factory->factory forward reference 继续对齐后端：`tool_registry_loader_factories_json` / `tool_registry_provider_factories_json` 里 outer factory 可以先于 inner file-backed factory 声明，前端 settings、trace、export 仍消费既有字段，不需要新增 factory 排序限制。
