...
- `security-hardening` 已进入，当前约 0%；前端优先记录后端安全契约变化，必要时再补登录/session/错误提示侧的兼容展示。
...
1. 当前状态：`security-hardening` 已进入，当前约 0%；前端先跟随后端安全契约，不主动改变 SSE、trace、export 或任务列表 API。
