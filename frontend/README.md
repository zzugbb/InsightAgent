...
- 当前主线已进入 `rag-product-experience`，进度约 12%：知识库治理表可展开查看版本明细，并按 source/document 汇总文档组摘要，展示每个文档组的版本数与 chunk 总数；新增派生逻辑落在独立 utils/test 文件，前端只消费现有 RAG `document_versions` 字段，不新增本地后端契约分支。
...
- knowledge governance targeted Chromium：`e2e/usage-dashboard.spec.ts:1543`，`1/1` 通过，覆盖真实 RAG ingest 后展开版本明细与 source/document 文档组摘要
...
- 知识库治理：列表、版本明细展开、文档组摘要、来源采样、shared 权限显隐、清空/删除
- 审计日志：筛选、分页、详情、导出
