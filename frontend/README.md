...
- 最近已对齐到代码的高信号能力：
  ...
  - HTTP JSON response adapter body fallback 已对齐后端：runner 仍优先使用 `read()`，但在常见 SDK adapter 只暴露 `.content` / `.body` / `.data` / `.text`，或 `read()` 返回不可用 body 而这些属性可用时，也能读取成功 JSON 响应体，仅支持 `read(amt)` 的 adapter 也会按 chunk 分块读到 EOF；只暴露 `json()` parsed body 的 adapter 也会重新序列化后继续映射；缺失 reader 仍保持稳定 `transport error` 诊断。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1391/1391`）
