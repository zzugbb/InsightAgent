...
- 最近已对齐到代码的高信号能力：
  ...
  - Provider/source inline shorthand 继续对齐后端 source/profile 语义：source 在根级直接声明 real tool shorthand 且同时声明 `profile` / disabled / overrides 等控制字段时，后端会把根级工具并入 source 级 `extra_tools`；前端 settings、trace、export 仍消费既有 selected registry / execution_summary 字段，不需要新增分支。
