...
- 阶段 5 协同（2026-05-18，composed file-backed manifests）：后端已继续把 runtime seam 往“真实 registry 可接入”方向推进，在 `load_tool_registry_file_payload()` 与 `build_tool_registry_from_file()` 这条链上新增 `registry_files` 组合能力，并支持按父 manifest 目录解析相对路径；命名 loader / provider / source 现在不只支持 pure extra_tools 与单文件 manifest，还支持 composed file-backed source。当前前端可见的 SSE/trace 契约保持不变。focused 回归脚本已扩展到 167 条兼容测试，`bash scripts/test_ci_e2e_tooling.sh common` 已再次通过
