...
- `real-tool-execution`、`queue-and-concurrency-lite`、`concurrency-fairness-policy` 与 `registry-governance` 均已封板；当前主线进入 `rag-governance-hardening`，进度约 `56%`；前端继续保持与后端 SSE / trace / export 契约稳定对齐，不新增独立本地语义分支。
...
2. 当前主线：`rag-governance-hardening`，进度约 `56%`；后端已收口 RAG source/metadata 脱敏、稳定文档版本 metadata、status/list 版本摘要、export 版本锚点、runtime trace 版本 metadata、reserved metadata 防覆盖、出站版本字段过滤、敏感 `knowledge_base_id` 安全化与 shared scope 列表边界，前端已补 trace 类型和搜索兼容。
