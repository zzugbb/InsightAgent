...
- Frontend：Next 16 readiness contract `8/8`，真实预检 `ready_with_actions`、0 blocker、2 actions；runtime dependency contract `9/9`；release gate 内置 node tests `167/167`；`set-state-in-effect` 例外精确限制在 6 个存量文件，全库强制扫描剩 17 处；ESLint `9.39.5`，lint 0 error / 2 warning，双构建通过。
...
- E2E/CI：真实服务目标 e2e `1/1`，覆盖 Runtime Debug RAG 输入保留、错误重试与知识库切换清理；最近 full Chromium 基线仍为 `59 passed / 1 skipped`。
- Hygiene：`git diff --check`、`git diff --cached --check` 与备份计划 diff 检查通过；`data/insightagent.plan.back.md` 无修改。

## 下一步前端计划

1. `next-major-upgrade-readiness` 当前 91%；Runtime Debug 会话 Memory 子组件按 `activeSessionId` 重建，RAG 草稿与检索状态继续独立保留。
2. 下一切片从 6 个文件、17 处命中继续按组件主题迁移；ESLint 10 等待 `eslint-plugin-react` 兼容后再以红测驱动升级。
