...
- 最近已对齐到代码的高信号能力：
  ...
  - 工具规划 wrapper 继续对齐后端：provider-generated `tools/plan`、单个 plan item、`task_plan` 的 planned fields、action step 初始输入与 `normalize_tool_spec` input 如果来自 `UserDict/UserList/UserString`，后端会先归一再写入 planner steps / trace meta；前端工作台、trace 搜索与导出回放仍消费同一套 SSE / trace / export 契约，不需要新增本地分支。
