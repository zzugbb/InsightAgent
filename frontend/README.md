...
1. `queue-and-concurrency-lite`：下一步补多任务并发与跨 session 前端专项 e2e，并择机复跑完整 Chromium。
...
4. e2e：下一步补多任务并发、刷新恢复与 session 切换专项回归。
