...
- 当前主线：`production-runtime-hardening` 后续运维体验，进度约 45%；已补后端 `/health.operations.operator_summary`、release gate `operator_summary` 与 export diagnostics overview `operator_summary`，前端契约保持只读兼容。
...
2. Release gate summary 与 export diagnostics overview 已对齐同一 operator-facing 口径，不改变前端 SSE / trace / export / task list 契约。
