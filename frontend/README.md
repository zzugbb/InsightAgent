...
- backend queue e2e phase：低并发 backend 下 queued cancel / safe wait_position / settings safe global/current-user/current-session active/waiting/available counts / followup completion 最近一次 fresh 通过；脚本 helper 现额外校验 queued snapshot 结构一致性、scope available slots 字段依赖与 governance 必填字段；上一轮尝试启动 8011 低并发 backend 时，提权审批通道连接断开，未能 fresh 重跑真实 queue e2e
- frontend queue phase：低并发 backend/frontend 下 `bash scripts/ci_run_frontend_e2e.sh --phase queue --api-base-url http://127.0.0.1:8011 --frontend-base-url http://127.0.0.1:3001` 最近一次 fresh 通过，`1 passed`；默认 full 环境下该低并发专项显式 skip
