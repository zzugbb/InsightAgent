...
- 当前主线已进入 `rag-product-experience`，进度约 8%：知识库治理表可展开查看版本明细，展示版本数、文档数、chunk 总数、source、document_id、document_version 与 content_hash 摘要；新增派生逻辑落在独立 utils/test 文件，前端只消费现有 RAG `document_versions` 字段，不新增本地后端契约分支。
...
- 知识库治理：列表、版本明细展开、来源采样、shared 权限显隐、清空/删除
- 审计日志：筛选、分页、详情、导出
