...
- 最近已对齐到代码的高信号能力：
  ...
  - Task/session export response summary wrapper 继续对齐后端：response-ready `trace_preview`、`messages`、`trace.steps`、`rag_chunks` 与 `rag_knowledge_base_ids` 中的 `UserList/UserString` 或 `model_dump()` item 会在后端归一并脱敏后再返回，前端导出回放继续消费既有 JSON shape，不需要新增分支。
