...
- 最近已对齐到代码的高信号能力：
  ...
  - HTTP JSON response adapter 现在会在 response 同时暴露 `.json()` / `.json` parsed body 与泛型 `__iter__` 时优先使用显式 parsed JSON body；如果 `.json()` 或 typed payload `.json(...)` dump 方法只是调用形态不适配，后端会继续尝试本体 iterable 或 `model_dump/dict/to_dict` fallback，前端看到的真实工具输出不会再因为 SDK response 的 metadata/key 迭代或 typed dump 方法差异被误报成 `transport error`。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1462/1462`）
