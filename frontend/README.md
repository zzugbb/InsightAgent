...
- 任务详情页 `trace_semantic` replay URL 已支持 `planner/retrieval/calculator/failure` 语义预设，未知值继续回退全量 Trace，便于 Task Center/Audit/外部链接直达对应回放视图。
...
- Node tests：task detail targeted `6/6`；8 个测试文件 `123/123` 通过，包含 frontend source size boundary。
...
1. 当前状态：`product-ux-polish` 约 10%，首个切口补齐任务详情页语义 replay URL preset。
...
- `trace_semantic` URL 参数兼容支持 `planner/retrieval/calculator/failure`；未知值回退 `all`，不影响既有 Failure 回放入口。
...
- 任务详情页支持通过 `trace_semantic` URL 参数直达 planner/retrieval/calculator/failure 语义 Trace。
