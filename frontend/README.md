...
- 当前主线：`product-ux-polish` 下一阶段约 50% 推进中；Knowledge Governance 已补状态级 next-action 与共享范围行内提示，普通成员明确只读并联系管理员，管理员明确变更会影响全部共享成员。
...
1. 当前推进 `product-ux-polish` 下一阶段，Knowledge Governance 已明确共享知识库的成员只读与管理员变更影响范围。
2. 下一批候选为 Knowledge Governance 到 Runtime Debug RAG 写入入口的直接处置衔接；前端回归门继续以 node/type/lint、低并发 queue phase、targeted Chromium 与 full Chromium 为准。
...
- Knowledge Governance operator next-action、共享范围说明与清空/删除禁用只由现有 query 状态、`chroma_reachable`、知识库 ID 和用户角色本地派生，不改变 shared RAG 权限或 API shape。
