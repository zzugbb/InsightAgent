...
- 最近已对齐到代码的高信号能力：
  ...
  - HTTP JSON response adapter 现在会在 response 同时暴露 `.json()` / `.json` parsed body 与泛型 `__iter__` 时优先使用显式 parsed JSON body；前端看到的真实工具输出不会再因为 SDK response 的 metadata/key 迭代被误报成 `transport error`，仅有本体 iterable 的 streaming fallback 仍保持原契约。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1460/1460`）
