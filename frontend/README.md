...
- 当前主线：`product-ux-polish` 下一阶段约 40% 推进中；Task Center、任务详情页、Audit Logs 与 Knowledge Governance 已补 operator next-action 提示；知识库陈旧缓存或 Chroma 不可达时暂停清空/删除，空列表会指向写入入口。
...
1. 当前推进 `product-ux-polish` 下一阶段，已把本地处置提示延伸到 Knowledge Governance 的陈旧、不可达与空列表状态。
2. 下一批候选为共享知识库只读/管理员处置提示；前端回归门继续以 node/type/lint、低并发 queue phase、targeted Chromium 与 full Chromium 为准。
...
- Knowledge Governance operator next-action 提示与清空/删除禁用只由现有 query 状态和 `chroma_reachable` 本地派生，不改变 RAG API shape。
