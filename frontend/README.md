...
- 当前主线：`release-observability-polish`，进度约 40%；release gate 内置前端 node 清单已纳入 task queue diagnostics 类型契约测试，发布/e2e artifacts 显式保留 14 天，release gate Markdown/JSON summary 已输出 schema version、步骤聚合计数与失败步骤标签。
