...
- **上下文 · Memory 占位**：「上下文」页展示 **Chroma 会话 Memory** 规划说明与 `memory_{session_id}` 命名示意（向量服务未接入）

## 下一步

与仓库根目录 `README.md` 一致：接入 **Chroma**、ingest/检索骨架；后端 **TraceStep** 已与 OpenAPI 对齐（见 `backend` README）。