import type { Messages } from "./types";

export const en: Messages = {
  errors: {
    networkBanner: "Cannot reach the backend API.",
    networkHint:
      "Make sure the API is running (default http://127.0.0.1:8000) and check NEXT_PUBLIC_API_BASE_URL.",
    unknownBanner: "Something went wrong with the request.",
    notFound: "The resource was not found (404).",
    auth: "You do not have permission to call this endpoint.",
    server: "Server error. Please try again later.",
    checkBackend: "If this persists, check the backend logs.",
    requestFailed: "Request failed",
  },
  auth: {
    requestFailedRetry: "Request failed. Please try again later.",
    requestFailedWithStatus: (status: number) => `Request failed (${status})`,
    invalidEmail: "Please enter a valid email address.",
    expiredRelogin: "Your session has expired. Please sign in again.",
    loginSuccess: "Signed in successfully.",
    registerSuccessAutoLogin: "Account created. Signed in automatically.",
    logoutSuccess: "Signed out.",
    checkingStatus: "Checking authentication status...",
    heroTitle: "Observable Agent Workbench",
    heroDesc:
      "Unify chat, execution traces, RAG, and cost metrics in one workbench, forming a closed loop from conversation to task delivery.",
    heroFeatureSseTrace: "Real-time SSE streaming with end-to-end trace replay",
    heroFeatureSessionTaskTrace: "Session, task, and trace linked in one view",
    heroFeatureMemoryRag: "Dual context support with Memory and RAG",
    heroFeatureTokenCost: "Task-level token and cost tracking throughout",
    formTitle: "Account Sign In",
    tabLogin: "Sign In",
    tabRegister: "Sign Up",
    emailLabel: "Email",
    displayNameOptionalLabel: "Display Name (optional)",
    displayNamePlaceholder: "Your display name",
    passwordLabel: "Password",
    passwordPlaceholderRegister: "At least 8 characters",
    passwordPlaceholderLogin: "Enter your password",
    submitLogin: "Enter Workbench",
    submitRegister: "Create account and continue",
    settingsButton: "Appearance & language",
    settingsTitle: "Sign-in page settings",
  },
  workbench: {
    selectSessionForHistory: "Pick a session on the left to load history.",
    sessionEmpty: "No messages in this session yet.",
    loadedHistory: "History loaded.",
    loadingMessages: "Loading messages…",
    noSessions: "No sessions yet.",
    streamFailedBadge: "This reply failed (may not be fully saved)",
    liveRegionDone: "Assistant finished responding.",
    liveRegionError: "Assistant response failed.",
    composerEnterSend: "Enter to send, Shift+Enter for a new line.",
    composerGenerating: "Generating reply…",
    composerCoolingDown: "Stopping the previous remote request. Please wait a moment.",
    cmdKHint: "⌘K / Ctrl+K focuses the input",
    traceExpandAll: "Show all steps",
    traceCollapse: "Show last 6 steps only",
    traceHidden: (n: number) => `${n} more steps hidden`,
    phaseDone: "Done",
    phaseError: "Failed",
    phaseCancelled: "Cancelled",
    phaseTimeout: "Timed out",
    phaseReplay: "Replaying",
    phaseRunning: "Running",
    phaseIdle: "Idle",
    sessionFallback: () => "New session",
    taskFallback: (shortId: string) => `Task ${shortId}`,
    hintRowNarrow:
      "On narrow screens, use the Session list and Trace & context buttons at the top.",
    sessionMissingReset:
      "This session no longer exists; selection was cleared.",
  },
  roles: {
    user: "You",
    assistantShort: "IA",
    assistantName: "InsightAgent",
  },
  chat: {
    kicker: "Chat",
    newChatTitle: "New chat",
    updatedAt: (ts: string) => `Updated ${ts}`,
    sessionList: "Session list",
    traceAndContext: "Trace & context",
    modeLabel: "Mode",
    ready: "Ready",
    generating: "Generating",
    closeBanner: "Dismiss",
    relatedTask: (shortId: string) => `Task ${shortId}`,
    streamOutputting: "Streaming",
    streamInterrupted: "Interrupted",
    streamLatest: "Latest output",
    scrollToBottom: "↓ Back to bottom",
    scrollToBottomAria: "Scroll to bottom",
    runtimeNoticeMock: "Running in mock mode. Responses are simulated.",
    runtimeNoticeRemote:
      "Running in remote mode. Verify provider, model, base URL, and API key settings.",
    runtimeNoticeRemoteMissingKey:
      "Remote mode is selected but API key is not configured yet.",
    goConfigureModel: "Configure model",
    remoteModeNeedConfig:
      "Remote mode is not fully configured. Please complete model settings first.",
    sessionEmptyTitle: "No messages in this session",
    sessionEmptyLead: "Type below and send to start.",
    onboardingTitle: "Start a conversation",
    onboardingLead:
      "Type and send to begin; a session is created if none is selected. Trace and context live in the right panel (use the top buttons on small screens).",
  },
  composer: {
    placeholder: "Ask a question or describe a task…",
    inputAria: "Message input",
    retry: "Retry last send",
    send: "Send",
    sending: "Generating…",
  },
  stream: {
    idleHint:
      "After sending a message, execution trace appears on the right; stream status details are shown here.",
    streamStarted: "Task stream started.",
    streamCompleted: "Task stream completed (done).",
    streamCancelled: "Task was cancelled.",
    streamTimeout: "Task timed out.",
    streamHeartbeat: "Receiving task stream (heartbeat ok).",
    loadingPersistedTrace: "Loading persisted trace...",
    persistedTraceLoaded: "Persisted trace loaded.",
    persistedTraceEmpty: "Persisted trace is empty for this task.",
    taskIdRequiredTrace: "Task ID is required to load trace.",
    failedLoadPersistedTrace: "Failed to load persisted trace.",
    taskIdRequiredDelta: "Task ID is required to load trace delta.",
    loadingTraceDeltaAfter: (seq: number) =>
      `Loading trace delta after seq=${seq}...`,
    traceDeltaLoaded: (count: number) => `Trace delta loaded (${count} steps).`,
    traceDeltaLoadedMore: (count: number) =>
      `Trace delta loaded (${count} steps, more available).`,
    traceDeltaEmpty: "No new trace delta steps.",
    failedLoadTraceDelta: "Failed to load trace delta.",
    toolRunning: (name: string) => `Tool running: ${name}`,
    toolStarted: (name: string) => `Tool started: ${name}`,
    toolStatus: (status: string, name: string) => `Tool ${status}: ${name}`,
    streamErrorByCode: (code: string) => {
      const normalized = code.trim().toLowerCase();
      if (normalized === "remote_api_key_required") {
        return "Remote mode requires an API key.";
      }
      if (normalized === "remote_base_url_required") {
        return "Remote mode requires a base URL.";
      }
      if (normalized === "remote_api_key_unauthorized") {
        return "Remote provider authorization failed. Check API key and base URL.";
      }
      if (normalized === "remote_provider_rate_limited") {
        return "Remote provider rate limit reached. Please retry later.";
      }
      if (normalized === "remote_provider_upstream_error") {
        return "Remote provider is temporarily unavailable. Please retry later.";
      }
      if (normalized === "remote_provider_http_error") {
        return "Remote provider request failed. Check provider configuration.";
      }
      if (normalized === "remote_provider_network_error") {
        return "Failed to reach remote provider. Check network or base URL.";
      }
      if (normalized === "remote_provider_invalid_json") {
        return "Remote provider returned invalid JSON.";
      }
      if (normalized === "remote_provider_empty_response") {
        return "Remote provider returned no usable content.";
      }
      if (normalized === "remote_provider_stream_invalid_json") {
        return "Remote provider stream returned malformed JSON chunks.";
      }
      if (normalized === "remote_provider_stream_interrupted") {
        return "Remote provider stream was interrupted.";
      }
      if (normalized === "task_cancelled") {
        return "Task was cancelled.";
      }
      if (normalized === "task_timeout") {
        return "Task timed out.";
      }
      return null;
    },
    streamErrorFallback: "Task stream error received.",
    streamErrorMessage: (
      message: string,
      fatal: boolean | null,
      retryCount: number | null,
    ) => {
      const fatalSuffix =
        fatal === null ? "" : fatal ? " (fatal)" : " (retryable)";
      const retrySuffix =
        typeof retryCount === "number" ? ` [retry=${retryCount}]` : "";
      return `Task stream error: ${message}${fatalSuffix}${retrySuffix}`;
    },
    promptEmpty: "Prompt cannot be empty.",
    creatingAndOpening: "Creating task and opening task stream...",
    taskCreateFailed: "Task creation failed.",
    streamClosed: "Task stream closed.",
    failedReadStream: "Failed to read task stream.",
    streamRecoveryStart: (taskId: string) =>
      `Recovering task stream (${taskId.slice(0, 8)}...).`,
    streamRecoveryDone: (taskId: string) =>
      `Task stream resumed (${taskId.slice(0, 8)}...).`,
    streamRecoveryFailed: (taskId: string) =>
      `Failed to resume task stream (${taskId.slice(0, 8)}...).`,
  },
  sidebar: {
    ariaLabel: "Session list",
    brandTitle: "Observable Agent",
    brandLead: "Dialogue opens, tasks close—execution visible, in the trace.",
    sessionsHeading: "Recent sessions",
    newSession: "New session",
    creating: "Creating…",
    loading: "Loading…",
    empty: "No sessions yet.",
    currentUserLabel: "Signed in as",
    currentUserUnknown: "Unknown user",
    settingsButton: "Settings",
    logout: "Log out",
    settingsMenuLabel: "Menu",
    menuTheme: "Theme",
    menuAccent: "Accent color",
    menuLanguage: "Language",
    menuModel: "Model & runtime",
    menuAudit: "Audit logs",
    menuUsage: "Usage dashboard",
    menuKnowledgeBase: "Knowledge bases",
    subviewBack: "Back",
    themeCurrentDark: "Dark",
    themeCurrentLight: "Light",
    langCurrentZh: "中文",
    langCurrentEn: "English",
    deleteSessionTitle: "Delete session",
    deleteSessionConfirm:
      "Delete this session? Associated messages and tasks will be removed.",
    deleteSessionOk: "Delete",
    deleteSessionCancel: "Cancel",
    deleteSessionAria: "Delete session",
    accentCustomColor: "Custom color",
    renameSessionMenu: "Rename",
    deleteSessionMenu: "Delete",
    renameSessionModalTitle: "Rename session",
    renameSessionPlaceholder: "Session title",
    renameSessionSave: "Save",
    sessionMoreAria: "Session actions",
    collapseSidebarAria: "Collapse sidebar",
    expandSidebarAria: "Expand sidebar",
    loadMoreSessions: "Load more",
    usage: {
      title: "Usage dashboard",
      lead:
        "Aggregate token and cost usage for this account in a recent window, with global/current-session scope and session/task rankings.",
      scopeGlobal: "Global",
      scopeSession: "Current session",
      scopeSessionFallback:
        "No active session selected, switched to global scope automatically.",
      windowLabel: (n: number) => `Last ${n} days`,
      refresh: "Refresh",
      refreshAria: "Refresh usage dashboard",
      loading: "Loading usage dashboard…",
      loadFailed: (reason: string) => `Load failed: ${reason}`,
      summaryTasksTotal: "Tasks total",
      summaryTasksWithUsage: "Tasks with usage",
      summaryTotalTokens: "Total tokens",
      summaryTotalCost: "Total cost",
      summaryAvgTokens: "Avg tokens / task",
      summaryAvgCost: "Avg cost / task",
      coverage: (withUsage: number, total: number) =>
        `Coverage ${withUsage} / ${total}`,
      trendTitle: "Trend",
      metricTokens: "Tokens",
      metricCost: "Cost",
      trendEmpty: "No trend data yet.",
      sessionsTitle: "Top sessions",
      tasksTitle: "Top tasks",
      tableSession: "Session",
      tableTasksWithUsage: "Tasks",
      tableTotalTokens: "Total tokens",
      tableCost: "Cost",
      tableLastTaskAt: "Last update",
      tablePrompt: "Task",
      tableUpdatedAt: "Updated at",
      tableNoData: "No usage data to display.",
      promptEmpty: "No prompt excerpt",
    },
    knowledgeBase: {
      title: "Knowledge base governance",
      lead: "View knowledge bases and chunk counts for this account, and clear or delete collections.",
      statusConnected: "Chroma connected",
      statusDisconnected: "Chroma unreachable",
      kbCount: (n: number) => `${n} knowledge base(s)`,
      refresh: "Refresh",
      refreshAria: "Refresh knowledge base list",
      noKnowledgeBases: "No knowledge bases for this account yet.",
      tableKbId: "Knowledge base ID",
      tableCollection: "Collection",
      tableDocuments: "Chunks",
      tableActions: "Actions",
      actionClear: "Clear",
      actionDelete: "Delete",
      actioning: "Processing…",
      clearConfirmTitle: (kbId: string) => `Clear knowledge base "${kbId}"?`,
      clearConfirmDescription:
        "All chunks in this knowledge base will be removed, while the knowledge base remains.",
      deleteConfirmTitle: (kbId: string) =>
        `Delete knowledge base "${kbId}"?`,
      deleteConfirmDescription:
        "This permanently deletes the knowledge base and all its data.",
      clearDone: (kbId: string, deleted: number) =>
        `Cleared ${kbId} (${deleted} removed).`,
      deleteDone: (kbId: string, deleted: number) =>
        `Deleted ${kbId} (${deleted} removed).`,
      opFailed: "Operation failed",
    },
    audit: {
      title: "Audit logs",
      lead:
        "Use dropdowns and text inputs to filter audit records quickly. The table shows key fields, and row expand reveals full details with raw JSON.",
      filterEventLabel: "Event type",
      filterEventAll: "All",
      filterEventLogin: "Login",
      filterEventLogout: "Logout",
      filterEventRefresh: "Refresh",
      filterEventSettingsUpdate: "Settings update",
      filterRangeLabel: "Time range",
      filterRangeAll: "All",
      filterRange7d: "Last 7 days",
      filterRange30d: "Last 30 days",
      filterSessionId: "Session ID",
      filterTaskId: "Task ID",
      filterSessionPlaceholder: "Optional exact session_id",
      filterTaskPlaceholder: "Optional exact task_id",
      filterReset: "Reset filters",
      searchPlaceholder: "Optional keyword filter for event/content/detail",
      loading: "Loading audit logs…",
      error: "Failed to load audit logs.",
      empty: "No audit events under current filters.",
      loadMore: "Load more",
      exportJson: "Export JSON",
      exportCsv: "Export CSV",
      exportLoading: "Exporting…",
      exportDone: (n: number) => `Exported ${n} audit events`,
      exportScopeLabel: "Export scope",
      exportScopeCurrent: "Current page",
      exportScopeAll: "All filtered results",
      total: (n: number) => `${n} total`,
      eventAt: (ts: string) => `At ${ts}`,
      detailLabel: "Detail",
      detailExpand: "Expand details",
      eventLabelLogin: "Login",
      eventLabelLogout: "Logout",
      eventLabelRefresh: "Token refresh",
      eventLabelSettingsUpdate: "Settings update",
      eventLabelUnknown: "Unknown event",
      colEvent: "Event",
      colTime: "Time",
      colSession: "Session",
      colTask: "Task",
      colDetail: "Content",
      rawJsonLabel: "Raw JSON",
      boolYes: "Yes",
      boolNo: "No",
      reasonPassword: "Email/password login",
      reasonRegisterAutoLogin: "Auto login after registration",
      scopeAll: "All sessions",
      scopeSingle: "Single session",
      scopeRefreshToken: "By refresh token",
      fieldReason: "Reason",
      fieldScope: "Scope",
      fieldRevoked: "Revoked",
      fieldMode: "Mode",
      fieldProvider: "Provider",
      fieldModel: "Model",
      fieldBaseUrlConfigured: "Base URL configured",
      fieldApiKeyConfigured: "API key configured",
      sessionLinked: "Linked session",
      sessionUserScope: "Account-level setting",
      taskLinked: "Linked task",
    },
  },
  inspector: {
    ariaShell: "Execution and context",
    ariaTablist: "Panel switcher",
    tabTrace: "Trace",
    tabContext: "Context",
    traceKicker: "Execution trace",
    timelineTitle: "Step timeline",
    stepsCount: (n: number) => `${n} steps`,
    stepsNone: "None",
    replayTrace: "Replay trace",
    loadDelta: "Load delta",
    stepEmpty: "No text for this step.",
    traceEmpty:
      "After you send a message, steps appear here; or pick a historical task under Context to replay.",
    traceNoMatch: "No trace steps match the current filters.",
    traceVisibleCount: (shown: number, total: number) =>
      `Showing ${shown} / ${total} steps`,
    traceFilterAll: "All",
    traceFilterThought: "Thought",
    traceFilterAction: "Action",
    traceFilterObservation: "Observation",
    traceFilterTool: "Tool",
    traceFilterRag: "RAG",
    traceFilterOther: "Other",
    traceSearchPlaceholder: "Search trace by title, content, ID, model, or tool",
    traceDensityComfortable: "Comfortable",
    traceDensityCompact: "Compact",
    contextKicker: "Context",
    summaryTitle: "Run summary",
    contextJumpTo: "Jump to",
    contextJumpOverview: "Overview",
    contextJumpSync: "Sync",
    contextJumpUsage: "Usage",
    contextJumpTaskSnapshot: "Task snapshot",
    contextJumpSessionExport: "Session export",
    contextJumpMemory: "Memory",
    contextJumpRag: "Knowledge base",
    contextJumpAudit: "Audit",
    contextJumpTasks: "Tasks",
    currentPhase: "Current phase",
    currentTask: "Current task",
    traceCursor: "Trace cursor",
    traceSyncStatus: "Delta sync",
    traceSyncRetries: "Retry count",
    traceSyncLastOk: "Last success",
    traceSyncNextRetry: "Next retry",
    traceSyncStateIdle: "Idle",
    traceSyncStateSyncing: "Syncing",
    traceSyncStateOk: "Healthy",
    traceSyncStateRetrying: "Retrying",
    traceSyncStatePaused: "Paused (background)",
    traceSyncWarning: (n: number) =>
      `Delta sync has failed ${n} times in a row and is retrying automatically.`,
    traceSyncLastError: (message: string) => `Last error: ${message}`,
    traceSyncRecovered: (time: string) => `Delta sync recovered at ${time}.`,
    traceSyncRetryEta: (seconds: number) =>
      `Next retry in about ${seconds}s.`,
    traceSyncLead: "Monitor sync health and retry behavior for trace delta fetching.",
    session: "Session",
    usageTitle: "Usage (current task)",
    usagePrompt: "Prompt tokens",
    usageCompletion: "Completion tokens",
    usageTotal: "Total tokens",
    usageCost: "Cost estimate",
    usageAvgTotal: "Avg total/task",
    usageAvgCost: "Avg cost/task",
    usageSummaryTitle: "Usage summary",
    usageScopeSession: "Scope: current session",
    usageScopeGlobal: "Scope: all tasks",
    usageLead: "Track token and cost metrics for current run and aggregate scope.",
    usageSessionTitle: "Session usage summary",
    usageTaskCount: (n: number) => `Tasks counted: ${n}`,
    usageTaskCoverage: (withUsage: number, total: number) =>
      `Tasks with usage: ${withUsage} / ${total}`,
    usageSummaryLoading: "Loading usage summary…",
    usageSummaryError: "Usage summary is temporarily unavailable.",
    usageSummaryEmpty: "No usage data is available in this scope yet.",
    usageSessionLoading: "Loading session usage summary…",
    usageSessionError: "Session usage summary is temporarily unavailable.",
    usageSessionEmpty: "No usage data is available for this session yet.",
    taskSnapshotTitle: "Task snapshot",
    taskSnapshotLead:
      "Inspect the selected task input, output summary, RAG hits, and error clues.",
    taskSnapshotStatus: "Task status",
    taskSnapshotCreatedAt: "Created at",
    taskSnapshotUpdatedAt: "Updated at",
    taskSnapshotStepCount: "Trace steps",
    taskSnapshotRagHits: "RAG hits",
    taskSnapshotRagKnowledgeBases: (ids: string) =>
      `Knowledge bases hit: ${ids}`,
    taskSnapshotPrompt: "Task input",
    taskSnapshotFinalAnswer: "Final answer (summary)",
    taskSnapshotFinalAnswerEmpty: "No final answer could be parsed from trace.",
    taskSnapshotLastObservation: "Last observation",
    taskSnapshotNoSelection: "Select a task from the task list first.",
    taskExportJson: "Export JSON",
    taskExportMarkdown: "Export Markdown",
    taskExportJsonDone: "Task JSON exported.",
    taskExportMarkdownDone: "Task Markdown exported.",
    sessionExportTitle: "Session export",
    sessionExportLead:
      "Export current session messages, task summaries, trace preview, and usage.",
    sessionExportNoSelection: "Select a session before exporting.",
    sessionExportJson: "Export Session JSON",
    sessionExportMarkdown: "Export Session Markdown",
    sessionExportJsonDone: "Session JSON exported.",
    sessionExportMarkdownDone: "Session Markdown exported.",
    currentTaskCard: "Current task",
    statusPrefix: "Status: ",
    latestTaskSession: "Latest task in this session",
    recentTasks: "Recent tasks",
    sessionTasks: "Tasks in this session",
    taskViewLabel: "Task view",
    taskIndexLead: "Filter and scan tasks to locate failures and recent updates quickly.",
    taskFilterAll: "All",
    taskFilterRunning: "Running",
    taskFilterDone: "Done",
    taskFilterError: "Failed",
    taskSortLatest: "Latest",
    taskSortOldest: "Oldest",
    taskPinFailed: "Failed first",
    taskSearchPlaceholder: "Search by task title or ID",
    taskFailureHint: "Failure hint",
    taskVisibleCount: (shown: number, total: number) =>
      `Showing ${shown} of ${total}`,
    taskEmpty: "No tasks match this filter.",
    taskCancel: "Cancel task",
    taskCancelDone: "Task cancel requested",
    taskCancelAlreadyTerminal: "Task is already finished",
    taskCancelFailed: "Failed to cancel task",
    loadMoreTasks: "Load more",
    backendUrl: "API base: ",
    seqLabel: "seq",
    collapseInspectorAria: "Collapse right panel",
    expandInspectorAria: "Expand trace and context",
    traceMeta: {
      model: "Model",
      stepKind: "Kind",
      tokens: "Tokens",
      cost: "Cost",
      toolLine: (name: string, status: string) =>
        `Tool ${name} · ${status}`,
      toolRetry: (count: number) => `retry ${count}`,
      toolError: (message: string) => `error ${message}`,
      ragLine: (chunkCount: number, knowledgeBaseId?: string) =>
        knowledgeBaseId
          ? `RAG · ${chunkCount} chunks · ${knowledgeBaseId}`
          : `RAG · ${chunkCount} chunks`,
    },
    traceViewList: "Timeline",
    traceViewFlow: "Flow",
    traceFlow: {
      kindThought: "Thought",
      kindAction: "Action",
      kindObservation: "Observation",
      kindTool: "Tool",
      kindRag: "RAG",
      kindOther: "Step",
      contentDetails: "Content",
      contentEmpty: "(empty)",
    },
    memory: {
      kicker: "Session memory",
      title: "Memory (Chroma)",
      lead:
        "Vectors are stored in `memory_{session_id}`; a summary is appended after each successful task. Use the controls below to add text and run a semantic query (requires a reachable Chroma server).",
      collectionLabel: "Collection name",
      chromaConnected: "Connected",
      chromaDisconnected: "Unreachable",
      docCount: (n: number) => `Embeddings: ${n}`,
      collectionExists: "Collection exists",
      collectionMissing: "No collection yet",
      statusLoading: "Loading memory status…",
      pickSession: "Select a session to show Chroma status and counts.",
      debugKicker: "Debug",
      addPlaceholder: "Paste text to store in this session’s memory…",
      addButton: "Add",
      addEmpty: "Enter some text before adding.",
      queryInputEmpty: "Enter a query before searching.",
      addSuccess: (n: number) => `Saved. Total: ${n}.`,
      queryPlaceholder: "Ask a question or enter keywords…",
      queryButton: "Query",
      queryEmpty: "No hits.",
      queryHits: (n: number) => `${n} hit(s)`,
      distanceLabel: "Distance",
      metadataPlaceholder:
        'Optional JSON object with string keys and values, e.g. {"source":"manual","kind":"note"}',
      metadataInvalid:
        "metadata must be a JSON object whose values are strings.",
      hitMetadataLabel: "Metadata",
    },
    rag: {
      kicker: "Knowledge retrieval",
      title: "RAG (Knowledge Base)",
      lead:
        "Ingest text into a knowledge base and query semantic hits. Retrieved chunks are also reflected in trace as RAG steps.",
      kbIdLabel: "Knowledge base ID",
      kbIdPlaceholder: "Default: default ([a-z0-9_-])",
      applyKb: "Apply",
      statusLoading: "Loading knowledge base status…",
      ingestPlaceholder:
        "Paste text to ingest into the knowledge base (auto chunking enabled)…",
      ingestSourcePlaceholder: "Optional source label (e.g. handbook.md)",
      ingestButton: "Ingest",
      ingestEmpty: "Enter text before ingesting.",
      ingestSuccess: (chunks: number, total: number) =>
        `Ingested ${chunks} chunks. Total: ${total}.`,
      queryPlaceholder: "Ask a question or enter keywords…",
      queryButton: "Query KB",
      queryEmptyInput: "Enter a query before searching.",
      queryEmpty: "No hits in this knowledge base yet.",
      queryHits: (n: number) => `${n} hit(s)`,
      docCount: (n: number) => `Knowledge chunks: ${n}`,
      collectionExists: "Collection exists",
      collectionMissing: "No collection yet",
      chromaConnected: "Connected",
      chromaDisconnected: "Unreachable",
      distanceLabel: "Distance",
      hitMetadataLabel: "Metadata",
    },
    audit: {
      kicker: "Security audit",
      title: "Audit logs (minimal)",
      lead:
        "Inspect login, logout, refresh, and settings update events with type and time filters.",
      filterEventLabel: "Event type",
      filterEventAll: "All",
      filterEventLogin: "Login",
      filterEventLogout: "Logout",
      filterEventRefresh: "Refresh",
      filterEventSettingsUpdate: "Settings update",
      filterRangeLabel: "Time range",
      filterRangeAll: "All",
      filterRange7d: "Last 7 days",
      filterRange30d: "Last 30 days",
      loading: "Loading audit logs…",
      error: "Failed to load audit logs.",
      empty: "No audit events for the current filters.",
      loadMore: "Load more audit events",
      total: (n: number) => `${n} total`,
      eventAt: (ts: string) => `At ${ts}`,
      detailLabel: "Detail",
      eventLabelLogin: "Login",
      eventLabelLogout: "Logout",
      eventLabelRefresh: "Token refresh",
      eventLabelSettingsUpdate: "Settings update",
      eventLabelUnknown: "Unknown event",
    },
  },
  settings: {
    title: "Runtime settings",
    lead: "Configure provider, model, and mode; changes apply to the whole workbench after save.",
    close: "Dismiss",
    loading: "Loading settings…",
    fieldMode: "Mode",
    fieldProvider: "Provider",
    fieldModel: "Model",
    fieldBaseUrl: "Base URL",
    fieldApiKey: "API key",
    optionalPlaceholder: "Optional",
    save: "Save settings",
    saving: "Saving…",
    validate: "Validate config",
    validating: "Validating…",
    validatePass: "Validation passed",
    validateFail: "Validation failed",
    validateErrorByCode: (code: string) => {
      const normalized = code.trim().toLowerCase();
      if (normalized === "remote_api_key_required") {
        return "Missing API key. Configure it before validation.";
      }
      if (normalized === "remote_base_url_required") {
        return "Missing base URL. Configure it before validation.";
      }
      if (normalized === "remote_base_url_invalid") {
        return "Invalid base URL. Use a full http(s) URL.";
      }
      if (normalized === "remote_api_key_unauthorized") {
        return "Authorization failed (401/403). Check API key and base URL pairing.";
      }
      if (normalized === "remote_preflight_network_error") {
        return "Preflight network check failed. Verify connectivity and gateway URL.";
      }
      if (normalized === "remote_base_url_unexpected_status") {
        return "Preflight returned an unexpected status. Check gateway and model endpoint.";
      }
      return null;
    },
    saveSuccess: "Settings saved successfully",
    saveFail: "Failed to save settings",
    requiredSuffix: "is required",
    apiKeyConfiguredClear: "Already set (leave blank to keep current key)",
    apiKeyRemoteRequired: "Required for initial remote setup",
    metaProvider: "Provider",
    metaModel: "Model",
    metaBaseUrl: "Base URL",
    metaApiKey: "API key",
    metaConfigured: "Configured",
    metaNotConfigured: "Not set",
    metaDatabase: "Database",
    remoteCompatibilityHint:
      "Remote currently supports OpenAI-compatible APIs only (/chat/completions with SSE).",
    appearanceTitle: "Appearance & language",
    themeLabel: "Theme",
    themeDark: "Dark",
    themeLight: "Light",
    languageLabel: "Language",
    languageZh: "中文",
    languageEn: "English",
    modalSubtitle:
      "Configure provider, model, and API credentials. Changes apply globally after save.",
  },
  a11y: {
    closeSessionDrawer: "Close session list",
    closeInspectorDrawer: "Close trace and context",
  },
};
