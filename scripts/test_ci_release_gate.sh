...
main() {
  ...
  assert_contains "npm run build:turbopack" "${TMP_DIR}/all.txt"
  ...
  assert_contains "- summary_kind: release_gate" "${TMP_DIR}/summary.md"
  assert_contains "- service_required: no" "${TMP_DIR}/summary.md"
  assert_contains "- phase: frontend" "${TMP_DIR}/summary.md"
  assert_contains "- result: DRY-RUN" "${TMP_DIR}/summary.md"
  assert_contains "- total_steps: 4" "${TMP_DIR}/summary.md"
  assert_contains "- dry_run_steps: 4" "${TMP_DIR}/summary.md"
  ...
  assert_contains '"service_required": false' "${TMP_DIR}/summary.json"
  assert_contains '"phase": "frontend"' "${TMP_DIR}/summary.json"
  assert_contains '"result": "DRY-RUN"' "${TMP_DIR}/summary.json"
  assert_contains '"step_summary": {"total": 4, "pass": 0, "fail": 0, "dry_run": 4}' "${TMP_DIR}/summary.json"
  ...
  assert_contains '"operator_summary": {"status": "review", "headline": "release gate dry run needs verification", "primary_action": "run_release_gate_without_dry_run", "highest_severity": "info", "total_steps": 4, "failed_steps": 0, "focus_phases": ["frontend"], "blocking_step_labels": []}' "${TMP_DIR}/summary.json"
  assert_contains '"label": "frontend build"' "${TMP_DIR}/summary.json"
  assert_contains '"label": "frontend Turbopack build"' "${TMP_DIR}/summary.json"
...
SH
  ...
  assert_contains "npm run build:turbopack" "${TMP_DIR}/frontend.txt"
