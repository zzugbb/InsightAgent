...
Behavior:
  ...
  - fail-on-empty: fail when included_count <= 0
...
status="ok"
if [ "${included_count}" -le 0 ] || [ "${missing_count}" -gt 0 ]; then
  status="warning"
fi
...
case "${strict_level}" in
  ...
  warn)
    ...
    ;;
  fail-on-empty)
    if [ "${included_count}" -le 0 ]; then
      gate_result="FAIL"
      gate_reason="strict-level fail-on-empty requires included_count>0"
      exit_code=1
    fi
    ;;
  fail-on-missing)
