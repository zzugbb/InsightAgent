...
1. `concurrency-fairness-policy`：当前主线；已完成可选按用户/按 session 并发执行槽位上限、capacity-aware oldest eligible FIFO 防插队、settings `task_queue_diagnostics` 等待策略/全局与当前用户安全计数/压力状态诊断与 backend queue e2e-like idle/压力诊断断言、安全 queue snapshot helper 覆盖，前端已展示 available slots 与当前用户计数，下一步按风险补真实低并发 e2e 复验或更细公平策略。
...
`GET /api/settings` 的响应包含只读 `task_queue_diagnostics`，用于观察当前单进程队列的 `max_concurrent`、`active_count`、`waiting_count`、`available_slots`、`current_user_active_count`、`current_user_waiting_count`、`current_user_limit_reached`、`has_waiting_tasks`、`saturated`、`pressure_state`、可选 per-user/per-session 上限、fairness 开关、`waiting_policy`、capacity-aware FIFO 标记与 poll interval；该字段不参与用户设置保存，也不改变 SSE / trace / export payload，不暴露内部 task ids。
- `POST /api/sessions`
