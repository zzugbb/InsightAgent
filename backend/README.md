...
- 当前主线：`runtime-dependency-modernization` 40%；Node 24 ESM 模块模式切片已完成，第二个切片新增后端 runtime dependency contract，并将 FastAPI 精确升级到首个声明支持 Python 3.14 的 `0.118.3`；Starlette 保持 `0.46.2`，路由注册不再触发 `asyncio.iscoroutinefunction` DeprecationWarning。
...
1. 当前主线：`runtime-dependency-modernization` 40%；Node 模块模式与 Python 3.14 FastAPI 兼容切片均已通过 release gate。
2. 下一切片核对 Next 工具链约束；后端继续保持 FastAPI 精确 pin、full slice 与 SSE / trace / export 外部契约稳定。
