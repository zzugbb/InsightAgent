...
- 本轮无后端运行时变更；前端任务详情页与 Workbench Inspector 语义筛选会清理旧搜索/kind 干扰，Task Center failure source chips 支持横向切换、重复点击取消 drilldown 并清理隐藏来源筛选，后端 SSE / trace / export shape 不变。
...
1. 当前状态：`product-ux-polish` 约 54%，后端侧暂保持契约稳定，配合前端 trace 回放与失败诊断可读性推进。
...
- Task Center failure source drilldown、重复点击取消与观测维度切换清理均为前端本地筛选语义，不改变任务列表 API、后端 trace 或 export shape。
