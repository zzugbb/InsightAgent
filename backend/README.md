...
- `backend/scripts/test_tool_runtime_slice.py` 拆分已完成：原入口继续保留，测试主体按 provider/source、planner、settings/registry、http_json、task/export/governance、runtime/result/rag 等主题拆到 `backend/scripts/tool_runtime_slice/`。
- 默认 settings 语义保持不变：provider/model/api_key 完整时自动走 `remote`，否则回退 canonical `mock`。

## 当前验证基线

- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`：`1707/1707` 通过
...
2. `pre-flight cleanup`：文档瘦身与 `backend/scripts/test_tool_runtime_slice.py` 拆分已完成，保持 `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 入口不变。
3. `tool_runtime.py` 分阶段抽模块：先候选抽 `tool_runtime_http_json.py`、`tool_runtime_registry.py`、`tool_runtime_planning.py`、`tool_runtime_results.py`、`tool_runtime_sanitization.py`，保留现有 import facade。
...
- `scripts/tool_runtime_slice/`：`test_tool_runtime_slice.py` 的主题 mixin 包，承接 provider/source、planner、settings/registry、http_json、task/export/governance、runtime/result/rag 等 slice 测试
