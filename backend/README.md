...
- 本轮无后端运行时变更；前端任务详情页语义筛选会同步可分享 URL，Task Center `failure_trace` 筛选打开详情可保持 Failure 回放焦点，后端 SSE / trace / export shape 不变。
...
1. 当前状态：`product-ux-polish` 约 25%，后端侧暂保持契约稳定，配合前端 trace 回放可读性推进。
...
- 任务详情页可通过兼容 URL 参数 `trace_semantic` 回放语义 Trace；该能力不改变后端 trace/export payload。
