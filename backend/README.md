...
- 当前主线：`test-maintainability-hardening`，进度 80%；四个大测试主题已拆为双分片，测试主题规模门禁为 2500 行，选择性运行支持零匹配诊断与匹配测试清单，后端运行时契约不变。
...
1. `test-maintainability-hardening` 当前 80%：收敛常用选择器清单与门禁自测，稳定原 mixin import 和测试发现入口。
