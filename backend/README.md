...
- 最近已对齐到代码的高信号能力：
  ...
  - 工具规划 wrapper 继续复用窄归一化：provider response content 中的 `tools/plan`、单个 provider plan item、`task_plan` 的 `planned_tool_names/labels/kinds`、action step 初始输入与 `normalize_tool_spec` 入口 input 若来自 `UserDict/UserList/UserString`，会在规划、过滤 extra planner、生成 task steps 与 trace 写入前转成 plain payload；普通文本不会被当成序列拆分。
