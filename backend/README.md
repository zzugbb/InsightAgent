...
1. `concurrency-fairness-policy`：当前主线，约 `99.8%`；已完成可选按用户/按 session 并发执行槽位上限、capacity-aware oldest eligible FIFO 防插队、duplicate active task 非拥有 slot 释放防护、旧等待项互斥 eligibility 容量估算、旧等待项预占 scope quota 后的当前任务准入判断、queued SSE queue snapshot 基础字段存在性与结构一致性、settings `task_queue_diagnostics` 基础字段存在性、等待策略/全局与当前用户/当前会话 active/waiting/available 安全计数、governance 必填字段、前端 diagnostics 类型契约、后端 `SettingsSummaryResponse` typed diagnostics 契约、`has_waiting_tasks`/`saturated` 一致性、`pressure_state` 派生一致性、当前用户/当前会话限额触顶与可用槽位诊断、available slots 字段依赖；backend queue e2e-like idle/压力诊断断言、安全 queue snapshot helper 覆盖、前端 available slots/当前用户/当前会话计数展示与本轮 backend queue e2e fresh 复验已完成，下一步主要是最终 diff/文档/提交收口。
