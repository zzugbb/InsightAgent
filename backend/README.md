...
- 当前进入 `observability-experience` 主线，进度约 82%；已完成任务失败线索聚合、来源分类、TaskResponse failure fields、task_failed audit event 兜底恢复、Usage Dashboard / Audit Logs 到任务详情的回放入口、Usage Dashboard top tasks 失败摘要派生、Audit Logs 失败 hint/source/code/message 可读详情、Audit Logs 服务端 keyword 过滤，以及前端共享 Trace Failure 语义统计/过滤与 Task Center 观测筛选；后端 SSE / trace / export shape 保持不变。
...
- frontend node tests：workbench utils / audit logs modal utils / task detail utils / stream store utils / model settings utils，当前 `98/98` 通过
