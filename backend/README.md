...
- 当前主线：`next-major-upgrade-readiness` 91%；前端 `set-state-in-effect` 已从全局关闭收窄为 6 个明确迁移文件，后端运行时及 SSE / trace / export 外部契约未修改。
...
- Frontend：Next 16 readiness contract `8/8`，真实预检 `ready_with_actions`、0 blocker、2 actions；runtime dependency contract `9/9`；release gate 内置 node tests `167/167`；`set-state-in-effect` 仅对 6 个存量文件保留例外，全库强制扫描剩 17 处；双构建均通过。
- E2E/CI：本切片真实服务目标 e2e `1/1`，覆盖 Runtime Debug RAG 输入保留、错误重试与知识库切换清理；最近 full Chromium 基线仍为 `59 passed / 1 skipped`。
- Hygiene：`py_compile`、`git diff --check`、`git diff --cached --check` 与备份计划 diff 检查通过；`data/insightagent.plan.back.md` 无修改。

## 下一步后端计划

1. `next-major-upgrade-readiness` 当前 91%；Runtime Debug 会话 Memory 状态已从全局 RAG 状态中拆分，目标 e2e 已确认无项目级 blocker。
2. 下一切片从前端 6 个显式债务文件继续迁移；后端保持 FastAPI 精确 pin、full slice 与 SSE / trace / export 外部契约稳定。
