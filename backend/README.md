...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1689/1689`）
  ...
  - `git diff --check` 通过
- 本轮收尾验证：先新增 `-k chat_completion` 与 `-k responses_api_tool_call_content` 红测并确认 Chat Completions `choices[].message.tool_calls`、legacy `choices[].message.function_call`、Responses API `output[].content[].tool_call` 与 typed SDK-style `output[].content[].tool_call` 失败，再补齐 provider planner 函数调用式 envelope 解析；既有 `choices[].message.content` 与 Responses API `output[].content[].text` JSON plan 路径保持稳定，`tool_name`/`function_name`/`function`、`parameters` 与 OpenAI-style `tool_calls[].function.name/arguments` 继续复用同一套受控 JSON 输入解析。同步验证 `-k responses_api_tool_call_content`、`-k chat_completion`、`-k build_tool_plan_provider`、`-k provider_tool_plan`、`-k provider_source`、`-k tool_registry`、`-k http_json`、完整 backend slice（`1689/1689`）、`bash scripts/test_ci_e2e_tooling.sh`、frontend node（`68/68`）、frontend build、backend full-stack e2e 与完整 Chromium browser e2e（`47/47`）均通过；普通沙箱跑 backend e2e 仍会被 `127.0.0.1:5432 Operation not permitted` 阻塞，提权后通过。
