...
- 最近已对齐到代码的高信号能力：
  ...
  - 本轮继续围绕 `http_json` 真实执行器收口请求/响应 wrapper、trace/export/SSE/audit/settings diagnostics 与真实上游错误诊断闭环；HTTPError wrapper 关闭、typed dump-json string wrapper、literal request mapping field-name wrapper、static diagnostics typed request value、execution summary typed wrapper、control field string wrapper、execution kind string wrapper、runtime semantic meta 的 execution_summary / execution_diagnostics / kind / runtime_semantic_kind wrapper、result preview/output keys 和 result-summary 输出字段 wrapper、registry/source spec wrapper、provider-generated tool plan、`task_plan` planned fields、iteration context 与 `normalize_tool_spec` 入口 wrapper、provider/source/loaders/factories/settings JSON 与 file manifest wrapper，以及 chat persistence trace display / markdown meta / RAG export 的 wrapper tool meta、输出投影 key、planner steps、result_summary、`execution_kind=UserString("http_json")`、`meta.rag.chunks` 与 `knowledge_base_id` 兼容均已进入回归，完整 backend slice 当前为 `1680/1680`。
  - Task/session export response summary 的 response-ready/plain-wrapper 回放继续补齐：`trace_preview`、`messages`、`trace.steps`、`rag_chunks` 与 `rag_knowledge_base_ids` 如果由 `UserList/UserString`、`model_dump()` item，或 plain dict 内层 wrapper 值进入，会先归一成 JSON-compatible payload，再复用现有 HTTP JSON 敏感字段脱敏；plain dict payload 的对象身份契约保持不变。
  - Task/session batch export payload summary 的 plain-wrapper 回放也继续前移：`get_task_rows_trace_preview_summary(...)`、`get_task_rows_export_summary(...)`、`get_task_rows_session_export_summary(...)` 与 `get_session_export_payload_summary(...)` 会在 batch/payload 聚合阶段清洗 `trace_preview` / nested `trace.preview`，避免只靠最后 response summary 才兜底脱敏。
  - Usage dashboard response summary 的 plain-wrapper outward rows 也继续对齐：`trend`、`by_session` 与 `top_tasks` 会先递归归一 `UserString` / `model_dump()` row，再进入 governance canonical label 与 response model，避免 usage 面板回放半迁移 payload 时残留不可序列化 wrapper。
  - Task response summary 的 plain-wrapper `trace_json` / `usage_json` 出口也继续对齐：`trace_json=UserString(...)` 会先转成普通字符串再复用 HTTP JSON 脱敏，`usage_json=UserString(...)` 会归一成可序列化字符串，task detail/list response model 继续保持既有字段 shape。
  - HTTP JSON response adapter 现在会在 response 同时暴露 `.json()` / `.json` parsed body 与泛型 `__iter__` 时优先使用显式 parsed JSON body；如果 `.json()`、typed payload `.json(...)` dump 方法或首个 `iter_*` streaming 方法只是调用形态不适配，会继续尝试本体 iterable、`model_dump/dict/to_dict` 或后续 iterator fallback，避免 SDK response 的 metadata/key 迭代、typed dump 方法差异或 iterator 方法签名差异被误报成 `transport error`。
  - HTTP JSON typed parsed body fallback 继续补齐：当根层或嵌套 typed payload 暴露的 `model_dump()` 只是 SDK/Pydantic 方法签名不适配时，会继续尝试 `dict()` / `to_dict()`，真实不可序列化 body 仍保留原有 JSON serializable 诊断。
  - HTTP JSON typed dump-json 错误诊断继续收紧：`model_dump_json()` / `to_json()` / typed `.json()` 只有 TypeError 调用形态不适配时才会 fallback；真实 serializer 运行时异常会保留 `response json body ... failed` 诊断，不会被后续 `to_dict()` 掩盖成假成功。
  - HTTP JSON typed dump-json TypeError 诊断继续细化：runtime 会先用 callable signature 判断零参 dump 调用是否可用，只有签名确实不匹配时才继续 fallback；签名可匹配后 serializer 内部抛出的 TypeError 会保留为真实上游/SDK dump 失败。
  - HTTP JSON typed dump callable signature metadata fallback 继续补齐：如果真实 SDK callable 的 `__signature__` 元数据本身抛错或不可检查，runner 会回到“未知签名但尝试真实调用”的路径；根层/嵌套 response parsed body、request `json_body`、`query_params` 与 `headers` 的 typed dump 都会继续执行，不会把坏 signature metadata 误报成上游 transport/config 错。
  - HTTP JSON typed dump-json string wrapper fallback 继续补齐：`model_dump_json()` / `to_json()` / typed `.json()` 返回 `UserString` 这类 string-like wrapper 时，会按 UTF-8 JSON 文本处理；根层/嵌套 response parsed body、request `json_body`、`query_params` 与 `headers` 均已覆盖。
  - HTTP JSON static diagnostics 继续复用真实 request coercion：direct typed request value，以及 supported runtime template 渲染出的 typed header/query/body value，会先归一成 JSON-compatible payload 再进入 object/value、Content-Type/Accept、duplicate query/header 与 reserved runtime template typo 诊断；typed adapter dump 出来的 `settings_*` / `tool_registry_*` typo 不再漏过 preflight。
  - HTTP JSON execution summary 继续复用同一套静态归一化：direct typed request mapping、`UserString` response_path 与 root template 渲染出的 typed `result_fields` 会正确产出 header/query/body count、response_path 与 result_field_names；通用 template render/missing/reference 扫描也会把 `UserString` 当文本处理，避免摘要侧路递归或漏字段。
  - HTTP JSON control field string wrapper 继续复用窄归一化：`url`、`method`、`timeout_ms` 的 `UserString` 会按 string-like value 进入 validation、runner 与 execution_summary，`method` runtime template 也会在摘要里按静态上下文渲染；普通 object 仍保持非法或默认语义。
  - HTTP JSON execution kind string wrapper 继续复用入口枚举归一化：`execution.kind=UserString("http_json")` 会进入 validation、runner dispatch 与 execution_summary，不再被误判成缺失 kind；普通 object 仍不会被随意 stringify。
  - HTTP JSON runtime semantic wrapper 继续复用窄归一化：registration 已携带的半迁移 `execution_summary` / `execution_diagnostics` 会接受 `UserString` key/value/list item 并在进入 `tool_start`、action meta、trace/export 前统一脱敏；`kind` / `runtime_semantic_kind` 的 `UserString` 会按现有 trace 契约归一成工具名级 `semantic_kind` 与 family 级 `semantic_family`。
  - HTTP JSON result projection wrapper 继续复用窄归一化：`result_preview_keys` / `result_output_keys` 若以 `UserList` 容器和 `UserString` key 进入 registration，仍会被当成显式投影并过滤敏感字段；真实输出里的 planner `steps`、calc `expression`、retrieval `knowledge_base_id` / `request_id` wrapper 会继续生成稳定 result summary 与 observation。
  - HTTP JSON registry spec wrapper 继续复用窄归一化：direct `extra_tools`、`overrides` 与 provider adapter spec 若来自 `UserDict/UserList/UserString` 半迁移对象，会先转成 plain registry spec，再进入 extra tool 构建、override 构建、provider/source adapter 与 invalid execution diagnostics；普通不可序列化对象仍保持跳过/诊断语义。
  - 工具规划 wrapper 继续复用窄归一化：provider response content 中的 `tools/plan`、单个 provider plan item、`task_plan` 的 `planned_tool_names/labels/kinds/execution_kinds`、action step 初始输入与 `normalize_tool_spec` 入口 input 若来自 `UserDict/UserList/UserString`，会在规划、过滤 extra planner、生成 task steps 与 trace 写入前转成 plain payload；普通文本不会被当成序列拆分。
  - Provider/source/file-backed registry wrapper 继续复用窄归一化：`tool_registry_*_json` settings 字段若是 `UserString`，loader/provider/source/factory artifacts 会按 JSON object 解析；file manifest、plain registry file 与 loader adapter 中的 `UserDict/UserList/UserString` 会先归一再处理 profile、disabled tools、registry_files/dirs/sources、extra_tools 与 overrides；provider adapter 通过 `registry_file` 构建真实工具时会使用当前 provider source 的模板上下文，provider/loader factory 与命名 provider/loader 背后的 `registry_file` 也会按 source settings 重建；registry-file backed factory 自身声明的 `profile` / disabled / `extra_tools` / `overrides` 会在 factory 调用时通过 adapter 生效，factory->factory alias 外层声明的 `extra_tools` / `overrides` 也会继续叠加到内层 file-backed factory，真实 HTTP JSON 请求 URL/header/query、response_path/result_fields 与 invalid execution diagnostics 不再因 alias 层丢失；当 alias 指向 profile factory 并由 outer override 重新启用 profile-disabled 工具时，diagnostics 也会以 default registry 为基准校验该 override 的 HTTP JSON 配置；provider/source 通过 provider_factory、loader_factory 或 provider factory alias 显式把 profile 重置为 `default` 时，会返回合成后的 effective registry，不再保留内层 `retrieval_only` 等 profile 的 disabled tools；registry-file backed factory、settings-backed named loader/provider、以及 source adapter 自身的 override invalid HTTP JSON execution diagnostics 都会进入对应 artifacts，并能并回 selected source diagnostics；manifest `registry_sources` 引用 settings-backed source/provider/loader/factory 时，相关 `registry_file` 相对路径会按当前 manifest 所在目录解析；已访问 registry file 会继续防循环，direct source 自引用、命名 provider/loader/factory 背后的同文件引用，以及 `source -> provider -> loader/loader_factory`、`source -> provider_factory`、`source -> source` 这类传递链都会折算为 `skipped_registry_sources` 而不是误报 missing source；如果命名 loader/provider/factory 或 source->source 链路里的 inner source 因缺失 registry file 无法构建，已收集的 missing/skipped/invalid diagnostics 仍会保留到 artifact map 并并回 root source diagnostics，同时保留 child source 自己的模板上下文，避免 `$tool_registry_provider_source` 被全局 selected source 串值；source spec 的 effective profile 也会进入 file-backed manifest、inline extra_tools、`source -> named provider -> registry_file`、`source -> named loader -> registry_file`、`source -> named provider -> named loader -> registry_file` 以及 provider/loader factory 的 HTTP JSON 请求和 preflight summary。本轮 real search 组合回归继续把 selected source/profile 锁进 POST headers、query_params、json_body，并覆盖 plan item 的 preview/output/summary 与 named loader/factory 的 preflight field counts；file-backed real calc 也已覆盖 provider planner、plan item POST request、preview/output/result_summary/observation、trace preview/session export response 回放、trace detail/delta 与 task export JSON 的结构化 result_summary 回填、preflight summary、脱敏 diagnostics 以及 settings validate tool details。
  - Provider/source inline shorthand 继续复用同一套 source/profile 上下文：`tool_registry_provider_sources_json`、`tool_registry_providers_json` 与 `tool_registry_loaders_json` 中的 source/provider/loader 若在根级直接声明 real tool shorthand，并同时声明 `profile` / disabled / overrides 等 adapter 控制字段，根级工具会并入对应 `extra_tools`，不会因为增加 profile 或治理字段而从 selected registry 消失；同名 `overrides` 会在合并后继续胜过 inline base tool，真实 HTTP JSON search 仍会按 selected source/profile 渲染 header/query/body、response_path 与 result_fields。
  - File manifest `registry_sources` 的 source->source forward reference 也继续复用 source 依赖排序：outer source 可以先于 inner source 出现在 settings JSON 中，后端会先解析同批 child source，再构建 outer source；真实 HTTP JSON 请求继续用 child source/profile 渲染模板，inner 缺失 registry file 的 diagnostics 会继续并回 outer/root manifest，循环 diagnostics 仍沿原路径输出。
  - Settings-backed named provider 的 provider->provider forward reference 也继续复用 provider 依赖排序：outer provider 可以先于 inner provider 出现在 `tool_registry_providers_json` 中，后端会先构建同批 inner provider，再让 outer provider 继承 registry 并叠加自身 extra tools / disabled tools。
  - Settings-backed named loader 的 loader->loader forward reference 也继续复用 loader 依赖排序：outer loader 可以先于 inner loader 出现在 `tool_registry_loaders_json` 中，后端会先构建同批 inner loader，再让 outer loader 继承 registry 并叠加自身 extra tools / disabled tools。
  - Settings-backed loader/provider factory 的 factory->factory forward reference 也继续复用 factory 依赖排序：outer factory 可以先于 inner file-backed factory 出现在 `tool_registry_loader_factories_json` / `tool_registry_provider_factories_json` 中，并且 outer factory 自身的 profile/disabled/extra_tools/overrides 会继续套用到 inner factory registry，真实 source/profile 组合不再依赖 factory JSON 声明顺序，也不会因 alias 层丢失 HTTP JSON 映射治理。
  - HTTP JSON `read()` body 错误诊断继续收紧：当显式 `read()` 返回坏 body 类型，且泛型 `__iter__` 只是空 metadata iterator 时，会保留 reader body type 诊断，不会被降级成 `empty JSON response`；空 `read()` / 空 chunked read 的后续 fallback 语义保持不变。
  - HTTP JSON `read()` 调用形态 fallback 继续补齐：当无参 `read()` 与 `read(amt)` 都只是 SDK 方法签名不适配，但 `.content` 等后续 source 明确为空时，会保持 `empty JSON response`，不会把调用形态 TypeError 误报成真实 body type `transport error`。
  - HTTP JSON body 属性 fallback 继续补齐：当 `.content` / `.body` / `.data` / `.text` 属性值本身不是 bytes/text 或可序列化 parsed JSON body 时，会继续尝试后续属性与 `.json()` parsed body；若 callable accessor 抛出真实运行时异常，仍保留 `transport error` 诊断，不会被 fallback 吞掉。
  - HTTP JSON body 属性错误诊断继续收紧：当显式 `.content` / `.body` / `.data` / `.text` body 属性是坏类型，且泛型 `__iter__` 只是空 metadata iterator 时，会保留 body type 诊断，不会被降级成 `empty JSON response`。
  - HTTP JSON parsed body fallback 继续补齐：当 `json()` / `.json` parsed body 存在但返回值不可序列化时，会继续尝试 response 本体 iterable；若 `json()` 本身抛出真实运行时异常，仍保持原始 `response json failed` 诊断。
  - HTTP JSON parsed body 错误诊断继续收紧：当显式 `json()` / `.json` parsed body 返回不可序列化对象，且泛型 `__iter__` 只是空 metadata iterator 时，会保留 parsed-body JSON serializable 诊断，不会被降级成 `empty JSON response`。
  - HTTP JSON `json()` 调用形态 fallback 继续补齐：当 `json()` 只是 SDK 方法签名不适配，但 `.text` 等后续 source 明确为空时，会保持 `empty JSON response`，不会把调用形态 TypeError 误报成 parsed-body `transport error`；真实 parsed body 类型错误仍保持 fatal 诊断。
  - HTTP JSON 空 `read()` fallback 继续补齐：当 `read()` 返回空 bytes/text 但 `.content` 或 `json()` 等后续 source 持有真实 body 时，会继续读取后续 source；若没有任何可用后续 body，仍保持原有 `empty JSON response` 契约。
  - HTTP JSON 默认空 `read()` 的分块 fallback 继续补齐：当无参 `read()` 返回空 bytes/text、但 `read(amt)` 才能读出真实 body 时，会先尝试分块读取；分块也不可用或为空时再继续后续 source。
  - HTTP JSON 空 chunked `read(amt)` fallback 继续补齐：当无参 `read()` 不适配、分块读取第一块即为空，但 `.content` 或 `json()` 等后续 source 持有真实 body 时，会继续尝试后续 source；最终没有可用 body 时仍保持 `empty JSON response`。
  - HTTP JSON chunked `read(amt)` 中途错误诊断继续收紧：分块读取尚未产出真实 chunk 时仍可回退后续 source；一旦已经读到真实 chunk，后续坏 chunk 类型或上游 read 异常会保持 fatal `transport error`，不会被 `.content` 等 fallback 掩盖。
  - HTTP JSON streaming iterator 中途错误诊断继续收紧：`iter_bytes()` / `iter_content()` / `iter_text()` / `iter_lines()` 在产出真实 chunk 前的运行时打开失败可继续回退 `json()` 等后续 source；一旦已经产出真实 chunk，后续迭代异常会保持 fatal `transport error`，不会被 parsed-body fallback 掩盖。
  - HTTP JSON streaming iterator 方法调用失败 fallback 继续补齐：当首个 `iter_*` 方法在返回 iterator 前就因 SDK stream 工厂不可用而抛运行时异常时，会继续尝试后续 iterator 方法或 `json()` parsed body；若最终没有任何可用 source，仍保留原 iterator 失败诊断。
  - HTTP JSON response 本体 iterable 初始失败诊断继续收紧：当 `iter_*` 已确认为空流，而泛型 `__iter__` 在首个 chunk 前因 SDK metadata iterator 失败时，不会把既有空响应诊断升级成 `transport error`；只有已经产出真实 chunk 后的本体迭代失败才继续 fatal。
  - HTTP JSON 空 `iter_*` fallback 继续补齐：当首个 streaming iterator 为空或只有 `None` keep-alive chunk 时，会继续尝试后续 iterator、`json()` 与 response 本体 iterable；最终没有可用 body 时仍保持 `empty JSON response`。
  - HTTP JSON 空 body 属性 fallback 继续补齐：当 `.content` / `.body` / `.data` / `.text` 属性存在但值为空 bytes/text 时，会继续尝试后续属性、`json()` 与 response iterable；最终没有可用 body 时仍保持 `empty JSON response`。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1680/1680`）
  ...
  - `git diff --check` 通过
- 本轮收尾验证：backend/full-stack e2e 阻塞已解除，Docker postgres/chroma 已通过提权确认运行；同一受控命令生命周期内启动 backend 后，`bash scripts/ci_run_backend_e2e.sh --phase main --base-url http://127.0.0.1:8000` 通过，覆盖 baseline、main path、export consistency 与 cancel-timeout skip-timeout；完整 Chromium 前端 e2e 也在同一 backend 生命周期内通过（`47/47`）。本轮继续补齐 backend/frontend e2e tooling 的 cwd 与 shell-quote 边界：backend boot 脚本从非仓库根目录调用时会使用 repo root 下的 helper 与 backend app-dir，并把相对 log/pid 路径解析到调用目录；backend/frontend runner 的 base URL、log dir 与 frontend dir 也会先 shell-quote；e2e finalize diagnostics/stage 链与 artifact stage guard 会用 repo root 下的 flow/pipeline/stage/guard helper；artifact list 内的 repo-root 相对条目、frontend failure index 与 frontend export diagnostics 的默认 `frontend/test-results` 也会解析到 repo root；diagnostics overview 对损坏 JSON 输入降级为 unavailable，diag guard 对损坏 JSON 输入直接 fail，避免把坏诊断误汇总为 PASS；真实 HTTP JSON provider search 的 `total_count/count/totalCount/totalResults/documentsTotal/documentsCount/documentCount/total_documents/totalDocuments/doc_count/docCount/docs_count/docsCount/records_total/recordsTotal/record_total/recordTotal/records_count/recordsCount/record_count/recordCount/total_records/totalRecords` 会归一为 `documents_total`，search hit 类 `hitCount/hitsCount/total_hits/totalHits/total_matches/totalMatches/resultsCount/matchesCount` 会归一为 `hit_count`，`data/records` 列表会在 retrieval 语义下推断 `documents_total`，并可按投影补齐 `hit_count`；`result_fields` 若保留上游 `totalCount`/`hitCount` 命名，也会借 registration 语义补齐 `documents_total`/`hit_count`；当 `items`/`documents` 列表被显式投影为 `hit_count` 时，也会补齐命中数且不扩大未声明 `hit_count` 的 raw output，即使真实响应未自带 `tool_kind`/`knowledge_base_id` 也会借 registration 语义补齐计数，RAG follow-up 会从 `documentText/pageContent/chunkText/passage` 等正文别名抽取 chunks，provider calc 的对象 `value/answer/computedValue` 会归一为 `result`，即使真实响应未自带 `tool_kind` 也会借 registration 语义补齐 `value/answer/computedValue` 结构化结果，`requestId/trace_id/correlationId` 等 request id 别名会安全归一为 `request_id`，确保 preview/output/summary 连续。验证新增 `-k total_count_camel_case`、`-k computed_value`、`-k documents_total_camel_case`、`-k hit_count_camel_case`、`-k data_camel_case_text_fields`、`-k records_text_aliases`、`-k data_count_from_registration_semantics`、`-k records_count_from_registration_semantics`、`-k answer_from_registration_semantics`、`-k computed_value_from_registration_semantics`、`-k value_from_scoped_object_registration_semantics`、`-k hit_count_from_data_registration_semantics`、`-k total_count_result_field_from_registration_semantics`、`-k hit_count_result_field_from_registration_semantics`、`-k hit_count_from_items_result_field_registration_semantics`、`-k total_hits_result_field_as_hit_count`、`-k total_matches_result_field_as_hit_count`、`-k total_records_result_field_as_documents_total`、`-k records_total_alias`、`-k total_documents_result_field_as_documents_total`、`-k doc_count_alias`、`-k custom_real_search_tool`、上一轮 provider alias 红测、`-k provider_source`、`-k http_json`、`bash scripts/test_ci_e2e_tooling.sh`、完整 backend slice（`1680/1680`）、前端 node（`68/68`）与 frontend build 均通过。
