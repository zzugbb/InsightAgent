...
- `backend/scripts/test_tool_runtime_slice.py` 第一阶段拆分已完成：原入口继续保留，provider/source、planner、settings/registry、http_json mapping、route/model_dump 主题已拆到 `backend/scripts/tool_runtime_slice/`。
...
2. `pre-flight cleanup`：继续拆分 `backend/scripts/test_tool_runtime_slice.py` 剩余 trace/export/sanitization 大块，保持 `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 入口不变。
...
- `scripts/tool_runtime_slice/`：`test_tool_runtime_slice.py` 的主题 mixin 包，当前承接 provider/source、planner、settings/registry、http_json mapping 与 route/model_dump 测试
