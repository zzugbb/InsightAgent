...
- 当前主线：`product-ux-polish` 下一阶段约 60% 推进中；Knowledge Governance 空态已可直达并聚焦 Runtime Debug RAG 写入入口，后端 shared RAG 权限、任务/审计/RAG API、SSE、trace 与 export payload 不变。
...
1. 当前推进 `product-ux-polish` 下一阶段；Knowledge Governance 到 Runtime Debug RAG 写入入口的直达聚焦已落地，本轮无后端 API 改动。
