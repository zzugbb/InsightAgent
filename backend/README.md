...
- 最近已对齐到代码的高信号能力：
  ...
  - Provider/source inline shorthand 继续复用同一套 source/profile 上下文：`tool_registry_provider_sources_json` 中 source 若在根级直接声明 real tool shorthand，并同时声明 `profile` / disabled / overrides 等 adapter 控制字段，根级工具会并入 source 级 `extra_tools`，不会因为给 source 增加 profile 而从 selected registry 消失；真实 HTTP JSON search 仍会按 selected source/profile 渲染 header/query/body。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1629/1629`）
