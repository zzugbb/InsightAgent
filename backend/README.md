...
- backend queue e2e phase：`TASK_QUEUE_MAX_CONCURRENT=1` backend 上 `bash scripts/ci_run_backend_e2e.sh --phase queue --base-url http://127.0.0.1:8011 --log-dir /tmp` 最近 fresh 通过，覆盖 queued cancel、safe wait_position、settings 全局/当前用户 active/waiting/available 安全计数、current session diagnostics 与 followup completion；脚本 helper 现额外校验 queued snapshot 结构一致性、scope available slots 字段依赖、scope active/waiting 计数上界、整数型 settings 计数/治理限额、布尔型 settings 状态/治理标记、非负治理限额、governance 必填字段与 settings `pressure_state` 枚举值
