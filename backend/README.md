...
- `provider-tool-expansion` 与 `ci-release-engineering` 均已 100% 封板；当前主线为 `production-runtime-hardening`，进度约 45%。
...
- SSE `error.diagnostic` 与 provider call/selection `task_failed` 审计 detail 已对齐低敏 `reason` 枚举，便于运行态错误按 network/auth/rate_limit/upstream/timeout/cancelled/config 等原因定位。
...
- Full slice：`backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`，`1987/1987` 通过。
- Targeted：`production_reliability 39/39`、`reconnect 8/8`、registry/http_json/provider/runtime/trace/export/usage 通过。
...
1. 当前主线：`production-runtime-hardening`，继续收敛 provider registry 运行态诊断、远端 provider 失败可观测性与发布后回归策略；当前已补齐 SSE error、provider call failure audit 与 provider selection failure audit 的低敏分类和 reason 枚举。
...
- remote provider 错误在 SSE `error` 中保持结构化 `code / fatal / retryable / detail / status_code`，并在 SSE 与 provider call/selection failure audit 中追加低敏 `diagnostic.category/reason/recoverability/http_status_family/has_detail`。
