...
- `real-tool-execution`、`queue-and-concurrency-lite` 与 `concurrency-fairness-policy` 均已封板；当前主线进入 `registry-governance`，进度约 `79%`；默认 settings 语义保持不变：provider/model/api_key 完整时自动走 `remote`，否则回退 canonical `mock`。
...
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`：`1843/1843` 通过
...
2. 当前主线：`registry-governance`，聚焦 selected source、settings/preflight、tool details、per-tool diagnostics、runtime semantic、trace/export/audit/SSE 的治理语义；本轮继续把 session export 的 session/task governance 接到同一 provider source alias map，确保导出 JSON/Markdown 中碰撞 source 编号一致。
