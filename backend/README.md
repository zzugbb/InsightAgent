...
- 当前主线已进入 `rag-product-experience`，进度约 12%；前端知识库治理表消费现有 `document_versions` 展开版本明细，并派生 source/document 文档组摘要；后端 RAG status/list、ingest/query、SSE、trace 与 export 外部响应 shape 保持不变。
...
- frontend node tests：knowledge base governance modal utils / workbench utils / audit logs modal utils / task detail utils / stream store utils / model settings utils，当前 `110/110` 通过
...
2. 当前主线：`rag-product-experience` 已启动，进度约 12%；已完成知识库版本明细展开与 source/document 文档组摘要，后端外部 RAG 契约保持稳定。
