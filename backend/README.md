...
- 当前主线：`test-maintainability-hardening`，进度 95%；四个大测试主题已拆为双分片，测试主题规模门禁为 2500 行，选择性运行支持零匹配诊断、匹配测试预览与六个维护选择器，后端运行时契约不变。
...
1. `test-maintainability-hardening` 当前 95%：执行封板核对与活跃文档收敛，确认原 mixin import 和测试发现入口稳定。
