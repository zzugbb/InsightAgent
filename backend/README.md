...
- 本轮无后端运行时变更；Task Center 与任务详情状态展示已对齐 normalized 任务语义，后端任务 API、SSE、trace 与 export shape 不变。
...
1. 当前状态：`product-ux-polish` 约 67%，后端侧保持契约稳定，配合前端 trace 回放与任务状态可读性推进。
...
- 任务详情页可通过兼容 URL 参数 `trace_semantic` 回放语义 Trace；前端语义切换与 normalized 状态展示均不改变后端任务、trace 或 export payload。
