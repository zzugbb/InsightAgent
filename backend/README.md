...
- 当前主线：`next-major-upgrade-readiness` 80%；Next `16.3.5` / React `19.2.8` 联合升级保持稳定，前端 `react-hooks/refs` 已全局启用，后端运行时及 SSE / trace / export 外部契约未修改。
...
- Frontend：Next 16 readiness contract `8/8`，真实预检 `ready_with_actions`、0 blocker、2 actions；runtime dependency contract `8/8`；release gate 内置 node tests `166/166`；`react-hooks/refs` 全局启用，ESLint 兼容锁定 `9.39.5`；默认 Turbopack build 与显式 webpack fallback 均通过。
...
1. `next-major-upgrade-readiness` 当前 80%；Next 16 / React 19.2 联合升级、双构建门禁、零漏洞审计、`refs` 规则迁移与真实 e2e 已确认无项目级 blocker。
2. 下一切片聚焦前端 `set-state-in-effect` 主题迁移与 ESLint 10 插件兼容性；后端继续保持 FastAPI 精确 pin、full slice 与 SSE / trace / export 外部契约稳定。
