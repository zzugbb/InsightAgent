...
- 最近已对齐到代码的高信号能力：
  ...
  - Settings-backed loader/provider factory 的 factory->factory forward reference 也继续复用 factory 依赖排序：outer factory 可以先于 inner file-backed factory 出现在 `tool_registry_loader_factories_json` / `tool_registry_provider_factories_json` 中，真实 source/profile 组合不再依赖 factory JSON 声明顺序。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1640/1640`）
