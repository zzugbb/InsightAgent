...
- 最近已对齐到代码的高信号能力：
  ...
  - Provider/source/file-backed registry wrapper 继续复用窄归一化：`tool_registry_*_json` settings 字段若是 `UserString`，loader/provider/source/factory artifacts 会按 JSON object 解析；file manifest、plain registry file 与 loader adapter 中的 `UserDict/UserList/UserString` 会先归一再处理 profile、disabled tools、registry_files/dirs/sources、extra_tools 与 overrides；provider adapter 通过 `registry_file` 构建真实工具时会使用当前 provider source 的模板上下文，provider/loader factory 与命名 provider/loader 背后的 `registry_file` 也会按 source settings 重建，避免 `$tool_registry_provider_source` 被全局 selected source 串值。
