...
- `concurrency-fairness-policy` 已启动：`app/services/task_queue_service.py` 的执行槽位新增可选 `TASK_QUEUE_MAX_CONCURRENT_PER_USER` 与 `TASK_QUEUE_MAX_CONCURRENT_PER_SESSION`，默认 `0` 关闭，开启后可限制单用户/单会话并发且不阻塞其他用户/会话；等待队列现在保持 capacity-aware oldest eligible FIFO，槽位紧张时新任务不会抢在已可执行的旧等待任务前面，空槽足够时仍可并行填充；`GET /api/settings` 现暴露只读 `task_queue_diagnostics`，包含限额、fairness 开关、`waiting_policy` 与 capacity-aware FIFO 标记，backend queue e2e 脚本会校验低并发设置诊断并复用已被 slice 覆盖的安全 queue snapshot helper，不改变外部 SSE / trace / export shape。
...
1. `concurrency-fairness-policy`：当前主线；已完成可选按用户/按 session 并发执行槽位上限、capacity-aware oldest eligible FIFO 防插队、settings `task_queue_diagnostics` 等待策略诊断与 backend queue e2e-like 诊断断言、安全 queue snapshot helper 覆盖，下一步按风险补真实低并发 e2e 复验或更细公平策略。
...
`GET /api/settings` 的响应包含只读 `task_queue_diagnostics`，用于观察当前单进程队列的 `max_concurrent`、可选 per-user/per-session 上限、fairness 开关、`waiting_policy`、capacity-aware FIFO 标记与 poll interval；该字段不参与用户设置保存，也不改变 SSE / trace / export payload。
- `POST /api/sessions`
