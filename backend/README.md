...
- `real-tool-execution`、`queue-and-concurrency-lite` 与 `concurrency-fairness-policy` 均已封板；当前主线进入 `registry-governance`，进度约 `57%`；默认 settings 语义保持不变：provider/model/api_key 完整时自动走 `remote`，否则回退 canonical `mock`。
...
2. 当前主线：`registry-governance`，聚焦 selected source、settings/preflight、tool details、per-tool diagnostics、runtime semantic、trace/export/audit/SSE 的治理语义；本轮把唯一脱敏 provider source alias 解析继续下沉到 task list/count 与 usage dashboard service filter，保持内部 raw source 保存、route 入口与直接 service 调用语义一致，并继续兼容普通 source 与服务层 filter normalizer。
