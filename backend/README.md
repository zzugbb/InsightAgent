...
- 最近已对齐到代码的高信号能力：
  ...
  - `http_json` URL、header/query value shape、`json_body` 严格 JSON 与 response mapping path 现在也会提前治理：URL 必须是绝对 `http(s)` 地址、不得携带 userinfo credentials，显式端口必须合法，且不得包含原始空格/控制字符；header name 必须是合法 HTTP token，header value 不允许 CR/LF 注入或其他 HTTP 控制字符，query 参数只接受 string/number/boolean 或这些值的列表；`json_body` 必须能编码成严格 JSON，静态配置里的 `NaN` / `Infinity` 或运行时模板渲染出的不可序列化对象都会按具体路径进入 diagnostics / fail-fast。settings/source 能静态渲染出的 URL/header/query/body 模板也会按渲染后结果进入 diagnostics；如果运行时模板把 URL 渲染成相对地址、带 credentials、非法端口、含控制字符的地址，或把 header/query/body 渲染成非法协议形态，runner 会在发请求前 fail-fast；`execution_summary.url_origin` 与 `url_path` 也会做安全化处理，避免非法端口裸抛、半可信 origin 或 path 中的 `token=...` / `api_key/...` 泄漏进 trace/export，`response_path` / `result_fields.*` 只接受 dot field 与数字索引语法，避免 `[-1]` / `[]` 一类非法 bracket 被部分解析成假路径。
  - `http_json` 上游 HTTP 错误、连接/传输错误、2xx 非 JSON 响应或非法 UTF-8 响应体现在会归一成稳定可读的 runtime error：错误信息包含 HTTP status、脱敏限长后的 reason、`transport error` 或 `invalid JSON response` 与敏感键/敏感 assignment 脱敏后的有限长度 preview，便于 trace/observation/export 排障，而不再只透出 urllib、OSError、JSON parser、codec 的泛化异常字符串、HTTP reason 里的敏感 assignment，或 JSON error body 普通字符串值里的 `token=...` 原文。
  ...
  - 请求侧 diagnostics 的路径现在也会做敏感字段名脱敏：query/body 协议错误、静态 unsupported runtime template variable 与运行时 missing template variable 如果落在 `query_params.api_key`、`json_body.access_token`、`json_body.filters[0].client_secret` 这类字段上，会统一显示为 `[redacted]`，避免 source/settings diagnostics、runtime error、trace/export 通过 path 泄漏敏感请求字段名。
  ...
  - `http_json` 结果字段元数据现在也会复用这条治理链：`execution_summary.result_field_names` 会对 `access_token` / `api_key` 等敏感字段名做脱敏；显式 `result_preview_keys` / `result_output_keys` 会过滤敏感 key，且显式非空列表如果只声明了敏感 key，不会再回退到模板默认 key，避免 registry settings、tool_start、trace 或 export 通过“字段名/投影 key”重新泄漏敏感语义。
