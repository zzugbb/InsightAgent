...
- `real-tool-execution`、`queue-and-concurrency-lite` 与 `concurrency-fairness-policy` 均已封板；当前主线进入 `registry-governance`，进度约 `100%`；默认 settings 语义保持不变：provider/model/api_key 完整时自动走 `remote`，否则回退 canonical `mock`。
...
2. 当前主线：`registry-governance` 已基本收口，聚焦 selected source、settings/preflight、tool details、per-tool diagnostics、runtime semantic、trace/export/audit/SSE 的治理语义；本轮继续把 settings runtime_artifacts diagnostics 构建入口纳入共享 alias map，确保 provider_sources、diagnostics_runtime 与 audit_event 里的碰撞 source 编号一致。
