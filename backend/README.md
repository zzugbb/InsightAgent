...
- 当前主线：`release-observability-polish`，进度约 44%；release readiness matrix 已补发布摘要、回滚判定与 artifact retention 检查，backend/frontend/release-gate artifacts 显式保留 14 天，release gate Markdown/JSON summary 已输出 summary kind、schema version、service-required 标识、步骤聚合计数与失败步骤标签。
