...
- 当前主线：`runtime-dependency-modernization` 100%；后端运行时及 SSE / trace / export 外部契约不变，可进入下一候选主线。
...
- Frontend：runtime dependency contract `6/6`；release gate 内置 node tests `156/156`；Next / eslint-config-next 为 `15.5.25`，ESLint 为 `9.39.5`，当前版本范围内可修复的审计依赖已清除。联网 `npm audit` 剩 2 项（1 moderate / 1 high / 0 critical），均受 Next 15.5 固定 PostCSS 约束。
- E2E/CI：本轮 service-backed Chromium `59 passed / 1 skipped`，SSE、trace、export、RAG、鉴权、恢复与远端错误映射路径均通过；tooling 回归由 release gate 覆盖。
...
2. 下一候选主线为 `next-major-upgrade-readiness`；后端继续保持 FastAPI 精确 pin、full slice 与 SSE / trace / export 外部契约稳定。
