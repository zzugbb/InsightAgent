...
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1698/1698`）
  ...
  - `git diff --check` 通过
- 本轮收尾验证：继续收口 HTTP JSON provider search 的 Azure/OData 与 organic search 输出；当 `result_fields.documents` 投到 `{@odata.count, value:[...]}` 或真实搜索根 payload 返回 `organic_results[] + search_information.total_results + search_metadata.id` 时，runtime 会补齐 `documents_total`、安全 `chunks` 与 `request_id`，继续贯通 preview/output/result-summary，同时保持无 registration 的旧 `matches` trace/export fallback 不额外扩展 `documents_total`。同步验证新增红测 `-k azure_odata_documents`、`-k organic_results_documents`，上一轮 ES/GraphQL/metadata/chunks 回归，`-k build_tool_result_summary`、`-k response_path_list`、`-k build_tool_rag_followup`、`-k http_json`、`-k tool_registry`、`-k provider_source` 与完整 backend slice（`1702/1702`）均通过；同一受控生命周期内后端 main phase e2e 通过，Chromium full e2e 首轮并发 `45/47`、两个失败用例单 worker 精确复跑 `2/2` 通过。
