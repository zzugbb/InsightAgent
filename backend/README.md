...
- 本轮无后端运行时变更；前端任务详情页与 Workbench Inspector 语义筛选会清理旧搜索/kind 干扰，Task Center failure source chips、状态筛选、状态展示与失败样式已对齐 normalized 任务语义，后端 SSE / trace / export shape 不变。
...
1. 当前状态：`product-ux-polish` 约 63%，后端侧暂保持契约稳定，配合前端 trace 回放与失败诊断可读性推进。
...
- 任务详情页可通过兼容 URL 参数 `trace_semantic` 回放语义 Trace；前端语义切换会同步 URL 并清理本地 search/kind 筛选，该能力不改变后端 trace/export payload。
- Workbench Inspector 语义筛选清理旧 search/kind 干扰属于前端本地状态变更，不改变 SSE、trace/delta、任务 API 或 export payload。
- Task Center failure source drilldown、重复点击取消、观测维度切换清理与 normalized 状态筛选/展示/样式均为前端本地语义，不改变任务列表 API、后端 trace 或 export shape。
