...
- `real-tool-execution`、`queue-and-concurrency-lite` 与 `concurrency-fairness-policy` 均已封板；当前主线进入 `registry-governance`，进度约 `94%`。
...
- 本轮继续收口 standalone service action / service actions collection 的 provider source，让 trace_step、trace_event 与 kwargs audit detail 在独立 `to_dict()` 出口也共用 alias map，避免同 base source 编号漂移。
