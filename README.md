...
2. 当前主线：`observability-experience` 已推进到约 92%，已完成任务失败线索摘要、来源分类、Task Center 观测筛选与 registry profile/provider source 本地筛选生效、Task Center 当前可见任务失败来源诊断分组、稳定失败码文本纳入 Failure 语义、任务中心/任务详情复用、Usage Dashboard / Audit Logs 到任务详情的回放入口、TaskResponse/audit failure hint 恢复、Task Center 列表 audit failure hint 批量回放、远端错误码可读映射、任务详情失败轨迹快捷定位、跨视图 Failure URL 预设直达、Usage Dashboard top tasks 失败摘要展示、Audit Logs 失败 hint/source/code/message 可读详情、Audit Logs 服务端 keyword 过滤，以及 Trace/Inspector/任务详情失败语义过滤与 Failure 统计。
3. 下一步继续围绕 trace 筛选效率、失败诊断 drilldown 和封板前完整回归补红测推进。
