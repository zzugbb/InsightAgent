...
- 当前主线已进入 `observability-experience`，进度约 48%：任务快照会从 trace diagnostics 中提取失败线索并标注来源（SSE error / tool error / trace content / persisted trace），Task Center 可展示/搜索该线索与来源，任务详情页会展示同一失败摘要；Usage Dashboard 任务榜会显示失败摘要并可直接打开任务详情回放，Audit Logs 任务列也可直接进入同一回放页；Trace 语义统计已新增 Failure 维度，Inspector 与任务详情页可按失败语义过滤轨迹。
...
2. 当前主线：`observability-experience` 已推进到约 48%，已完成任务失败线索摘要、来源分类、任务中心/任务详情复用、Usage Dashboard / Audit Logs 到任务详情的回放入口、Usage Dashboard top tasks 失败摘要展示，以及 Trace/Inspector/任务详情失败语义过滤与 Failure 统计。
3. 下一步继续围绕 Task Center 失败定位、任务详情异常聚合、audit 详情可读性、trace 筛选效率和回放效率补红测推进。
