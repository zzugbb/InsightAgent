...
- 当前主线已进入 `observability-experience`，进度约 90%：任务快照会从 trace diagnostics、TaskResponse failure fields 与 task_failed audit event 中提取失败线索并标注来源（SSE error / tool error / trace content / persisted trace），Task Center/任务详情/Usage Dashboard 会统一把稳定错误码映射为可读失败说明；Task Center 可从任务列表批量回放 audit failure hint，展示/搜索该线索与来源，并支持按 Needs attention / Failed status / Failure hint / Failure trace 做观测筛选，且会按当前筛选后的可见任务汇总失败来源诊断分组；Task Center 的 registry profile/provider source 筛选已进入本地任务快照过滤；任务详情页会展示同一失败摘要，可从失败线索快捷定位 Failure 轨迹，并在原始 trace 缺少错误 step 时合成本地 failure 回放节点；Trace Failure 语义已覆盖 rate limited、unauthorized、permission denied、connection refused 等稳定失败码文本；Usage Dashboard 任务榜会显示失败摘要并可直接打开任务详情回放，Audit Logs 任务列也可直接进入同一回放页；Audit Logs keyword 已进入服务端过滤，分页、total、导出与 e2e 搜索口径一致；Trace 语义统计已新增 Failure 维度，Inspector 与任务详情页可按失败语义过滤轨迹。
...
- frontend targeted TS：本轮涉及的 workbench utils、Task Center 与 i18n 文案通过 targeted `tsc`
...
2. 当前主线：`observability-experience` 已推进到约 90%，已完成任务失败线索摘要、来源分类、Task Center 观测筛选与 registry profile/provider source 本地筛选生效、Task Center 当前可见任务失败来源诊断分组、稳定失败码文本纳入 Failure 语义、任务中心/任务详情复用、Usage Dashboard / Audit Logs 到任务详情的回放入口、TaskResponse/audit failure hint 恢复、Task Center 列表 audit failure hint 批量回放、远端错误码可读映射、任务详情失败轨迹快捷定位、Usage Dashboard top tasks 失败摘要展示、Audit Logs 失败 hint/source/code/message 可读详情、Audit Logs 服务端 keyword 过滤，以及 Trace/Inspector/任务详情失败语义过滤与 Failure 统计。
3. 下一步继续围绕 trace 筛选效率、跨视图回放效率和失败诊断 drilldown 补红测推进。
