...
- 当前主线为 `production-runtime-hardening`，进度约 35%；SSE `error.diagnostic` 与 provider `task_failed` 审计 detail 已对齐低敏 `reason` 枚举，不改变旧字段。
...
- Backend：full slice `1986/1986`；module boundary `4/4`；targeted production_reliability `38/38`、reconnect `8/8` 通过。
