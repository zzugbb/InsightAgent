...
- backend：
  ...
  - CI artifact PR 路径感知扩展（2026-05-12）：新增 `scripts/ci_resolve_artifact_stage_path_level.sh` 与 `scripts/ci_collect_changed_files.sh`（配套 `scripts/test_ci_collect_changed_files.sh`），将 artifact guard 的 PR 升级条件从“仅看 PR”收紧为“PR 且命中关键目录/文件”；backend/frontend workflow 现统一通过脚本生成变更文件清单，`actions/checkout` 同步切到 `fetch-depth: 0`，当 PR base diff 不可解析时会保守回退到关键路径标记，避免浅克隆或缺失 base SHA 导致 guard 误判
