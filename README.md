...
| 阶段 5+ 产品化 | 进行中 | `full-data-auth` 首版已落地（JWT/多用户隔离/凭证加密），PG 迁移与更完整治理待后续 |
...
- frontend：
  ...
  - Auth 交互细化：登录区标题文案改为“登录账号”；提交按钮禁用态提升对比度；退出按钮右移避免与右侧面板折叠控件重叠
  - Auth 交互细化（继续）：移除“当前用户：未登录”提示；注册邮箱格式前置校验并给出友好错误；退出按钮改为顶部居中并适配明暗主题
...
1. `full-data-auth`：已完成首版（用户体系 + JWT/API 鉴权 + 凭证加密存储，当前基于 SQLite），后续补 PostgreSQL 迁移与更细粒度权限。
