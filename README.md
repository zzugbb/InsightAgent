...
- `task-cancel-timeout` 后端 e2e（cancel + timeout）与 CI 首版已落地，`running-task-recovery` 前端首版与可视化提示已落地，`rag-kb-governance-lite`、`usage-dashboard-lite`、`audit-event-expansion` 与 `provider-usage-alignment` 已完成首版并补齐来源趋势联动；前端可视化回归 CI 首版也已接入，下一步推进导出/取消恢复交互场景扩展回归。
