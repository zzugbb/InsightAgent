...
- frontend：
  ...
  - W3 增量：mock 工具链路支持可复现的错误+重试语义（`[mock-tool-error]`），`tool_end` / `error` / `trace.meta.tool` 已携带 `retry_count/error`
...
- 真实工具调用与错误/重试语义（W3）
- W3 首版已打通 `tool_start/tool_end` 事件链路（mock 语义），并补齐 mock 错误重试语义到 SSE/Trace
- 真实 RAG ingest/检索（W4）
