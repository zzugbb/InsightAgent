...
- 最近代码主线已经打通：
  ...
  - `http_json` 的 URL、header/query value shape、请求 JSON header、`json_body` 严格 JSON 与 response mapping path 也继续收紧：URL 必须是绝对 `http(s)` 地址、不得携带 userinfo credentials，显式端口必须合法，不得包含原始空格/控制字符或 fragment；header name 必须是合法 HTTP token，header value 不允许 CR/LF 注入或其他 HTTP 控制字符，header 名不得以大小写变体重复，显式 `Content-Type` 在声明 `json_body` 时必须是 `application/json` 或 `+json`，若声明 charset 则必须是 UTF-8，显式 `Accept` 必须允许 JSON 且对应 media range 的 `q` 必须是有限的 `(0, 1]`；query 参数名不得包含空白/控制字符或 `= & ? #` 这类 query 结构分隔符，query 值只接受标量或标量列表；`json_body` 必须能编码成严格 JSON，`NaN` / `Infinity` 这类非有限数或运行时模板渲染出的不可序列化对象都会按 `json_body.score` / `json_body.payload` 这类路径提前诊断。settings/source 能静态渲染出的 URL/header/query/body 模板也会按渲染后结果进入 diagnostics；运行时模板若把 URL 渲染成相对地址、带 credentials、非法端口、带 fragment、含控制字符的地址，或把 header/query/body 渲染成非法协议形态，都会在发请求前 fail-fast；`execution_summary.url_origin` 与 `url_path` 也会做安全化处理，避免非法端口裸抛、半可信 origin，或 path 中的 `token=...`、`api_key/...` 字段名和值泄漏进 trace/export，`response_path` / `result_fields.*` 只接受 dot field 与数字索引语法，避免非法 bracket 片段被部分解析成错误上游路径。
  - `http_json` 上游返回 HTTP 错误、连接/传输错误、非 2xx 响应对象、2xx 非 JSON 响应、显式非 JSON `Content-Type`、204/空成功体或非法 UTF-8 响应体时，runtime 现在会把稳定的 HTTP status、脱敏限长后的 reason、`transport error`、`invalid JSON response`、`empty JSON response`、`invalid JSON response charset` 或 `invalid JSON response content-type` 与有限 preview 一起落入错误信息；除 `HTTPError` 外，`status/code/status_code/getcode()` 这类 adapter 响应状态也会在 response mapping 前拦截，避免 302/503 等真实上游状态被伪装成成功或 fatal mapping error；response body 也会先把 `str/bytearray/memoryview` 归一成 bytes，不支持的 body 类型会稳定落成 `transport error`；`Content-Encoding: gzip/deflate` 会在 JSON 解析前解压，HTTPError 的压缩错误体也会走同一条 preview 链路，坏压缩体或暂不支持的 encoding 会稳定报错并带脱敏 body preview；`Content-Type` 里的 bytes-like header 会按文本解析，`application/json` 与 `+json` subtype 继续作为 JSON 响应接受，`charset=utf-16` 这类声明会用于 JSON body 解码，未知或敏感 charset 会安全失败；bytes 形态的 `text/html` 网关/登录页即使 body 形似 JSON 也不会再伪装成成功映射；JSON error body 的敏感 key、普通字符串值里的敏感 assignment、HTML/raw text preview、transport reason 与 bytes reason 里的 `token/api_key/secret=...` 现在会连标签和值一起显示为 `[redacted]`。
  ...
  - 对显式声明了 `response_path` 的 `http_json` real tool，当前 runner 也不再悄悄退回根 payload：如果上游响应里根本找不到这个路径，或者配置里给的是空白 `response_path`，都会直接按配置/协议错 fail-fast，且错误里的 path 会做敏感 assignment 脱敏，避免把坏掉的响应映射伪装成“工具还能跑，只是结果有点怪”或把敏感 path 片段带进回放。
  - 对显式声明了 `result_fields` 的 `http_json` real tool，如果所有字段映射都落空，当前 runner 也会直接 fail-fast，并把失败的映射项以有限数量摘要带出来；这样上游响应漂移或 registry mapping 写错时，不会再返回空结果对象伪装成成功执行，也不会让超长 mapping 列表撑爆 trace/export。
  - `http_json` 成功输出现在也会复用同一套敏感 payload 脱敏：无论是没有 `result_fields` 的根响应 / `response_path` scoped 响应，还是显式映射后的字段输出，`secret/token/api_key/password/...` key 与字符串里的敏感 assignment 都会在进入 preview/output/observation/export 前被替换为 `[redacted]`。
  - `http_json` 的结果元数据与响应映射诊断现在也会进入同一条敏感字段治理：`execution_summary.result_field_names` 与 `response_path` 会脱敏敏感字段名、assignment 和 path segment；显式配置的 `result_preview_keys` / `result_output_keys` 会过滤敏感 key，`result_fields.access_token`、`$.meta.api_key` 一类静态/运行时映射诊断会显示为 `[redacted]`，且当显式非空列表只剩敏感 key 时不会回退到模板默认 key，避免 settings/tool_start/trace/export 通过字段名、path 或投影策略重新暴露敏感语义。
  ...
  - task export markdown / markdown meta 这一层现在也已补上同类防回归覆盖：JSON-string safe output 会继续按 `effective_result_output_keys` 裁成安全 output，再交给 trace markdown 与 display helper 消费，不会把旁路字段重新写回导出内容。
  ...
  - session export markdown builder 现在也会继续把旧 `trace_preview.content_excerpt` 里的 `Label: {...}` 与 `Tool done: ... Preview: ... Output: ...` 归一成推断摘要，同时保留安全 preview/output 片段；旧会话导出 markdown 不会再单独退回裸 JSON 或泛化 `Tool done:` 文案。
  ...
  - 同一套保守 fallback 现在也继续覆盖只有 name-only label 的旧 real/provider calc：当 step meta、preview excerpt 或 observation 里只剩 `result + request_id`，且 label 是 `Hosted Math` / `Provider Math` 这类 real calc 名称时，final answer、tool observation、task/session preview、markdown export 与前端 workbench display 也会优先产出 `Calculated result = ... (request id ...)`，而不是退回 generic payload / `Tool done:` 文案。
...
- 当前最近一次已记录校验基线：
  - `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1037/1037`）
