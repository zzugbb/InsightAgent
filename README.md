...
- 最近代码主线已经打通：
  ...
  - `http_json` 上游返回 HTTP 错误或 2xx 非 JSON 响应时，runtime 现在会把稳定的 HTTP status/reason 或 `invalid JSON response` 与敏感键脱敏后的有限 body preview 一起落入错误信息；trace/observation/export 能看到可排障的 `HTTP 429 ... body: ...` / `invalid JSON response ... body: ...` 语义，而不是只剩 urllib/JSON parser 原始异常文案。
