...
- backend：
  ...
  - `remote` 模式改为严格失败：缺少 `api_key` 或 provider 未实现时，任务流返回明确错误码（不再静默回落 `mock`）
...
- frontend：
  ...
  - 审计页可读性优化：主视图改为分页表格展示，筛选区升级为双行下拉+输入框检索并统一控件尺寸，事件类型标签化；总条数下置表格左下角，操作区右对齐，并恢复行展开明细（展开内不再二次展开，主表保留会话/任务 ID）
