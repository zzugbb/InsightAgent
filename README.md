...
- backend：
  ...
  - CI artifact PR 路径感知扩展（2026-05-12）：新增 `scripts/ci_resolve_artifact_stage_path_level.sh`，将 artifact guard 的 PR 升级条件从“仅看 PR”收紧为“PR 且命中关键目录/文件”；backend/frontend workflow 现在都会根据变更文件清单决定是否把 PR 门禁升级到 `fail-on-empty`
