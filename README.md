...
- 最近代码主线已经打通：
  ...
  - 工具规划 wrapper 继续对齐真实执行器：provider 返回的 `tools/plan`、单个 plan item、`task_plan` 的 `planned_tool_names/labels/kinds`、trace action step 初始输入，以及 `normalize_tool_spec` 的 `input` 若来自 `UserDict/UserList/UserString`，会先归一成 plain payload 再规划、过滤 planner 工具、生成 steps 与写入 trace；普通文本仍不会被当成序列拆分。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1598/1598`）
