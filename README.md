...
- 当前主线已进入 `rag-product-experience`，进度约 44%：知识库治理表可展开查看同一知识库下的版本明细，并按 source/document 汇总文档组摘要，展示每个文档组的版本数与 chunk 总数；文档组支持删除该 source/document 下全部 chunks，后端新增 `rag_document_delete` 审计事件；Runtime Debug 的 RAG 查询结果会基于 metadata 展示 source/document/version/hash/chunk 召回来源摘要，并基于 distance 展示强/中/弱召回质量标签和“距离越低语义越接近”的解释；RAG ingest/query/status/list、外部 SSE / trace / export / e2e 契约保持稳定。
...
- frontend main path Chromium：`e2e/workbench-main-path.spec.ts:302`，`1/1` 通过，覆盖真实 RAG ingest/query 后的召回来源摘要、召回质量标签与 distance 解释
