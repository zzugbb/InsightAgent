...
- 最近代码主线已经打通：
  ...
  - HTTP JSON static diagnostics 继续对齐真实执行器：`headers`、`query_params`、`json_body` 的 direct typed value 与 supported runtime template 渲染出的 typed value 会先按 request runtime 的 JSON-compatible 规则归一化后再做 object/value/Content-Type/Accept/duplicate/template typo 诊断；typed adapter dump 出来的 reserved `settings_*` / `tool_registry_*` typo 也会提前进入 diagnostics。
  - HTTP JSON execution summary 继续对齐真实执行器：direct typed request mapping、`UserString` response_path 与 root template 渲染出的 typed `result_fields` 会先复用静态归一化，再生成 header/query/body count、response_path 与 result_field_names；通用 template render/missing/reference 扫描也会把 `UserString` 当文本处理，避免 settings/preflight 摘要侧路递归或缺字段。
  - HTTP JSON control field string wrapper 继续对齐真实执行器：`url`、`method`、`timeout_ms` 里的 `UserString` 会按明确 string-like value 处理，`method` runtime template 在 execution summary 里也会按静态上下文渲染后展示；普通 object 仍不会被随意 stringify。
  - HTTP JSON execution kind string wrapper 继续对齐真实执行器：`execution.kind=UserString("http_json")` 会在 validation、runner dispatch 与 execution_summary 中按真实 `http_json` 处理；其他普通 object 仍保持缺失/unsupported 语义。
  - HTTP JSON runtime semantic wrapper 继续对齐真实执行器：半迁移 registration 里的 `execution_summary` / `execution_diagnostics` 若携带 `UserString` key/value/list item，会先按明确 string-like value 归一化再脱敏进入 `tool_start`、action meta、trace/export；`kind` / `runtime_semantic_kind` 的 `UserString` 也会保留现有 trace `semantic_kind` 契约并补齐 `semantic_family`。
  - HTTP JSON result projection wrapper 继续对齐真实执行器：显式 `result_preview_keys` / `result_output_keys` 若来自 `UserList` / `UserString`，仍会被视为显式投影配置并过滤敏感字段；真实输出里的 planner `steps`、calc `expression`、retrieval `knowledge_base_id` 与 `request_id` wrapper 会进入 result summary、observation、trace/export 摘要。
  - HTTP JSON registry spec wrapper 继续对齐真实执行器：direct `extra_tools`、`overrides` 与 provider adapter spec 如果来自 `UserDict/UserList/UserString`，会先归一成 plain registry spec 再构建注册、diagnostics 与 selected source provider；普通不可序列化对象仍不会被静默接入。
  - 工具规划 wrapper 继续对齐真实执行器：provider 返回的 `tools/plan`、单个 plan item、`task_plan` 的 `planned_tool_names/labels/kinds/execution_kinds`、trace action step 初始输入，以及 `normalize_tool_spec` 的 `input` 若来自 `UserDict/UserList/UserString`，会先归一成 plain payload 再规划、过滤 planner 工具、生成 steps 与写入 trace；普通文本仍不会被当成序列拆分。
  - Provider/source/file-backed registry wrapper 继续对齐真实执行器：settings JSON 字段若来自 `UserString`，loader/provider/source/factory artifact 会按 JSON object 解析；file manifest、plain registry file、loader adapter 和 registry composition 中的 `UserDict/UserList/UserString` 会先归一再处理 profile、disabled tools、registry_files/dirs/sources、extra_tools 与 overrides；provider adapter 通过 `registry_file` 构建真实工具时会使用当前 provider source 的模板上下文，provider/loader factory 与命名 provider/loader 背后的 `registry_file` 也会按 source settings 重建；manifest `registry_sources` 引用 settings-backed source/provider/loader/factory 时，相关 `registry_file` 相对路径会按当前 manifest 所在目录解析；已访问 registry file 会继续防循环，direct source 自引用、命名 provider/loader/factory 背后的同文件引用，以及 `source -> provider -> loader/loader_factory`、`source -> provider_factory`、`source -> source` 这类传递链都会折算为 `skipped_registry_sources` 而不是误报 missing source；如果命名 loader/provider/factory 或 source->source 链路里的 inner source 因缺失 registry file 无法构建，已收集的 missing/skipped/invalid diagnostics 仍会保留到 artifact map 并并回 root source diagnostics，同时保留 child source 自己的模板上下文，避免 `$tool_registry_provider_source` 被全局 selected source 串值；source spec 的 effective profile 也会进入 file-backed manifest、inline extra_tools、`source -> named provider -> registry_file`、`source -> named loader -> registry_file`、`source -> named provider -> named loader -> registry_file` 以及 provider/loader factory 的 HTTP JSON 请求和 preflight summary。本轮 real search 组合回归继续把 selected source/profile 锁进 POST headers、query_params、json_body，并覆盖 plan item 的 preview/output/summary 与 named loader/factory 的 preflight field counts；file-backed real calc 也已覆盖 provider planner、plan item POST request、preview/output/result_summary/observation、trace preview/session export response 回放、trace detail/delta 与 task export JSON 的结构化 result_summary 回填、preflight summary、脱敏 diagnostics 以及 settings validate tool details。
...
- 当前最近一次已记录校验基线：
- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`1599/1599`）
