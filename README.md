...
- `task-cancel-timeout` 后端 e2e（cancel + timeout）与 CI 首版已落地，`running-task-recovery` 前端首版与可视化提示已落地，`rag-kb-governance-lite`、`usage-dashboard-lite`、`audit-event-expansion` 与 `provider-usage-alignment` 已完成首版；下一步推进前端可视化回归（CI）与 usage dashboard 的来源维度回归校验。
