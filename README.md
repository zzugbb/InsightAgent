...
- 最近代码主线已经打通：
  ...
  - `http_json` 的 URL、header/query value shape、`json_body` 严格 JSON 与 response mapping path 也继续收紧：URL 必须是绝对 `http(s)` 地址、不得携带 userinfo credentials，显式端口必须合法，且不得包含原始空格/控制字符；header name 必须是合法 HTTP token，header value 不允许 CR/LF 注入或其他 HTTP 控制字符，query 参数只接受标量或标量列表；`json_body` 必须能编码成严格 JSON，`NaN` / `Infinity` 这类非有限数或运行时模板渲染出的不可序列化对象都会按 `json_body.score` / `json_body.payload` 这类路径提前诊断。settings/source 能静态渲染出的 URL/header/query/body 模板也会按渲染后结果进入 diagnostics；运行时模板若把 URL 渲染成相对地址、带 credentials、非法端口、含控制字符的地址，或把 header/query/body 渲染成非法协议形态，都会在发请求前 fail-fast；`execution_summary.url_origin` 与 `url_path` 也会做安全化处理，避免非法端口裸抛、半可信 origin 或 path 中的 `token=...` / `api_key/...` 泄漏进 trace/export，`response_path` / `result_fields.*` 只接受 dot field 与数字索引语法，避免非法 bracket 片段被部分解析成错误上游路径。
  - `http_json` 上游返回 HTTP 错误、连接/传输错误、2xx 非 JSON 响应或非法 UTF-8 响应体时，runtime 现在会把稳定的 HTTP status、脱敏限长后的 reason、`transport error` 或 `invalid JSON response` 与敏感键/敏感 assignment 脱敏后的有限 preview 一起落入错误信息；trace/observation/export 能看到可排障的 `HTTP 429 ... body: ...` / `transport error: ...` / `invalid JSON response ... body: ...` 语义，而不是只剩 urllib/JSON parser/codec 原始异常文案、未截断的低层异常字符串、HTTP reason 里的敏感 assignment，或 JSON error body 普通字符串值里的 `token=...` 原文。
  ...
  - 同一条请求侧 diagnostics 现在也会安全化敏感字段路径：`query_params.api_key`、`json_body.access_token`、`json_body.filters[0].client_secret` 这类协议错误或缺失模板变量路径会显示为 `[redacted]`，避免 settings/source diagnostics、runtime error、trace/export 为了定位请求配置问题而重新暴露敏感字段名。
  ...
  - `http_json` 的结果元数据现在也会进入同一条敏感字段治理：`execution_summary.result_field_names` 会对 `access_token` / `api_key` 一类字段名脱敏；显式配置的 `result_preview_keys` / `result_output_keys` 会过滤敏感 key，且当显式非空列表只剩敏感 key 时不会回退到模板默认 key，避免 settings/tool_start/trace/export 通过字段名或投影策略重新暴露敏感语义。
...
- 当前最近一次已记录校验基线：
  - `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py` 通过（`993/993`）
