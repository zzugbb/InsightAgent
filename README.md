...
- 最近代码主线已经打通：
  ...
  - settings/preflight 的 `tool_details` 与前端 model settings tool detail summary 现也继续透出并展示 `execution_summary`；不用进任务 trace，就能先看出某个 real tool 当前会打到哪类 HTTP endpoint。
