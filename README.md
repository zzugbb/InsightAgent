...
- 最近代码主线已经打通：
  ...
  - 同一套推断逻辑现在也会继续覆盖只有 `output_preview` 的旧 tool step；即使老 payload 没落 `output + effective_result_output_keys`，planner/retrieval/calc 的 trace preview、task export markdown 与前端 workbench 回放也会优先展示 `Planned steps - ...` / `Retrieved ...` 一类摘要，再附带 preview 片段。
