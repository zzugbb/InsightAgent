...
- `real-tool-execution`、`queue-and-concurrency-lite` 与 `concurrency-fairness-policy` 均已封板；当前主线进入 `registry-governance`，进度约 `29%`。
...
- `backend/scripts/test_tool_runtime_slice.py` 已拆到 `backend/scripts/tool_runtime_slice/`；`tool_runtime.py` 已拆出 planner、execution、HTTP JSON、registry 四个 facade 模块，外部 import 保持稳定；registry diagnostics summary、runtime/preflight/action 回灌、模型输出层、export artifact 与 task/usage response provider source 安全摘要已开始收紧。
- 默认运行策略保持不变：provider/model/api_key 完整时自动走 `remote`，否则回退 canonical `mock`。

## 当前验证基线

- `backend/.venv/bin/python backend/scripts/test_tool_runtime_slice.py`：`1799/1799` 通过
...
2. 当前主线：`registry-governance`，聚焦 selected source、settings/preflight、tool details、per-tool diagnostics、runtime semantic、trace/search/export 的治理语义；当前已收口 diagnostics summary 脱敏后去重、provider source values 安全化、runtime/preflight 回灌 totals 派生/安全规整、模型 `to_dict()` 输出层、task/session export artifact 与 task/usage response provider source 脱敏。
