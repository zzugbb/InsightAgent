...
| 阶段 5+ 产品化 | 进行中 | `full-data-auth` 首版已落地（JWT/多用户隔离/凭证加密），PostgreSQL 迁移主线已启动，更完整治理待后续 |
...
- backend：
  - 数据库后端支持 `INSIGHT_AGENT_DB_BACKEND=sqlite|postgres|auto`，可按环境切换 SQLite / PostgreSQL
  - 新增 `backend/scripts/migrate_sqlite_to_postgres.py`，支持 `users/user_settings/sessions/tasks/messages` 一次性平迁到 PostgreSQL（幂等 upsert）
...
- `SQLite`：系统“完整账本”。保存会话、消息、任务、trace、usage 的原始记录。
- `Chroma Memory`：当前会话“便签本”（`memory_{session_id}`）。保存可语义召回的会话记忆片段（含任务完成后的自动摘要）。
- `Chroma RAG`：项目“知识库”（`kb_{knowledge_base_id}`）。保存导入文档的分块内容，用于知识检索增强。

三者是互补，不是重复：

- 只用 `RAG` 不够：RAG 主要是外部知识，不包含你在本次对话里刚确认的偏好和约束。
- 只用 `SQLite` 不够：SQLite 是全量原文，直接每轮全量喂给模型成本高；Memory 用向量检索可只取最相关片段。
- 所以实际分工是：`SQLite` 管“真实历史”，`Memory` 管“会话记忆”，`RAG` 管“外部知识”。

一个直观例子：

- 你说“后续都用中文、回答不超过 5 行” -> 这类临时约束更适合进 Memory。
- 你导入产品手册/API 文档 -> 这类长期资料进 RAG。
- 你完整的对话与任务执行记录 -> 始终落 SQLite。
...
1. `full-data-auth`：已完成首版（用户体系 + JWT/API 鉴权 + 凭证加密存储），PostgreSQL 迁移主线已启动，后续补 refresh/session/audit 与更细粒度权限。
...
5. `full-qa-ops`（基础版）：后端 pytest、关键 e2e、CI 自动校验。
...
1. 本机开发：继续使用 `compose.yml` / `compose.full.yml`。
...
- 真实 LLM/Embedding provider 接入与质量联调
