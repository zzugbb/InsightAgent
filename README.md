...
- 当前主线为 `production-runtime-hardening`，进度约 65%；SSE `error.diagnostic` 与失败审计 detail 已对齐低敏 `reason` 枚举，前端审计详情可读展示该低敏原因，不改变旧字段。
...
- Frontend：node tests `123/123`、`npm run lint`、`npm run build` 通过。
