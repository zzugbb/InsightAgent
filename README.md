...
macOS 下也可在 Finder 中双击 [`start_insightagent.command`](/Users/gaobingbing/Desktop/code/person/InsightAgent/start_insightagent.command) 直接启动。
...
- `full-trace-session-lite` 与导出链路持续推进中：任务快照、单任务导出、会话导出均已落地；下一步进入 `remote-provider-hardening` 与主链路 e2e 收口。
- 先补 `remote-provider-hardening`，把真实模型联调错误体验收口，再完善导出链路的 e2e 校验。
