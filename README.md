...
- 最近代码主线已经打通：
  ...
  - `http_json.execution.method`、`timeout_ms` 与 tool 级 `default_timeout_ms` 现在也按真实协议配置治理：显式 method 必须是 `GET / POST / PUT / PATCH / DELETE`，显式 timeout 必须是正数毫秒；拼错不会再静默归一成 `GET`，坏 timeout 不会在 provider 构建时裸抛异常，而会提前进入 `invalid/tool_executions` 并在运行时 fail-fast 或安全回退到模板默认值。
