...
- 当前主线已进入 `rag-product-experience`，进度约 24%：知识库治理表可展开查看同一知识库下的版本明细，并按 source/document 汇总文档组摘要，展示每个文档组的版本数与 chunk 总数；文档组支持删除该 source/document 下全部 chunks，后端新增 `rag_document_delete` 审计事件；版本明细继续按 source/document/version 稳定排序，并展示 source、document_id、document_version 与 content_hash 摘要；RAG ingest/query/status/list、外部 SSE / trace / export / e2e 契约保持稳定。
