...
- backend：
  ...
  - 最小审计已落地：记录 `login/logout/refresh/settings_update` 到 `audit_logs`
  - 审计查询接口：`GET /api/audit/logs`（支持 `event_type/start_at/end_at` 与分页）
...
- frontend：
  ...
  - 首次引导页已落地：注册/登录后若未配置 API Key，先进入引导页完成 `provider/model/api_key` 保存（remote 模式）再进入工作台
  - 右侧 Context 新增最小审计面板：支持按事件类型与时间范围查看审计日志，并分页加载更多
