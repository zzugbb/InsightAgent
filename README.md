...
- 当前主线为 `ci-release-engineering`，进度约 78%；release gate 已支持 PR diff 自动分层、手动阶段选择、Markdown/JSON 摘要和 GitHub Actions summary artifact；release readiness matrix 与 backend/frontend e2e workflow 已覆盖静态门禁、service-backed main/timeout/queue、smoke/full/queue 与 artifact guard，main push artifact stage 严格度升级为 `fail-on-missing`。
