...
- `product-ux-polish` 已进入开发，当前约 25%：任务详情页 `trace_semantic` replay URL 已支持 `planner/retrieval/calculator/failure`，详情页语义筛选会同步可分享 URL；Task Center 在 `failure_trace` 筛选下打开详情会保持 Failure 回放焦点。
...
- Frontend：workbench utils targeted `69/69`、task detail targeted `7/7`；node tests `125/125`、`npm run lint`、`npm run build` 通过。
- E2E 基线：targeted Chromium semantic URL replay `1/1`、remote failure replay `1/1` 通过；backend main 通过；frontend full Chromium `52 passed / 1 skipped`；queue 阶段已纳入 backend/frontend CI workflow。
- Hygiene：`py_compile`、`git diff --check`、`git diff --cached --check`、备份计划 diff 检查通过；`data/insightagent.plan.back.md` 无修改。

## 当前开发计划

1. 当前状态：`product-ux-polish` 约 25%，trace 语义 replay URL、详情页语义筛选 URL 同步与 Task Center failure trace 详情焦点已补齐。
...
- 任务详情页 `trace_semantic` URL 参数保持兼容扩展：支持 `planner/retrieval/calculator/failure`，未知值回退 `all`；详情页语义切换仅同步 URL，不改变 trace/export payload。
