...
1. `concurrency-fairness-policy`：当前主线；已完成可选按用户/按 session 并发执行槽位上限、capacity-aware oldest eligible FIFO 防插队、settings `task_queue_diagnostics` 等待策略、全局/当前用户安全计数与压力状态诊断、backend queue e2e-like idle/压力诊断断言与安全 queue snapshot helper 覆盖、前端运行设置 active/waiting/available 可观测入口，下一步按风险补真实低并发 e2e 复验或更细公平策略。
