...
- `task-cancel-timeout` 后端 e2e（cancel + timeout）与 CI 首版已落地，`running-task-recovery` 前端首版与可视化提示已落地，`rag-kb-governance-lite`、`usage-dashboard-lite`、`audit-event-expansion` 与 `provider-usage-alignment` 已完成首版并补齐来源趋势联动；前端可视化回归 CI 已扩展到主链路导出/取消恢复/边界异常场景并接入 smoke 跨浏览器矩阵，本地最新回归为 chromium `25/25`、smoke `15/15`，下一步继续补异常态覆盖深度与跨会话切换下 UI 态断言。
