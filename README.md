...
- `real-tool-execution`、`queue-and-concurrency-lite` 与 `concurrency-fairness-policy` 均已封板；当前主线进入 `registry-governance`，进度约 `91%`。
...
- 本轮继续收口 settings summary/validate response，让当前选中 provider source 即使不在 available options 里也参与同一 alias map，避免与 options 中同 base source 编号错位。
