...
- frontend：
  ...
  - `full-trace-session` 重排收口：主工作台新增中间区 `chat | tasks` 双视图切换（左侧与右侧结构保持不变）；任务索引筛选/排序/搜索能力迁入中间“任务中心”；右侧下线任务级重复块（任务用量/任务快照/任务索引），任务级分析统一下沉到 `/tasks/[taskId]`
  - `full-trace-session` 清理收口：右侧 Inspector 旧任务块已从代码中物理删除（不再以样式隐藏保留），主入口以“任务中心 + 任务详情页”作为唯一任务分析路径
  - `full-trace-session` 回归对齐：前端 Playwright 用例已切换到“聊天头部任务中心入口 -> 任务中心 -> 任务详情导出”主链路，不再依赖已移除的 Inspector 任务导出选择器
  - `trace-export-json-md` 首版已落地：支持单任务导出 JSON 与 Markdown（含任务元信息、task-linked 消息、TraceStep、RAG chunks 与 usage），并在任务快照区提供一键导出入口
  - `session-export-lite` 首版已落地：支持当前会话导出 JSON 与 Markdown（会话消息、任务摘要、Trace 预览、RAG 命中统计、会话级 usage 汇总），入口已统一迁移到中栏头部“更多”菜单（chat/tasks 双视图一致）
