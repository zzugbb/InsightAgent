...
- `task-cancel-timeout` 后端 e2e（cancel + timeout）与 CI 首版已落地，`running-task-recovery` 前端首版与可视化提示已落地，`rag-kb-governance-lite` 已完成首版；下一步推进 `usage-dashboard-lite` 与 `audit-event-expansion`。
