...
- 最近代码主线已经打通：
  ...
  - `http_json` 的 URL、header/query value shape 与 response mapping path 也继续收紧：URL 必须是绝对 `http(s)` 地址，header 值只接受标量，query 参数只接受标量或标量列表，运行时模板若把 URL 渲染成相对地址或把 query 渲染成对象都会在发请求前 fail-fast；`response_path` / `result_fields.*` 也只接受 dot field 与数字索引语法，避免非法 bracket 片段被部分解析成错误上游路径。
  - `http_json` 上游返回 HTTP 错误、2xx 非 JSON 响应或非法 UTF-8 响应体时，runtime 现在会把稳定的 HTTP status/reason 或 `invalid JSON response` 与敏感键脱敏后的有限 body preview 一起落入错误信息；trace/observation/export 能看到可排障的 `HTTP 429 ... body: ...` / `invalid JSON response ... body: ...` 语义，而不是只剩 urllib/JSON parser/codec 原始异常文案。
